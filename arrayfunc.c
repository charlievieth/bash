...
static SHELL_VAR *assign_array_element_internal
    PARAMS((SHELL_VAR *, char *, char *, char *, int, char *, int, array_eltstate_t *));
...
/* Convert a shell variable to an array variable.  The original value is
   saved as array[0]. */
SHELL_VAR *convert_var_to_assoc(var)
SHELL_VAR *var;
{
	char *oldval;
	HASH_TABLE *hash;
...
char *make_array_variable_value(entry, ind, key, value, flags)
SHELL_VAR *entry;
arrayind_t ind;
char *key;
char *value;
...
static SHELL_VAR *bind_array_var_internal(entry, ind, key, value, flags)
SHELL_VAR *entry;
arrayind_t ind;
char *key;
char *value;
...
SHELL_VAR *bind_array_element(entry, ind, value, flags)
SHELL_VAR *entry;
arrayind_t ind;
char *value;
int flags;
...
SHELL_VAR *bind_assoc_variable(entry, name, key, value, flags)
SHELL_VAR *entry;
char *name;
char *key;
char *value;
...
inline void flush_eltstate(array_eltstate_t *estatep) {
	if (estatep)
		FREE(estatep->key);
}
...
array_eltstate_t *estatep;
{
	...
	if (((isassoc == 0 || (flags & (ASS_NOEXPAND | ASS_ALLOWALLSUB)) == 0) &&
	    ...
	    (sublen <= 1) || (sub[sublen] != '\0')) /* sanity check */
	{
		free(vname);
...
WORD_LIST *expand_compound_array_assignment(var, value, flags)
SHELL_VAR *var;
char *value;
int flags;
{
...
static void assign_assoc_from_kvlist(var, nlist, h, flags) SHELL_VAR *var;
WORD_LIST *nlist;
HASH_TABLE *h;
int flags;
{
...
/* Return non-zero if L appears to be a key-value pair associative array
   compound assignment. */
int kvpair_assignment_p(l)
WORD_LIST *l;
{ return (l && (l->word->flags & W_ASSIGNMENT) == 0 && l->word->word[0] != '['); /*]*/ }

char *expand_and_quote_kvpair_word(w)
char *w;
{
	char *r, *s, *t;
...
void assign_compound_array_list(var, nlist, flags) SHELL_VAR *var;
WORD_LIST *nlist;
int flags;
{
	ARRAY *a;
	...
	for (list = nlist; list; list = list->next) {
	   ...
	   existing values by default. */
		...
		if ((list->word->flags & W_ASSIGNMENT) && w[0] == '[') {
			...
			set_exit_status(EXECUTION_FAILURE);
			report_error(_("%s: %s: must use subscript when assigning associative array"),
			             var->name, w);
			continue;
...
static char *quote_assign(string) const char *string;
{
	size_t slen;
	int saw_eq;
	char *temp, *t, *subs;
...
char *expand_and_quote_assoc_word(w, type)
char *w;
int type;
{
...
void quote_compound_array_list(list, type) WORD_LIST *list;
int type;
{
	char *s, *t;
	WORD_LIST *l;
...
void print_assoc_assignment(var, quoted) SHELL_VAR *var;
int quoted;
{
	char *vstr;
...
int tokenize_array_reference(name, flags, subp)
char *name;
int flags;
char **subp;
{
...
arrayind_t array_expand_index(var, s, len, flags)
SHELL_VAR *var;
char *s;
int len;
int flags;
{
  ...
  if ((flags & AV_NOEXPAND) == 0)
    ...
    t = exp;
#else
	t = expand_arith_string(exp, Q_DOUBLE_QUOTES | Q_ARITH |
...
char *array_variable_name(s, flags, subp, lenp) const char *s;
int flags;
char **subp;
int *lenp;
{
...
static char *array_value_internal(s, quoted, flags, estatep) const char *s;
int quoted, flags;
array_eltstate_t *estatep;
{
