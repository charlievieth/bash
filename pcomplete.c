...
static STRINGLIST *gen_shell_function_matches
    PARAMS((COMPSPEC *, const char *, const char *, char *, int, WORD_LIST *, int, int, int *));
static STRINGLIST *gen_command_matches
    PARAMS((COMPSPEC *, const char *, const char *, char *, int, WORD_LIST *, int, int));

static STRINGLIST *gen_progcomp_completions
    PARAMS((const char *, const char *, const char *, int, int, int *, int *, COMPSPEC **));
...
#else
    debug_printf(format, va_alist) const char *format;
va_dcl
#endif
{
	va_list args;
...
void set_itemlist_dirty(it) ITEMLIST *it;
{ it->flags |= LIST_DIRTY; }
...
STRINGLIST *filter_stringlist(sl, filterpat, text)
STRINGLIST *sl;
char *filterpat;
const char *text;
{
...
STRINGLIST *completions_to_stringlist(matches)
char **matches;
{
	STRINGLIST *sl;
	int mlen, i, n;
...
static int it_init_aliases(itp)
ITEMLIST *itp;
{
#ifdef ALIAS
	alias_t **alias_list;
...
static void init_itemlist_from_varlist(itp, svfunc) ITEMLIST *itp;
SVFUNC *svfunc;
{
	SHELL_VAR **vlist;
	STRINGLIST *sl;
...
static int it_init_bindings(itp)
ITEMLIST *itp;
{
	char **blist;
	STRINGLIST *sl;
...
static int it_init_builtins(itp)
ITEMLIST *itp;
{
	STRINGLIST *sl;
	register int i, n;
...
static int it_init_enabled(itp)
ITEMLIST *itp;
{
	STRINGLIST *sl;
	register int i, n;
...
static int it_init_disabled(itp)
ITEMLIST *itp;
{
	STRINGLIST *sl;
	register int i, n;
...
static int it_init_exported(itp)
ITEMLIST *itp;
{
...
static int it_init_hostnames(itp)
ITEMLIST *itp;
{
	STRINGLIST *sl;
...
static int it_init_joblist(itp, jstate)
ITEMLIST *itp;
int jstate;
{
#if defined(JOB_CONTROL)
...
static int it_init_signals(itp)
ITEMLIST *itp;
{
	STRINGLIST *sl;
...
static int it_init_variables(itp)
ITEMLIST *itp;
{
...
static int it_init_setopts(itp)
ITEMLIST *itp;
{
	STRINGLIST *sl;
...
/* Generate a list of all matches for TEXT using the STRINGLIST in itp->slist
   ...
   match. */
static STRINGLIST *gen_matches_from_itemlist(itp, text)
ITEMLIST *itp;
const char *text;
{
	STRINGLIST *ret, *sl;
	...
	if ((itp->flags & (LIST_DIRTY | LIST_DYNAMIC)) || (itp->flags & LIST_INITIALIZED) == 0) {
...
static char *pcomp_filename_completion_function(text, state) const char *text;
int state;
{
	...
	if (state == 0) {
	   ...
	   calling readline, do the dequoting. */
		else if (iscompgen && iscompleting && pcomp_curtxt && *pcomp_curtxt == 0 && text &&
		         (*text == '\'' || *text == '"') && text[1] == text[0] && text[2] == 0 &&
		         rl_filename_dequoting_function)
			dfn = (*rl_filename_dequoting_function)((char *)text, rl_completion_quote_character);
	   ...
	   checking variable_context, and may end up doing that. */
		else if (iscompgen && iscompleting && rl_filename_dequoting_function && pcomp_curtxt &&
		         text && STREQ(pcomp_curtxt, text) == 0 && variable_context &&
...
static STRINGLIST *gen_action_completions(cs, text)
COMPSPEC *cs;
const char *text;
{
	...
	GEN_XCOMPS(flags, CA_SERVICE, text, bash_servicename_completion_function, cmatches, ret,
	           tmatches);
...
static STRINGLIST *gen_wordlist_matches(cs, text)
COMPSPEC *cs;
const char *text;
{
	WORD_LIST *l, *l2;
	...
	l = split_at_delims(cs->words, strlen(cs->words), (char *)NULL, -1, 0, (int *)NULL,
	                    (int *)NULL);
	if (l == 0)
		return ((STRINGLIST *)NULL);
...
static SHELL_VAR *bind_comp_words(lwords)
WORD_LIST *lwords;
{
	SHELL_VAR *v;
...
static void bind_compfunc_variables(line, ind, lwords, cw, exported) char *line;
int ind;
WORD_LIST *lwords;
int cw, exported;
{
...
static void unbind_compfunc_variables(exported) int exported;
{
...
static WORD_LIST *build_arg_list(cmd, cname, text, lwords, ind)
char *cmd;
const char *cname;
const char *text;
WORD_LIST *lwords;
...
static STRINGLIST *gen_shell_function_matches(cs, cmd, text, line, ind, lwords, nw, cw, foundp)
COMPSPEC *cs;
const char *cmd;
const char *text;
char *line;
...
static STRINGLIST *gen_command_matches(cs, cmd, text, line, ind, lwords, nw, cw)
COMPSPEC *cs;
const char *cmd;
const char *text;
char *line;
...
STRINGLIST *gen_compspec_completions(cs, cmd, word, start, end, foundp)
COMPSPEC *cs;
const char *cmd;
const char *word;
int start, end;
int *foundp;
{
...
#ifdef DEBUG
		debug_printf("command_line_to_word_list (%s, %d, %d, %p, %p)", line, llen,
		             pcomp_ind - start, &nw, &cw);
#endif
	...
	if (cs->funcname) {
		foundf = 0;
		tmatches = gen_shell_function_matches(cs, cmd, word, line, pcomp_ind - start, lwords, nw,
		                                      cw, &foundf);
		if (foundf != 0)
			found = foundf;
		if (tmatches) {
#ifdef DEBUG
			if (progcomp_debug) {
				debug_printf("gen_shell_function_matches (%p, %s, %s, %p, %d, %d) -->", cs, cmd,
				             word, lwords, nw, cw);
				strlist_print(tmatches, "\t");
				rl_on_new_line();
			}
...
#ifdef DEBUG
			if (progcomp_debug) {
				debug_printf("gen_command_matches (%p, %s, %s, %p, %d, %d) -->", cs, cmd, word,
				             lwords, nw, cw);
				strlist_print(tmatches, "\t");
				rl_on_new_line();
			}
...
void pcomp_set_readline_variables(flags, nval) int flags, nval;
{
...
void pcomp_set_compspec_options(cs, flags, set_or_unset) COMPSPEC *cs;
...
static STRINGLIST *
    gen_progcomp_completions(ocmd, cmd, word, start, end, foundp, retryp, lastcs) const char *ocmd;
const char *cmd;
const char *word;
int start, end;
int *foundp, *retryp;
...
/* The driver function for the programmable completion code.  Returns a list
   ...
   rl_line_buffer). */
char **programmable_completions(cmd, word, start, end, foundp) const char *cmd;
const char *word;
int start, end, *foundp;
{
	...
	do {
	   ...
	   an associated COMPSPEC. */
		...
		if (found == 0)
			ret = gen_progcomp_completions(DEFAULTCMD, ocmd, word, start, oend, &found, &retry,
			                               &lastcs);
