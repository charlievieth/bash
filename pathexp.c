...
int unquoted_glob_pattern_p(string)
...
int glob_char_p(s) const char *s;
{
	switch (*s) {
	case '*':
	case '[':
...
char *quote_globbing_chars(string) const char *string;
{
	size_t slen;
	char *temp, *t;
	const char *s, *send;
...
void setup_glob_ignore(name) char *name;
{
	char *v;
...
int should_ignore_glob_matches() {
...
void ignore_glob_matches(names) char **names;
{
...
void setup_ignore_patterns(ivp) struct ignorevar *ivp;
{
