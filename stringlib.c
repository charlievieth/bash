...
char *find_token_in_alist(token, alist, flags)
int token;
STRING_INT_ALIST *alist;
int flags;
{
...
int find_index_in_alist(string, alist, flags)
char *string;
STRING_INT_ALIST *alist;
int flags;
{
...
/* Cons a new string from STRING starting at START and ending at END,
   not including END. */
char *substring(string, start, end) const char *string;
int start, end;
{
	register int len;
...
char *strsub(string, pat, rep, global)
char *string, *pat, *rep;
int global;
{
...
char *strcreplace(string, c, text, flags)
char *string;
int c;
const char *text;
int flags;
...
void strip_leading(string) char *string;
{
	char *start = string;
...
void strip_trailing(string, len, newlines_only) char *string;
int len;
int newlines_only;
{
	while (len >= 0) {
