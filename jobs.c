...
#define WAITPID(pid, statusp, options) wait3((union wait *)statusp, options, (struct rusage *)0)
#else
#if defined(_POSIX_VERSION) || defined(HAVE_WAITPID)
#define WAITPID(pid, statusp, options) waitpid((pid_t)pid, statusp, options)
#else
#if defined(HAVE_WAIT3)
#define WAITPID(pid, statusp, options) wait3(statusp, options, (struct rusage *)0)
#else
#define WAITPID(pid, statusp, options) wait3(statusp, options, (int *)0)
#endif /* HAVE_WAIT3 */
...
static struct pipeline_saver *alloc_pipeline_saver() {
...
void save_pipeline(clear) int clear;
{
	sigset_t set, oset;
...
PROCESS *restore_pipeline(discard)
int discard;
{
...
COMMAND *deferred;
{
	...
	if ((interactive_shell == 0 || subshell_environment) && i == js.j_jobslots &&
...
static ps_index_t bgp_getindex() {
...
static ps_index_t *pshash_getbucket(pid)
pid_t pid;
{
...
static struct pidstat *bgp_add(pid, status)
pid_t pid;
int status;
{
...
static void pshash_delindex(psi) ps_index_t psi;
{
	struct pidstat *ps;
	ps_index_t *bucket;
...
void save_proc_status(pid, status) pid_t pid;
int status;
{
	sigset_t set, oset;
...
static void procsub_free(p) PROCESS *p;
{
	FREE(p->command);
	free(p);
}

PROCESS *procsub_add(p)
PROCESS *p;
{
	sigset_t set, oset;
...
static void cleanup_dead_jobs() {
	...
	for (i = 0; i < js.j_jobslots; i++) {
		if (i < js.j_firstj && jobs[i])
			INTERNAL_DEBUG(
			    ("cleanup_dead_jobs: job %d non-null before js.j_firstj (%d)", i, js.j_firstj));
		if (i > js.j_lastj && jobs[i])
			INTERNAL_DEBUG(
			    ("cleanup_dead_jobs: job %d non-null after js.j_lastj (%d)", i, js.j_lastj));
...
static int processes_in_job(job)
int job;
{
	int nproc;
	register PROCESS *p;
...
static void delete_old_job(pid) pid_t pid;
{
	PROCESS *p;
	int job;
	...
	if (job != NO_JOB) {
		INTERNAL_DEBUG(
		    ("delete_old_job: found pid %d in job %d with state %d", pid, job, jobs[job]->state));
...
static void realloc_jobs_list() {
  ...
  itrace ("realloc_jobs_list: js.j_ndead %d js.c_reaped %d", js.j_ndead, js.c_reaped);
	...
	/* Need to reset these */
	if (js.j_current == NO_JOB || js.j_previous == NO_JOB || js.j_current > js.j_lastj ||
...
static int compact_jobs_list(flags)
int flags;
{
	if (js.j_jobslots == 0 || jobs_list_frozen)
...
void delete_job(job_index, dflags) int job_index, dflags;
{
	register JOB *temp;
	PROCESS *proc;
	int ndel;
	...
	if ((dflags & DEL_WARNSTOPPED) && subshell_environment == 0 && STOPPED(job_index))
		internal_warning(_("deleting stopped job %d with process group %ld"), job_index + 1,
		                 (long)jobs[job_index]->pgrp);
	temp = jobs[job_index];
	if (temp == 0)
		return;
	...
	if (temp->state == JDEAD) {
	   ...
	   mess with this total. */
		...
		if (js.c_reaped < 0) {
			INTERNAL_DEBUG(
			    ("delete_job (%d pgrp %d): js.c_reaped (%d) < 0 ndel = %d js.j_ndead = %d",
...
/* Must be called with SIGCHLD blocked. */
void nohup_job(job_index) int job_index;
{
	register JOB *temp;
...
static void add_process(name, pid) char *name;
pid_t pid;
{
	PROCESS *t, *p;
	...
	if (p) {
		if (j == NO_JOB)
			internal_debug("add_process: process %5ld (%s) in the_pipeline", (long)p->pid,
			               p->command);
		if (PALIVE(p))
			internal_warning(_("add_process: pid %5ld (%s) marked as still alive"), (long)p->pid,
			                 p->command);
...
void append_process(name, pid, status, jid) char *name;
pid_t pid;
int status;
int jid;
{
...
/* Map FUNC over the list of jobs.  If FUNC returns non-zero,
   ...
   and INDEX. */
static int map_over_jobs(func, arg1, arg2)
...
int arg1, arg2;
{
	...
	for (i = result = 0; i < js.j_jobslots; i++) {
		if (i < js.j_firstj && jobs[i])
			INTERNAL_DEBUG(
			    ("map_over_jobs: job %d non-null before js.j_firstj (%d)", i, js.j_firstj));
		if (i > js.j_lastj && jobs[i])
...
static PROCESS *find_pid_in_pipeline(pid, pipeline, alive_only)
pid_t pid;
PROCESS *pipeline;
int alive_only;
{
...
static PROCESS *find_pipeline(pid, alive_only, jobp)
pid_t pid;
int alive_only;
...
/* Return the PROCESS * describing PID.  If JOBP is non-null return the index
   ...
   SIGCHLD blocked. */
static PROCESS *find_process(pid, alive_only, jobp)
pid_t pid;
int alive_only;
int *jobp; /* index into jobs list or NO_JOB */
{
...
static int find_job(pid, alive_only, procp)
pid_t pid;
int alive_only;
PROCESS **procp;
{
...
int get_job_by_pid(pid, block, procp)
pid_t pid;
int block;
PROCESS **procp;
{
...
static char *printable_job_status(j, p, format)
int j;
PROCESS *p;
int format;
{
	...
	for (;;) {
		...
		if (format > -1 && job_index >= 0) {
			...
			if (temp) {
				...
				if ((WIFSTOPPED(show->status) == 0) && (WIFCONTINUED(show->status) == 0) &&
		...
		if (p == last && job_index >= 0) {
			...
			if (strcmp(temp, jobs[job_index]->wd) != 0)
				fprintf(stream, _("  (wd: %s)"), polite_directory_format(jobs[job_index]->wd));
		}
...
static void pretty_print_job(job_index, format, stream) int job_index, format;
FILE *stream;
{
	register PROCESS *p;
...
static int print_job(job, format, state, job_index)
JOB *job;
...
pid_t make_child(command, flags)
char *command;
int flags;
{
	...
	if (pid == 0) {
	   ...
	   and it's wrong to close the file in that case. */
		...
		if (job_control) {
			...
			if ((flags & FORK_NOTERM) == 0 && async_p == 0 && pipeline_pgrp != shell_pgrp &&
...
static void draino(fd, ospeed) int fd, ospeed;
{
...
static PROCESS *find_last_proc(job, block)
int job;
int block;
{
	register PROCESS *p;
...
int wait_for_single_pid(pid, flags)
pid_t pid;
int flags;
{
...
int wait_for_background_pids(ps)
struct procstat *ps;
{
	register int i, r;
	...
	for (njobs = any_stopped = 0, check_async = 1;;) {
		...
		for (i = 0; i < js.j_jobslots; i++) {
			if (i < js.j_firstj && jobs[i])
				INTERNAL_DEBUG(("wait_for_background_pids: job %d non-null before js.j_firstj (%d)",
				                i, js.j_firstj));
			if (i > js.j_lastj && jobs[i])
				INTERNAL_DEBUG(("wait_for_background_pids: job %d non-null after js.j_lastj (%d)",
				                i, js.j_lastj));
...
static sighandler wait_sigint_handler(sig)
int sig;
{
	...
	if (this_shell_builtin && this_shell_builtin == wait_builtin) {
	   ...
	   what POSIX.2 says (see builtins/wait.def for more info). */
		if (this_shell_builtin && this_shell_builtin == wait_builtin && signal_is_trapped(SIGINT) &&
...
static int process_exit_signal(status)
WAIT status;
{ return (WIFSIGNALED(status) ? WTERMSIG(status) : 0); }

static int process_exit_status(status)
WAIT status;
{
...
static WAIT job_signal_status(job)
int job;
{
	register PROCESS *p;
	WAIT s;
...
static WAIT raw_job_exit_status(job)
int job;
{
	register PROCESS *p;
	int fail;
...
int job_exit_status(job)
int job;
{ return (process_exit_status(raw_job_exit_status(job))); }

int job_exit_signal(job)
int job;
...
itrace("wait_for: blocking wait for %d returns %d child = %p", (int)pid, r, child);
	...
	last_command_exit_signal =
	    (job != NO_JOB) ? job_exit_signal(job) : (child ? process_exit_signal(child->status) : 0);
		...
		if (interactive_shell && subshell_environment == 0) {
			...
			if (WIFSIGNALED(s) || WIFSTOPPED(s)) {
			   ...
			   the user has requested it, get a possibly new window size */
				...
				if (RL_ISSTATE(RL_STATE_COMPLETING | RL_STATE_DISPATCHING | RL_STATE_TERMPREPPED) ==
				    0)
#endif
					get_tty_state();
			...
			if (job_control && IS_JOBCONTROL(job) && IS_FOREGROUND(job) && WIFSIGNALED(s) &&
			    WTERMSIG(s) == SIGINT) {
...
static int most_recent_job_in_state(job, state)
int job;
JOB_STATE state;
{
...
static void set_current_job(job) int job;
{
	...
	if (js.j_previous != js.j_current && js.j_previous != NO_JOB && jobs[js.j_previous] &&
	...
	candidate =
	    RUNNING(js.j_current) ? job_last_running(js.j_current) : job_last_running(js.j_jobslots);
...
/* Set up the job structures so we know the job and its processes are
   all running. */
static void set_job_running(job) int job;
{
	register PROCESS *p;
...
int job, foreground;
{
	...
	do {
		printf("%s%s", p->command ? p->command : "", p->next != jobs[job]->pipe ? " | " : "");
		p = p->next;
	} while (p != jobs[job]->pipe);
...
static sighandler sigchld_handler(sig)
int sig;
{
	int n, oerrno;
...
/* waitchld() reaps dead or stopped children.  It's called by wait_for and
   ...
   processes. */
static int waitchld(wpid, block)
pid_t wpid;
int block;
{
	WAIT status;
	...
	do {
	   ...
	   is not active.  XXX - was interactive_shell instead of job_control */
		waitpid_flags = (job_control && subshell_environment == 0) ? (WUNTRACED | wcontinued) : 0;
		if (sigchld || block == 0)
...
static int set_job_status_and_cleanup(job)
int job;
{
	PROCESS *child;
	...
	if (JOBSTATE(job) == JDEAD) {
	   ...
	   now it does this if any process in the pipeline exits due to SIGINT. */
		else if (wait_sigint_received && child_caught_sigint == 0 && IS_FOREGROUND(job) &&
		         IS_JOBCONTROL(job) == 0) {
			int old_frozen;
...
static void setjstatus(j) int j;
{
#if defined(ARRAY_VARS)
	register int i;
	register PROCESS *p;
...
void run_sigchld_trap(nchild) int nchild;
{
	char *trap_command;
	int i;
...
static void notify_of_job_status() {
	...
	for (job = 0, dir = (char *)NULL; job < js.j_jobslots; job++) {
		if (jobs[job] && IS_NOTIFIED(job) == 0) {
			...
			if ((job_control == 0 && interactive_shell) ||
			   ...
			   a `wait'. */
				if (DEADJOB(job) &&
				    (interactive_shell || (find_last_pid(job, 0) != last_asynchronous_pid)))
					jobs[job]->flags |= J_NOTIFIED;
				continue;
			}
			...
			switch (JOBSTATE(job)) {
			case JDEAD:
				if (interactive_shell == 0 && termsig && WIFSIGNALED(s) && termsig != SIGINT &&
...
int initialize_job_control(force)
int force;
{
	pid_t t;
...
void debug_print_pgrps() {
	itrace("original_pgrp = %ld shell_pgrp = %ld terminal_pgrp = %ld", (long)original_pgrp,
	       (long)shell_pgrp, (long)terminal_pgrp);
	itrace("tcgetpgrp(%d) -> %ld, getpgid(0) -> %ld", shell_tty, (long)tcgetpgrp(shell_tty),
	       (long)getpgid(0));
...
static int set_new_line_discipline(tty)
int tty;
{
#if defined(NEW_TTY_DRIVER)
	int ldisc;
...
static sighandler sigcont_sighandler(sig)
int sig;
{
...
static sighandler sigstop_sighandler(sig)
int sig;
{
...
int give_terminal_to(pgrp, force)
pid_t pgrp;
int force;
{
	sigset_t set, oset;
...
static int maybe_give_terminal_to(opgrp, npgrp, flags)
pid_t opgrp, npgrp;
int flags;
{
	int tpgrp;
	...
	if (tpgrp == npgrp) {
		...
		internal_debug("%d: maybe_give_terminal_to: terminal pgrp == %d shell pgrp = %d new pgrp = "
		               "%d in_background = %d",
		               (int)getpid(), tpgrp, opgrp, npgrp, running_in_background);
		return -1;
	} else
...
void delete_all_jobs(running_only) int running_only;
{
	register int i;
	sigset_t set, oset;
	...
	if (js.j_jobslots) {
		...
		for (i = 0; i < js.j_jobslots; i++) {
			if (i < js.j_firstj && jobs[i])
				INTERNAL_DEBUG(
				    ("delete_all_jobs: job %d non-null before js.j_firstj (%d)", i, js.j_firstj));
			if (i > js.j_lastj && jobs[i])
				INTERNAL_DEBUG(
				    ("delete_all_jobs: job %d non-null after js.j_lastj (%d)", i, js.j_lastj));
...
void nohup_all_jobs(running_only) int running_only;
{
	register int i;
	sigset_t set, oset;
...
int count_all_jobs() {
	...
	for (i = n = 0; i < js.j_jobslots; i++) {
		if (i < js.j_firstj && jobs[i])
			INTERNAL_DEBUG(
			    ("count_all_jobs: job %d non-null before js.j_firstj (%d)", i, js.j_firstj));
		if (i > js.j_lastj && jobs[i])
			INTERNAL_DEBUG(
			    ("count_all_jobs: job %d non-null after js.j_lastj (%d)", i, js.j_lastj));
...
static void mark_dead_jobs_as_notified(force) int force;
{
	...
	if (force) {
		...
		for (i = 0; i < js.j_jobslots; i++) {
			if (jobs[i] && DEADJOB(i) &&
			    (interactive_shell || (find_last_pid(i, 0) != last_asynchronous_pid)))
	...
	for (i = ndead = ndeadproc = 0; i < js.j_jobslots; i++) {
		if (i < js.j_firstj && jobs[i])
			INTERNAL_DEBUG(("mark_dead_jobs_as_notified: job %d non-null before js.j_firstj (%d)",
			                i, js.j_firstj));
		if (i > js.j_lastj && jobs[i])
			INTERNAL_DEBUG(("mark_dead_jobs_as_notified: job %d non-null after js.j_lastj (%d)", i,
			                js.j_lastj));
  ...
  if (ndeadproc != js.c_reaped)
	...
	if (ndead != js.j_ndead)
		INTERNAL_DEBUG(
		    ("mark_dead_jobs_as_notified: ndead (%d) != js.j_ndead (%d)", ndead, js.j_ndead));
...
itrace("mark_dead_jobs_as_notified: child_max = %d ndead = %d ndeadproc = %d", js.c_childmax, ndead, ndeadproc);
	...
	for (i = 0; i < js.j_jobslots; i++) {
		if (jobs[i] && DEADJOB(i) &&
			...
			if (i < js.j_firstj && jobs[i])
				INTERNAL_DEBUG(
				    ("mark_dead_jobs_as_notified: job %d non-null before js.j_firstj (%d)", i,
				     js.j_firstj));
			if (i > js.j_lastj && jobs[i])
				INTERNAL_DEBUG(("mark_dead_jobs_as_notified: job %d non-null after js.j_lastj (%d)",
				                i, js.j_lastj));
...
void set_jobs_list_frozen(s) int s;
{ jobs_list_frozen = s; }

/* Allow or disallow job control to take place.  Returns the old value
   of job_control. */
int set_job_control(arg)
int arg;
{
	int old;
...
void set_maxchild(nchild) int nchild;
{
...
static void pipe_read(pp) int *pp;
{
	char ch;
...
void save_pgrp_pipe(p, clear) int *p;
int clear;
{
	p[0] = pgrp_pipe[0];
	p[1] = pgrp_pipe[1];
...
void restore_pgrp_pipe(p) int *p;
{
	pgrp_pipe[0] = p[0];
	pgrp_pipe[1] = p[1];
}
