...
static void pushexp() {
	...
	if (expr_depth >= expr_stack_size) {
		...
		expr_stack =
		    (EXPR_CONTEXT **)xrealloc(expr_stack, expr_stack_size * sizeof(EXPR_CONTEXT *));
	}
...
static void expr_bind_variable(lhs, rhs) char *lhs, *rhs;
{
	SHELL_VAR *v;
	int aflags;
...
static int expr_skipsubscript(vp, cp)
char *vp, *cp;
{
	int flags, isassoc;
	SHELL_VAR *entry;
...
static void expr_bind_array_element(tok, ind, rhs) char *tok;
arrayind_t ind;
char *rhs;
{
	char *lhs, *vname;
...
static void init_lvalue(lv) struct lvalue *lv;
{
	lv->tokstr = 0;
	lv->tokvar = 0;
	lv->tokval = lv->ind = -1;
...
static void free_lvalue(lv) struct lvalue *lv;
...
static void evalerror(msg) const char *msg;
{
	char *name, *t;
	...
	for (t = expression; t && whitespace(*t); t++)
		;
	internal_error(_("%s%s%s: %s (error token is \"%s\")"), name ? name : "", name ? ": " : "",
...
int builtin_error(format, arg1, arg2, arg3, arg4, arg5)
char *format;
{
	fprintf(stderr, "expr: ");
