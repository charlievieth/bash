...
static char *heredoc_expand(redirectee, ri, lenp)
...
static int heredoc_write(fd, heredoc, herelen)
int fd;
char *heredoc;
size_t herelen;
{
...
static int here_document_to_fd(redirectee, ri)
WORD_DESC *redirectee;
enum r_instruction ri;
{
	char *filename;
...
use_tempfile:
	...
	if (fstat(fd, &st1) < 0 || S_ISREG(st1.st_mode) == 0 || fstat(fd2, &st2) < 0 ||
	    S_ISREG(st2.st_mode) == 0 || same_file(filename, filename, &st1, &st2) == 0) {
		unlink(filename);
		free(filename);
		close(fd);
		close(fd2);
...
static int redir_special_open(spec, filename, flags, mode, ri)
int spec;
char *filename;
int flags, mode;
enum r_instruction ri;
...
static int noclobber_open(filename, flags, mode, ri)
char *filename;
int flags, mode;
enum r_instruction ri;
{
	...
	if ((fstat(fd, &finfo2) == 0) && (S_ISREG(finfo2.st_mode) == 0) && r == 0 &&
	    (S_ISREG(finfo.st_mode) == 0) && same_file(filename, filename, &finfo, &finfo2))
		return fd;
...
static int do_redirection_internal(redirect, flags, fnp)
REDIRECT *redirect;
int flags;
char **fnp;
{
	...
	if (TRANSLATE_REDIRECT(ri)) {
	   ...
	   continue. */
		...
		} else if (ri == r_duplicating_output_word && (redirect->rflags & REDIR_VARASSIGN) == 0 &&
		           redirector == 1) {
			sd = redirect->redirector;
	   ...
	   new redirection. */
		if (new_redirect->instruction == r_err_and_out) {
			...
			xbcopy((char *)new_redirect->redirectee.filename, (char *)redirectee,
			       sizeof(WORD_DESC));

			alloca_hack = (char *)alloca(1 + strlen(new_redirect->redirectee.filename->word));
	...
	case r_output_force:
		...
		if (flags & RX_ACTIVE) {
			...
			if ((flags & RX_UNDOABLE) &&
			    ((redirect->rflags & REDIR_VARASSIGN) == 0 || varassign_redir_autoclose)) {
	...
	case r_move_output:
	  ...
	  if (((fcntl (redir_fd, F_GETFD, 0) == 1) || redir_fd < 2 || (flags & RX_CLEXEC)) &&
	       (redirector > 2))
#else
			if (((fcntl(redir_fd, F_GETFD, 0) == 1) || (redir_fd < 2 && (flags & RX_INTERNAL)) ||
			     (flags & RX_CLEXEC)) &&
			    (redirector > 2))
#endif
				SET_CLOSE_ON_EXEC(redirector);
			...
			if ((redirect->flags & RX_INTERNAL) && (redirect->flags & RX_SAVCLEXEC) &&
			    redirector >= 3 && (redir_fd >= SHELL_FD_BASE || (redirect->flags & RX_SAVEFD)))
...
static int add_undo_redirect(fd, ri, fdbase)
int fd;
enum r_instruction ri;
int fdbase;
{
...
static int add_undo_close_redirect(fd)
int fd;
{
	REDIRECT *closer;
	REDIRECTEE sd;
...
static void add_exec_redirect(dummy_redirect) REDIRECT *dummy_redirect;
{
...
/* Return 1 if the redirection specified by RI and REDIRECTOR alters the
   standard input. */
static int stdin_redirection(ri, redirector) enum r_instruction ri;
int redirector;
{
	switch (ri) {
	case r_input_direction:
...
static int redir_varassign(redir, fd)
REDIRECT *redir;
int fd;
{
	WORD_DESC *w;
