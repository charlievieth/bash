...
#define VALID_SPECIAL_LENGTH_PARAM(c) ((c) == '-' || (c) == '?' || (c) == '#' || (c) == '@')

/* Evaluates to 1 if C is one of the shell's special parameters for which an
   indirect variable reference may be made. */
#define VALID_INDIR_PARAM(c)                                                                       \
	((posixly_correct == 0 && (c) == '#') || (posixly_correct == 0 && (c) == '?') || (c) == '@' || \
	 (c) == '*')
...
#endif
static char *parameter_brace_remove_pattern
    PARAMS((char *, char *, array_eltstate_t *, char *, int, int, int));
...
#endif
static char *parameter_brace_transform
    PARAMS((char *, char *, array_eltstate_t *, char *, int, int, int, int));
...
static int
    verify_substring_values PARAMS((SHELL_VAR *, char *, char *, int, intmax_t *, intmax_t *));
static int
    get_var_and_type PARAMS((char *, char *, array_eltstate_t *, int, int, SHELL_VAR **, char **));
static char *mb_substring PARAMS((char *, int, int));
static char *
    parameter_brace_substring PARAMS((char *, char *, array_eltstate_t *, char *, int, int, int));
...
static char *
    parameter_brace_patsub PARAMS((char *, char *, array_eltstate_t *, char *, int, int, int));

static char *pos_params_casemod PARAMS((char *, char *, int, int));
static char *parameter_brace_casemod
    PARAMS((char *, char *, array_eltstate_t *, int, char *, int, int, int));
...
#if defined(DEBUG)
void dump_word_flags(flags) int flags;
{
	int f;
...
static char *quoted_substring(string, start, end)
char *string;
int start, end;
{
	register int len, l;
...
static char *quoted_strchr(s, c, flags)
char *s;
int c, flags;
{
	register char *p;
	...
	for (p = s; *p; p++) {
		if (((flags & ST_BACKSL) && *p == '\\') || ((flags & ST_CTLESC) && *p == CTLESC)) {
			p++;
			if (*p == '\0')
				return ((char *)NULL);
			continue;
...
static int unquoted_member(character, string)
int character;
char *string;
{
	size_t slen;
...
static int unquoted_substring(substr, string)
char *substr, *string;
{
	size_t slen;
	int sindex, c, sublen;
...
static char *string_extract(string, sindex, charlist, flags)
char *string;
int *sindex;
char *charlist;
int flags;
...
static char *string_extract_double_quoted(string, sindex, flags)
char *string;
int *sindex, flags;
{
	size_t slen;
...
static inline int skip_single_quoted(string, slen, sind, flags) const char *string;
size_t slen;
int sind;
int flags;
{
...
int xflags;
{
	...
	if (string[*sindex] == LPAREN || (xflags & SX_COMPLETE))
		return (
		    extract_delimited_string(string, sindex, "$(", "(", ")", xflags | SX_COMMAND)); /*)*/
	else {
...
/* Extract and create a new string from the contents of STRING, a
   ...
   needs to be skipped. */
static char *extract_delimited_string(string, sindex, opener, alt_opener, closer, flags)
char *string;
int *sindex;
...
int *sindex, quoted, flags;
{
	...
	while (c = string[i]) {
		...
		if (c == '$' && string[i + 1] == '\'') {
			...
			if ((posixly_correct || extended_quote == 0) && dolbrace_state != DOLBRACE_QUOTE &&
	   ...
	   verbatim. */
		if (c == '`') {
			...
			t = string_extract(string, &si, "`",
			                   flags); /* already know (flags & SX_NOALLOC) == 0) */
			CHECK_STRING_OVERRUN(i, si, slen, c);
...
static char *extract_dollar_brace_string(string, sindex, quoted, flags)
char *string;
int *sindex, quoted, flags;
{
	register int i, c;
	...
	while (c = string[i]) {
		...
		if (c == RBRACE) {
			...
			dolbrace_state = (nesting_level < PARAMEXPNEST_MAX) ?
			                     dbstate[nesting_level] :
			                     dbstate[0]; /* Guess using initial state */
			i++;
			continue;
		}
	   ...
	   arithmetic substitutions through verbatim. */
		...
		if (c == '\'') {
			/*itrace("extract_dollar_brace_string: c == single quote flags = %d quoted = %d
			 * dolbrace_state = %d", flags, quoted, dolbrace_state);*/
...
static int skip_matched_pair(string, start, open, close, flags) const char *string;
...
int skipsubscript(string, start, flags) const char *string;
int start, flags;
...
int skip_to_delim(string, start, delims, flags)
char *string;
int start;
char *delims;
int flags;
		   ...
		   `regular' single-quoted strings. */
			...
			if (string[i + 1] == LPAREN)
				temp = extract_delimited_string(string, &si, "$(", "(", ")",
				                                SX_NOALLOC | SX_COMMAND | completeflag); /* ) */
			else
			...
			temp = extract_delimited_string(string, &si, (c == '<') ? "<(" : ">(", "(", ")",
...
int char_is_quoted(string, eindex)
char *string;
int eindex;
{
	...
	while (1) {
	   ...
	   is closer to the behavior of the shell parser. */
		if (ts == te && d2 && member(string[ts], d2)) {
			...
			if (ifs_split)
				while (
				    member(string[te], d) && spctabnl(string[te]) &&
...
char *string_list_internal(list, sep)
WORD_LIST *list;
char *sep;
{
...
int flags;
{
	...
	tlist = (quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES | Q_PATQUOTE)) ? quote_list(list) :
...
char *string_list_pos_params(pchar, list, quoted, pflags)
int pchar;
WORD_LIST *list;
int quoted, pflags;
{
...
/* This performs word splitting and quoted null character removal on
   STRING. */
#define issep(c) (((separators)[0]) ? ((separators)[1] ? isifs(c) : (c) == (separators)[0]) : 0)

/* member of the space character class in the current locale */
#define ifs_whitespace(c) ISSPACE(c)

/* "adjacent IFS white space" */
#define ifs_whitesep(c) ((sh_style_split || separators == 0) ? spctabnl(c) : ifs_whitespace(c))
...
int quoted;
{
	...
	sh_style_split = separators && separators[0] == ' ' && separators[1] == '\t' &&
	                 separators[2] == '\n' && separators[3] == '\0';
...
char *get_word_from_string(stringp, separators, endptr)
...
/* Remove IFS white space at the end of STRING.  Start at the end
   ...
   non-zero.  */
char *strip_trailing_ifs_whitespace(string, separators, saw_escape)
...
int saw_escape;
{
	...
	while (s > string &&
	       ((spctabnl(*s) && isifs(*s)) || (saw_escape && *s == CTLESC && spctabnl(s[1]))))
		s--;
	*++s = '\0';
	return string;
}
...
#if defined(ARRAY_VARS)
static SHELL_VAR *do_compound_assignment(name, value, flags)
char *name, *value;
int flags;
{
	SHELL_VAR *v;
...
static int do_assignment_internal(word, expand) const WORD_DESC *word;
int expand;
{
...
int do_word_assignment(word, flags)
WORD_DESC *word;
int flags;
...
int do_assignment_no_expand(string)
char *string;
{
	WORD_DESC td;
...
static char *expand_string_if_necessary(string, quoted, func)
char *string;
int quoted;
EXPFUNC *func;
{
...
static inline char *expand_string_to_string_internal(string, quoted, func)
char *string;
int quoted;
EXPFUNC *func;
{
...
char *expand_string_unsplit_to_string(string, quoted)
char *string;
int quoted;
...
static char *quote_string_for_repl(string, flags)
char *string;
int flags;
{
	size_t slen;
...
static char *expand_string_for_patsub(string, quoted)
char *string;
int quoted;
{
	WORD_LIST *value;
...
char *expand_arith_string(string, quoted)
char *string;
int quoted;
{
	WORD_DESC td;
...
char *expand_string_dollar_quote(string, flags)
char *string;
int flags;
{
...
static WORD_LIST *call_expand_word_internal(w, q, i, c, e)
WORD_DESC *w;
int q, i, *c, *e;
{
	WORD_LIST *result;
...
/* Perform parameter expansion, command substitution, and arithmetic
   ...
   in the result.  */
static WORD_LIST *expand_string_internal(string, quoted)
char *string;
int quoted;
{
	WORD_DESC td;
...
WORD_LIST *expand_string_unsplit(string, quoted)
char *string;
int quoted;
{
	WORD_LIST *value;
...
WORD_LIST *expand_string_assignment(string, quoted)
char *string;
int quoted;
{
	WORD_DESC td;
...
WORD_LIST *expand_prompt_string(string, quoted, wflags)
char *string;
int quoted;
int wflags;
{
...
/* This does not perform word splitting or dequote the WORD_LIST
   it returns. */
static WORD_LIST *expand_string_for_rhs(string, quoted, op, pflags, dollar_at_p, expanded_p)
char *string;
...
int *dollar_at_p, *expanded_p;
{
	...
	expand_no_split_dollar_star = (quoted & (Q_DOUBLE_QUOTES | Q_HERE_DOCUMENT)) || op == '=' ||
	                              ifs_is_null == 0; /* XXX - was 1 */
...
static WORD_LIST *expand_string_for_pat(string, quoted, dollar_at_p, expanded_p)
char *string;
...
WORD_LIST *expand_string(string, quoted)
char *string;
int quoted;
{
	WORD_LIST *result;
...
WORD_LIST *expand_word(word, quoted)
WORD_DESC *word;
int quoted;
{
...
/* Expand WORD, but do not perform word splitting on the result.  This
   does parameter expansion, command substitution, arithmetic expansion,
   and quote removal. */
WORD_LIST *expand_word_unsplit(word, quoted)
WORD_DESC *word;
int quoted;
{
	WORD_LIST *result;
...
static char *quote_escapes_internal(string, flags) const char *string;
int flags;
{
	const char *s, *send;
	char *t, *result;
	...
	while (*s) {
		if ((skip_ctlesc == 0 && *s == CTLESC) || (skip_ctlnul == 0 && *s == CTLNUL) ||
		    (quote_spaces && *s == ' '))
			*t++ = CTLESC;
		COPY_CHAR_P(t, s, send);
	}
...
char *quote_escapes(string) const char *string;
{ return (quote_escapes_internal(string, 0)); }

char *quote_rhs(string) const char *string;
...
char *dequote_escapes(string) const char *string;
{
	const char *s, *send;
	char *t, *result;
	size_t slen;
...
static WORD_LIST *list_dequote_escapes(list)
WORD_LIST *list;
{
	register WORD_LIST *w;
	char *t;
...
static char *make_quoted_char(c)
int c;
{
	char *temp;
...
static WORD_LIST *quote_list(list)
WORD_LIST *list;
{
	register WORD_LIST *w;
	char *t;
...
/* Remove CTLESC protecting a CTLESC or CTLNUL in place.  Return the passed
   string. */
char *remove_quoted_escapes(string)
char *string;
{
	char *t;
...
/* Perform quoted null character removal on each element of LIST.
   This modifies LIST. */
void word_list_remove_quoted_nulls(list) WORD_LIST *list;
{
	register WORD_LIST *t;
...
#if defined(HANDLE_MULTIBYTE)
#ifdef INCLUDE_UNUSED
static unsigned char *mb_getcharlens(string, len)
char *string;
int len;
{
	int i, offset, last;
...
static char *remove_upattern(param, pattern, op)
char *param, *pattern;
int op;
{
	register size_t len;
...
#if defined(HANDLE_MULTIBYTE)
/* Returns its first argument if nothing matched; new memory otherwise */
static wchar_t *remove_wpattern(wparam, wstrlen, wpattern, op)
wchar_t *wparam;
size_t wstrlen;
wchar_t *wpattern;
int op;
...
static int match_upattern(string, pat, mtype, sp, ep)
char *string, *pat;
int mtype;
char **sp, **ep;
{
...
/* Match WPAT anywhere in WSTRING and return the match boundaries.
   ...
   character version. */
static int match_wpattern(wstring, indices, wstrlen, wpat, mtype, sp, ep)
wchar_t *wstring;
char **indices;
size_t wstrlen;
wchar_t *wpat;
...
char **sp, **ep;
{
...
#if defined(EXTENDED_GLOB)
	if (extended_glob)
		simple &= (wpat[1] != L'(' || (wpat[0] != L'*' && wpat[0] != L'?' && wpat[0] != L'+' &&
		                               wpat[0] != L'!' && wpat[0] != L'@')); /*)*/
#endif
	...
	/* If the pattern doesn't match anywhere in the string, go ahead and
	   ...
	   `*' as first and last character, making a new pattern if necessary. */
	len = wcslen(wpat);
	if (wpat[0] != L'*' || (wpat[0] == L'*' && wpat[1] == WLPAREN && extended_glob) ||
	    wpat[len - 1] != L'*') {
		int unescaped_backslash;
		wchar_t *wpp;
	...
	case MATCH_ANY:
		for (n = 0; n <= wstrlen; n++) {
			n2 = simple ? (WFOLD(*wpat) == WFOLD(wstring[n])) :
...
static char *getpattern(value, quoted, expandpat)
char *value;
int quoted, expandpat;
{
  ...
  if (expandpat && (quoted & (Q_HERE_DOCUMENT|Q_DOUBLE_QUOTES)) && *tword)
	...
	l = *value ? expand_string_for_pat(
	                 value, (quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES)) ? Q_PATQUOTE : quoted,
	                 (int *)NULL, (int *)NULL) :
	             (WORD_LIST *)0;
	if (l)
		word_list_remove_quoted_nulls(l);
...
#if defined(ARRAY_VARS)
static char *array_remove_pattern(var, pattern, patspec, starsub, quoted)
SHELL_VAR *var;
char *pattern;
int patspec;
...
static char *parameter_brace_remove_pattern(varname, value, estatep, patstr, rtype, quoted, flags)
char *varname, *value;
...
int rtype, quoted, flags;
{
	...
	case VT_ARRAYMEMBER:
		...
		if (vtype == VT_VARIABLE)
			FREE(val);
		if (temp1) {
			val = (quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES)) ? quote_string(temp1) :
			                                                       quote_escapes(temp1);
			free(temp1);
			temp1 = val;
		}
...
static void add_fifo_list(pathname) char *pathname;
{
	int osize, i;
	...
	if (nfifo >= fifo_list_size - 1) {
		...
		fifo_list =
		    (struct temp_fifo *)xrealloc(fifo_list, fifo_list_size * sizeof(struct temp_fifo));
...
void close_new_fifos(list, lsize) void *list;
int lsize;
{
	int i;
	char *plist;
...
void set_procsub_status(ind, pid, status) int ind;
pid_t pid;
int status;
{
	if (ind >= 0 && ind < nfifo)
...
static void reap_some_procsubs(max) int max;
{
	int i;
...
static void add_fifo_list(fd) int fd;
{
...
static char *process_substitute(string, open_for_read_in_child)
char *string;
int open_for_read_in_child;
{
	...
	if (pipeline_pgrp == 0 ||
	    (subshell_environment & (SUBSHELL_PIPE | SUBSHELL_FORK | SUBSHELL_ASYNC)) == 0)
...
static char *optimize_cat_file(r, quoted, flags, flagp)
REDIRECT *r;
...
char *s;
{
	...
	/* If unbound variables should generate an error, report one and return
	   failure. */
	if ((var == 0 || invisible_p(var) || (assoc_p(var) == 0 && array_p(var) == 0)) &&
...
static int valid_brace_expansion_word(name, var_is_special)
char *name;
int var_is_special;
{
...
static int chk_atstar(name, quoted, pflags, quoted_dollar_atp, contains_dollar_at)
char *name;
int quoted, pflags;
...
static WORD_DESC *parameter_brace_expand_word(name, var_is_special, quoted, pflags, estatep)
char *name;
...
array_eltstate_t *estatep;
{
	...
	if (legal_number(name, &arg_index)) {
		...
		if (tt)
			temp = (*tt && (quoted & (Q_DOUBLE_QUOTES | Q_HERE_DOCUMENT))) ? quote_string(tt) :
			                                                                 quote_escapes(tt);
		else
			temp = (char *)NULL;
		FREE(tt);
	} else if (var_is_special) /* ${@} */
	{
		...
		ret = param_expand(tt, &sindex, quoted, (int *)NULL, (int *)NULL, (int *)NULL, (int *)NULL,
		                   pflags);

		/* Make sure we note that we saw a quoted null string and pass the flag back
	   to the caller in addition to the value. */
		if ((quoted & (Q_DOUBLE_QUOTES | Q_HERE_DOCUMENT)) && STR_DOLLAR_AT_STAR(name) && ret &&
		    ret->word && QUOTED_NULL(ret->word))
	...
	expand_arrayref:
		...
		if (es.subtype == 0 && temp) {
			temp = (*temp && (quoted & (Q_DOUBLE_QUOTES | Q_HERE_DOCUMENT))) ? quote_string(temp) :
			                                                                   quote_escapes(temp);
			rflags |= W_ARRAYIND;
		}
	   ...
	   returning the W_HASQUOTEDNULL flag to the caller in addition to TEMP. */
		else if (es.subtype == 1 && temp && QUOTED_NULL(temp) &&
		         (quoted & (Q_DOUBLE_QUOTES | Q_HERE_DOCUMENT)))
			rflags |= W_HASQUOTEDNULL;
		else if (es.subtype == 2 && temp && QUOTED_NULL(temp) &&
		         (quoted & (Q_DOUBLE_QUOTES | Q_HERE_DOCUMENT)))
			rflags |= W_HASQUOTEDNULL;
		...
		if (var_isset(var) && invisible_p(var) == 0) {
			...
			if (temp)
				temp = (*temp && (quoted & (Q_DOUBLE_QUOTES | Q_HERE_DOCUMENT))) ?
				           quote_string(temp) :
				           ((pflags & PF_ASSIGNRHS) ? quote_rhs(temp) : quote_escapes(temp));
			FREE(tt);
		} else
			temp = (char *)NULL;
...
static char *parameter_brace_find_indir(name, var_is_special, quoted, find_nameref)
char *name;
...
static WORD_DESC *parameter_brace_expand_indir(
    name, var_is_special, quoted, pflags, quoted_dollar_atp, contains_dollar_at)
char *name;
int var_is_special, quoted, pflags;
int *quoted_dollar_atp, *contains_dollar_at;
{
...
int op, quoted, pflags, *qdollaratp, *hasdollarat;
{
	...
	if (l) {
	   ...
	   is the only expansion that creates more than one word. */
		if (qdollaratp && ((l_hasdollat && quoted) || l->next)) {
			/*itrace("parameter_brace_expand_rhs: %s:%s: l != NULL, set *qdollaratp", name,
			 * value);*/
			*qdollaratp = 1;
		}
	   ...
	   quoted null string and set the flags to indicate it. */
		if (l->next == 0 && (quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES)) && QUOTED_NULL(temp) &&
		    QUOTED_NULL(l->word->word) && (l->word->flags & W_HASQUOTEDNULL)) {
			w->flags |= W_HASQUOTEDNULL;
			/*itrace("parameter_brace_expand_rhs (%s:%s): returning quoted null, turning off
	...
	/* bash-4.4/5.0 */
	vname = name;
	if (*name == '!' && (legal_variable_starter((unsigned char)name[1]) || DIGIT(name[1]) ||
...
/* Deal with the right hand side of a ${name:?value} expansion in the case
   ...
   printed. */
static void parameter_brace_expand_error(name, value, check_null) char *name, *value;
int check_null;
{
	WORD_LIST *l;
	char *temp;
...
{
	return (name[1] == '\0' || /* ${#} */
	        ((sh_syntaxtab[(unsigned char)name[1]] & CSPECVAR) &&
	         name[2] == '\0') ||                        /* special param */
...
char *name;
{
	...
	if (name[1] == '\0') /* ${#} */
	   ...
	   scalar variable value. */
		else if ((var || (var = find_variable(name + 1))) && invisible_p(var) == 0 &&
		         array_p(var) == 0 && assoc_p(var) == 0 && var->dynamic_value == 0)
...
static int verify_substring_values(v, value, substr, vtype, e1p, e2p)
SHELL_VAR *v;
char *value, *substr;
int vtype;
intmax_t *e1p, *e2p;
...
static int get_var_and_type(varname, value, estatep, quoted, flags, varp, valp)
char *varname, *value;
array_eltstate_t *estatep;
int quoted, flags;
SHELL_VAR **varp;
char **valp;
{
	...
	want_indir = *varname == '!' && (legal_variable_starter((unsigned char)varname[1]) ||
	...
	*varp = (SHELL_VAR *)NULL;
	...
	if (valid_array_reference(vname, 0)) {
		...
		*varp = v;
		*valp =
		    assoc_p(v) ? assoc_reference(assoc_cell(v), "0") : array_reference(array_cell(v), 0);
	} else
#endif
	{
...
static char *string_var_assignment(v, s)
SHELL_VAR *v;
char *s;
{
...
#if defined(ARRAY_VARS)
static char *array_var_assignment(v, itype, quoted, atype)
SHELL_VAR *v;
int itype, quoted, atype;
{
	...
	if (v == 0)
		return (char *)NULL;
	if (atype == 2)
		val = array_p(v) ? array_to_kvpair(array_cell(v), 0) : assoc_to_kvpair(assoc_cell(v), 0);
	else
		val = array_p(v) ? array_to_assign(array_cell(v), 0) : assoc_to_assign(assoc_cell(v), 0);
	...
	if (val == 0 && (invisible_p(v) || var_isset(v) == 0))
		...
		ret =
		    (quoted & (Q_DOUBLE_QUOTES | Q_HERE_DOCUMENT)) ? quote_string(val) : quote_escapes(val);
		free(val);
		val = ret;
	}
...
static char *pos_params_assignment(list, itype, quoted)
WORD_LIST *list;
int itype;
int quoted;
{
...
static char *string_transform(xc, v, s)
int xc;
SHELL_VAR *v;
char *s;
{
...
#if defined(ARRAY_VARS)
static char *array_transform(xc, var, starsub, quoted)
int xc;
SHELL_VAR *var;
int starsub; /* so we can figure out how it's indexed */
int quoted;
...
static int valid_parameter_transform(xform)
char *xform;
{
	if (xform[1])
		return 0;
...
static char *parameter_brace_transform(varname, value, estatep, xform, rtype, quoted, pflags, flags)
char *varname, *value;
array_eltstate_t *estatep;
char *xform;
...
static char *mb_substring(string, s, e)
char *string;
int s, e;
{
	char *tt;
...
static char *parameter_brace_substring(varname, value, estatep, substr, quoted, pflags, flags)
char *varname, *value;
array_eltstate_t *estatep;
char *substr;
...
static int shouldexp_replacement(s)
char *s;
{
	size_t slen;
	int sindex, c;
...
char *pat_subst(string, pat, rep, mflags)
char *string, *pat, *rep;
int mflags;
{
...
static char *pos_params_pat_subst(string, pat, rep, mflags)
char *string, *pat, *rep;
int mflags;
{
...
static char *parameter_brace_patsub(varname, value, estatep, patsub, quoted, pflags, flags)
char *varname, *value;
array_eltstate_t *estatep;
char *patsub;
int quoted, pflags, flags;
	...
	if (rep) {
	   ...
	   special.  THIS IS NOT BACKWARDS COMPATIBLE WITH BASH-4.2. */
		if (shell_compatibility_level > 42 && patsub_replacement == 0)
			rep = expand_string_if_necessary(rep, quoted & ~(Q_DOUBLE_QUOTES | Q_HERE_DOCUMENT),
...
static char *pos_params_modcase(string, pat, modop, mflags)
char *string, *pat;
int modop;
int mflags;
{
...
static int chk_arithsub(s, len) const char *s;
int len;
{
	int i, count;
	DECLARE_MBSTATE;
...
int *indexp, quoted, pflags, *quoted_dollar_atp, *contains_dollar_at;
{
	...
	/* ${#-} is a valid expansion and means to take the length of $-.
	   Similarly for ${#?} and ${##}... */
	if (name[0] == '#' && name[1] == '\0' && check_nullness == 0 && VALID_SPECIAL_LENGTH_PARAM(c) &&
	    string[sindex] == RBRACE) {
	...
	/* ...but ${#%}, ${#:}, ${#=}, ${#+}, and ${#/} are errors. */
	if (name[0] == '#' && name[1] == '\0' && check_nullness == 0 && member(c, "%:=+/") &&
	    string[sindex] == RBRACE) {
		temp = (char *)NULL;
	...
	want_indir = *name == '!' && (legal_variable_starter((unsigned char)name[1]) ||
	...
	if (want_indir && string[sindex - 1] == RBRACE && string[sindex - 2] == RBRACK &&
	...
	if (valid_brace_expansion_word(want_indir ? name + 1 : name, var_is_special) == 0) {
		temp = (char *)NULL;
		goto bad_substitution; /* substitution error */
	}
	...
	if (want_indir) {
		tdesc =
		    parameter_brace_expand_indir(name + 1, var_is_special, quoted, pflags | local_pflags,
	...
	*indexp = sindex;
	...
	/* All the cases where an expansion can possibly generate an unbound
	   variable error. */
	if (want_substring || want_patsub || want_casemod || c == '@' || c == '#' || c == '%' ||
	    c == RBRACE) {
	...
	if (want_substring) {
		temp1 = parameter_brace_substring(name, temp, &es, value, quoted, pflags,
	   ...
	   quoted_dollar_at == 0. */
		if (temp1 && (quoted_dollar_atp == 0 || *quoted_dollar_atp == 0) && QUOTED_NULL(temp1) &&
		...
		temp1 = parameter_brace_patsub(name, temp, &es, value, quoted, pflags,
		                               (tflag & W_ARRAYIND) ? AV_USEIND : 0);
		FREE(value);
		FREE(temp);
#if defined(ARRAY_VARS)
		...
		temp1 = parameter_brace_casemod(name, temp, &es, modspec, value, quoted, pflags,
		                                (tflag & W_ARRAYIND) ? AV_USEIND : 0);
		FREE(value);
		FREE(temp);
#if defined(ARRAY_VARS)
	...
	bad_substitution:
		...
		if (shell_compatibility_level <= 43)
			...
			return ((posixly_correct && interactive_shell == 0) ? &expand_wdesc_fatal :
			                                                      &expand_wdesc_error);
	...
	case '@':
		temp1 = parameter_brace_transform(name, temp, &es, value, c, quoted, pflags,
		                                  (tflag & W_ARRAYIND) ? AV_USEIND : 0);
		free(temp);
		free(value);
#if defined(ARRAY_VARS)
	...
	case '%': /* ${param%[%]pattern} */
		...
		temp1 = parameter_brace_remove_pattern(name, temp, &es, value, c, quoted,
		                                       (tflag & W_ARRAYIND) ? AV_USEIND : 0);
		free(temp);
		free(value);
#if defined(ARRAY_VARS)
		...
		} else /* VAR not set or VAR is NULL. */
		{
			...
			if (c == '=' && var_is_special) {
			   ...
			   removed. */
				if (quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES))
					quoted |= Q_DOLBRACE;
				ret = parameter_brace_expand_rhs(name, value, c, quoted, pflags, quoted_dollar_atp,
				                                 contains_dollar_at);
				/* XXX - fix up later, esp. noting presence of
...
static WORD_DESC *param_expand(string,
                               sindex,
                               quoted,
                               expanded_something,
                               contains_dollar_at,
...
int *quoted_dollar_at_p, *had_quoted_null_p, pflags;
{
	   ...
	   even if it's quoted. */
		if ((quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES)) && list == 0)
			...
			/* We check whether or not we're eventually going to split $* here,
			   ...
			   identical to $@ */
			if (expand_no_split_dollar_star && quoted == 0 && ifs_is_set == 0 &&
			    (pflags & PF_ASSIGNRHS)) {
			   ...
			   separate with space */
				...
				FREE(temp1);
			           ...
			           ifs_is_null == 0 && (pflags & PF_ASSIGNRHS)) {
	...
	case LBRACE:
		tdesc = parameter_brace_expand(string, &zindex, quoted, pflags, quoted_dollar_at_p,
		                               contains_dollar_at);
	...
	unbound_variable:
		...
		free(temp1);
		set_exit_status(EXECUTION_FAILURE);
		return ((unbound_vars_is_error && interactive_shell == 0) ? &expand_wdesc_fatal :
		                                                            &expand_wdesc_error);
	}
...
static char *expand_array_subscript(string, sindex, quoted, flags)
char *string;
int *sindex;
int quoted, flags;
{
...
static WORD_LIST *expand_word_internal(word, quoted, isexp, contains_dollar_at, expanded_something)
WORD_DESC *word;
int quoted, isexp;
	...
	for (sindex = 0;;) {
		...
		case '=':
			...
			if ((word->flags & W_ASSIGNMENT) &&
			    (posixly_correct == 0 || (word->flags & W_TILDEEXP)) && assignoff == -1 &&
			    sindex > 0)
				assignoff = sindex;
		...
		case ':':
			...
			if ((word->flags & (W_ASSIGNMENT | W_ASSIGNRHS)) &&
			    (posixly_correct == 0 || (word->flags & W_TILDEEXP)) && string[sindex + 1] == '~')
				internal_tilde = 1;
		...
		case '~':
			...
			if ((word->flags & W_NOTILDE) || (sindex > 0 && (internal_tilde == 0)) ||
			    (quoted & (Q_DOUBLE_QUOTES | Q_HERE_DOCUMENT))) {
				internal_tilde = 0;
				if (isexp == 0 && (word->flags & (W_NOSPLIT | W_NOSPLIT2)) == 0 && isifs(c) &&
				    (quoted & (Q_DOUBLE_QUOTES | Q_HERE_DOCUMENT)) == 0)
		...
		case '$':
			...
			tword = param_expand(string, &sindex, quoted, expanded_something, &temp_has_dollar_at,
			                     &quoted_dollar_at, &had_quoted_null, pflags);
		...
		case '`': /* Backquoted command substitution. */
		{
			...
			temp = string_extract(string, &sindex, "`",
			                      (word->flags & W_COMPLETE) ? SX_COMPLETE : SX_REQMATCH);
		...
		case '\\':
			...
			/* This is the fix for " $@\ " */
			         ...
			         ((sh_syntaxtab[c] & tflag) == 0) && isexp == 0 && isifs(c)) {
		...
		case '"':
			...
			temp = string_extract_double_quoted(string, &sindex,
			                                    (word->flags & W_COMPLETE) ? SX_COMPLETE : 0);

			/* If the quotes surrounded the entire string, then the
			   whole word was quoted. */
			quoted_state =
			    (t_index == 1 && string[sindex] == '\0') ? WHOLLY_QUOTED : PARTIALLY_QUOTED;
			...
			if (temp && *temp) {
				...
				list = expand_word_internal(tword, Q_DOUBLE_QUOTES | (quoted & Q_ARITH), 0,
			...
			if (list) {
				if (list->next) {
					...
					temp = quoted_dollar_at ? string_list_dollar_at(list, Q_DOUBLE_QUOTES, 0) :
		...
		add_ifs_character:
			if ((quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES)) ||
			    (isexp == 0 && isifs(c) && (word->flags & (W_NOSPLIT | W_NOSPLIT2)) == 0)) {
	...
	if (*istring == '\0') {
      ...
      if (quoted_dollar_at == 0 && (had_quoted_null || quoted_state == PARTIALLY_QUOTED))
	   ...
	   double-quoted $@ while expanding it. */
		else if (has_dollar_at && quoted_dollar_at == 0 && ifs_chars && quoted == 0 &&
		         (word->flags & W_NOSPLIT2)) {
			tword = alloc_word_desc();
...
WORD_LIST *word_split(w, ifs_chars)
WORD_DESC *w;
char *ifs_chars;
{
	WORD_LIST *result;
...
WORD_LIST *list;
{
	...
	for (t = list, result = (WORD_LIST *)NULL; t; t = t->next) {
	   ...
	   identical to the original word. */
		if (tresult && tresult->next == 0 && t->next == 0 && (t->word->flags & W_ARRAYREF) &&
		    STREQ(t->word->word, tresult->word->word))
...
static void exp_jump_to_top_level(v) int v;
{
...
WORD_LIST *expand_words(list)
WORD_LIST *list;
{ return (expand_word_list_internal(list, WEXP_ALL)); }

/* Same as expand_words (), but doesn't hack variable or environment
   variables. */
WORD_LIST *expand_words_no_vars(list)
WORD_LIST *list;
{ return (expand_word_list_internal(list, WEXP_NOVARS)); }

WORD_LIST *expand_words_shellexp(list)
WORD_LIST *list;
{ return (expand_word_list_internal(list, WEXP_SHELLEXP)); }

static WORD_LIST *glob_expand_word_list(tlist, eflags)
WORD_LIST *tlist;
int eflags;
{
...
static WORD_LIST *brace_expand_word_list(tlist, eflags)
WORD_LIST *tlist;
int eflags;
{
...
static int make_internal_declare(word, option, cmd)
char *word;
char *option;
char *cmd;
{
...
static WORD_LIST *expand_oneword(value, flags)
char *value;
int flags;
{
	WORD_LIST *l, *nl;
...
/* Expand and process an argument to a declaration command. We have already
   ...
   quote removal. */
static WORD_LIST *expand_declaration_argument(tlist, wcmd)
WORD_LIST *tlist, *wcmd;
{
	...
	tlist->word->flags &=
	    ~(W_ASSIGNMENT | W_NOSPLIT | W_COMPASSIGN | W_ASSIGNARG | W_ASSIGNASSOC | W_ASSIGNARRAY);

	return (tlist);
}
#endif /* ARRAY_VARS */

static WORD_LIST *shell_expand_word_list(tlist, eflags)
WORD_LIST *tlist;
int eflags;
{
...
int eflags;
{
	...
	if ((eflags & WEXP_VARASSIGN) && subst_assign_varlist) {
		do_assignment_statements(subst_assign_varlist,
		                         (new_list && new_list->word) ? new_list->word->word : (char *)NULL,
		                         new_list == 0);
