...
void setup_exec_ignore(varname) char *varname;
...
int file_status(name) const char *name;
{
	struct stat finfo;
	int r;
...
int executable_file(file) const char *file;
{
	int s;
...
int is_directory(file) const char *file;
{ return (file_status(file) & FS_DIRECTORY); }

int executable_or_directory(file) const char *file;
{
	int s;
...
char *find_user_command(name) const char *name;
...
char *find_path_file(name) const char *name;
{ return (find_user_command_internal(name, FS_READABLE)); }

static char *_find_user_command_internal(name, flags) const char *name;
int flags;
{
	char *path_list, *cmd;
	SHELL_VAR *var;
...
static char *find_user_command_internal(name, flags) const char *name;
int flags;
{
#ifdef __WIN32__
	char *res, *dotexe;
...
int flags;
{
	...
	if (hashed_file)
		...
		command =
		    find_user_command_in_path(pathname, path_list, FS_EXEC_PREFERRED | FS_NODIRS, &st);
...
char *user_command_matches(name, flags, state) const char *name;
int flags, state;
{
	register int i;
...
static char *find_absolute_program(name, flags) const char *name;
int flags;
{
	int st;
...
static char *find_in_path_element(name, path, flags, name_len, dotinfop, rflagsp) const char *name;
char *path;
int flags, name_len;
...
*/
static char *find_user_command_in_path(name, path_list, flags, rflagsp) const char *name;
char *path_list;
int flags, *rflagsp;
{
	char *full_path, *path;
...
char *find_in_path(name, path_list, flags) const char *name;
char *path_list;
int flags;
