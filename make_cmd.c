...
static COMMAND *
    make_for_or_select PARAMS((enum command_type, WORD_DESC *, WORD_LIST *, COMMAND *, int));
...
WORD_DESC *make_word_flags(w, string)
WORD_DESC *w;
const char *string;
{
	register int i;
...
WORD_DESC *make_word_from_token(token)
int token;
{
	char tokenizer[2];
...
WORD_LIST *make_word_list(word, wlink)
WORD_DESC *word;
WORD_LIST *wlink;
{
	WORD_LIST *temp;
...
COMMAND *make_command(type, pointer) enum command_type type;
SIMPLE_COM *pointer;
{
	COMMAND *temp;
...
static COMMAND *make_for_or_select(type, name, map_list, action, lineno) enum command_type type;
WORD_DESC *name;
WORD_LIST *map_list;
COMMAND *action;
int lineno;
...
COMMAND *make_for_command(name, map_list, action, lineno)
WORD_DESC *name;
WORD_LIST *map_list;
COMMAND *action;
int lineno;
...
COMMAND *make_select_command(name, map_list, action, lineno)
WORD_DESC *name;
WORD_LIST *map_list;
COMMAND *action;
int lineno;
...
#if defined(ARITH_FOR_COMMAND)
static WORD_LIST *make_arith_for_expr(s)
char *s;
{
	WORD_LIST *result;
	WORD_DESC *wd;
	...
	wd = make_word(s);
	wd->flags |= W_NOGLOB | W_NOSPLIT | W_QUOTED | W_NOTILDE |
	             W_NOPROCSUB; /* no word splitting or globbing */
...
/* Note that this function calls dispose_words on EXPRS, since it doesn't
   ...
   any arguments. */
COMMAND *make_arith_for_command(exprs, action, lineno)
WORD_LIST *exprs;
COMMAND *action;
int lineno;
{
...
COMMAND *make_group_command(command)
COMMAND *command;
{
	GROUP_COM *temp;
...
PATTERN_LIST *make_pattern_list(patterns, action)
WORD_LIST *patterns;
COMMAND *action;
{
	PATTERN_LIST *temp;
...
COMMAND *make_if_command(test, true_case, false_case)
...
static COMMAND *make_until_or_while(which, test, action) enum command_type which;
COMMAND *test, *action;
{
	WHILE_COM *temp;
...
COMMAND *make_while_command(test, action)
COMMAND *test, *action;
{ return (make_until_or_while(cm_while, test, action)); }

COMMAND *make_until_command(test, action)
COMMAND *test, *action;
...
COMMAND *make_cond_command(cond_node)
COND_COM *cond_node;
{
#if defined(COND_COMMAND)
	COMMAND *command;
...
void make_here_document(temp, lineno) REDIRECT *temp;
int lineno;
{
	...
	while (full_line = read_secondary_line(delim_unquoted)) {
	   ...
	   read_a_line leaves the newline at the end, so don't print another. */
		...
		if (STREQN(line, redir_word, redir_len) && (parser_state & PST_EOFTOKEN) &&
		    shell_eof_token && strchr(line + redir_len, shell_eof_token)) {
...
REDIRECT *make_redirection(source, instruction, dest_and_filename, flags)
REDIRECTEE source;
...
int flags;
{
	...
	case r_duplicating_output_word: /* 1>&$foo */
		...
		if (w->word[wlen] == '-') /* Yuck */
		{
			...
			if (all_digits(w->word) && legal_number(w->word, &lfd) && lfd == (int)lfd) {
				dispose_word(w);
				temp->instruction =
				    (instruction == r_duplicating_input_word) ? r_move_input : r_move_output;
				temp->redirectee.dest = lfd;
			} else
				temp->instruction = (instruction == r_duplicating_input_word) ? r_move_input_word :
				                                                                r_move_output_word;
		}
	...
	default:
		programming_error(_("make_redirection: redirection instruction `%d' out of range"),
		                  instruction);
		abort();
		break;
	}
...
COMMAND *clean_simple_command(command)
COMMAND *command;
{
...
/* The Yacc grammar productions have a problem, in that they take a
   ...
   `;' bind `tighter' than `&'. */
COMMAND *connect_async_list(command, command2, connector)
