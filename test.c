...
static void test_syntax_error(format, arg) char *format, *arg;
{
...
static int stat_mtime(fn, st, ts)
char *fn;
struct stat *st;
struct timespec *ts;
{
...
static int arithcomp(s, t, op, flags)
char *s, *t;
int op, flags;
{
	intmax_t l, r;
...
static int patcomp(string, pat, op)
char *string, *pat;
int op;
{
	int m;
...
int binary_test(op, arg1, arg2, flags)
char *op, *arg1, *arg2;
int flags;
{
	int patmatch;
...
int flags;
{
	...
	case 'O': /* File is owned by you? */
		return (sh_stat(arg, &stat_buf) == 0 && (uid_t)current_user.euid == (uid_t)stat_buf.st_uid);

	case 'G': /* File is owned by your group? */
		return (sh_stat(arg, &stat_buf) == 0 && (gid_t)current_user.egid == (gid_t)stat_buf.st_gid);
	...
	case 'h': /* File is a symbolic link? */
		...
		return (FALSE);
#else
		return ((arg[0] != '\0') && (lstat(arg, &stat_buf) == 0) && S_ISLNK(stat_buf.st_mode));
...
static int posixtest() {
	...
	case 4:
		if (argv[pos][0] == '!' && argv[pos][1] == '\0') {
			...
			value = !three_arguments();
			break;
		} else if (argv[pos][0] == '(' && argv[pos][1] == '\0' && argv[argc - 1][0] == ')' &&
		           argv[argc - 1][1] == '\0') {
			advance(1);
			value = two_arguments();
			pos = argc;
