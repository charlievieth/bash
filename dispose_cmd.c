...
void dispose_command(command) COMMAND *command;
{
	if (command == 0)
		return;
...
void dispose_function_def_contents(c) FUNCTION_DEF *c;
{
	dispose_word(c->name);
...
void dispose_function_def(c) FUNCTION_DEF *c;
{
...
void dispose_word(w) WORD_DESC *w;
{
	FREE(w->word);
...
void dispose_word_desc(w) WORD_DESC *w;
{
	w->word = 0;
	ocache_free(wdcache, WORD_DESC, w);
}
...
void dispose_words(list) WORD_LIST *list;
{
	WORD_LIST *t;
...
void dispose_word_array(array) char **array;
{
	register int count;
...
void dispose_redirects(list) REDIRECT *list;
{
	register REDIRECT *t;
