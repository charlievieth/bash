...
/* Copyright (C) 1987,1989,1991,1995,1998,2001,2003,2005,2006,2008,2009 Free Software Foundation,
   Inc.
...
#define HASH_SHOULDGROW(table) ((table)->nentries >= (table)->nbuckets * HASH_REHASH_FACTOR)
...
HASH_TABLE *hash_create(buckets)
int buckets;
{
...
static void hash_rehash(table, nsize) HASH_TABLE *table;
int nsize;
{
	int osize, i, j;
...
static void hash_grow(table) HASH_TABLE *table;
{
	int nsize;
...
static void hash_shrink(table) HASH_TABLE *table;
{
	int nsize;
...
HASH_TABLE *hash_copy(table, cpdata)
HASH_TABLE *table;
...
unsigned int hash_string(s) const char *s;
{
...
int hash_bucket(string, table) const char *string;
HASH_TABLE *table;
{
	unsigned int h;
...
BUCKET_CONTENTS *hash_remove(string, table, flags) const char *string;
HASH_TABLE *table;
int flags;
{
	int bucket;
...
BUCKET_CONTENTS *hash_insert(string, table, flags)
char *string;
HASH_TABLE *table;
int flags;
{
...
void hash_walk(table, func) HASH_TABLE *table;
hash_wfunc *func;
{
	register int i;
	BUCKET_CONTENTS *item;
...
void hash_pstats(table, name) HASH_TABLE *table;
char *name;
{
