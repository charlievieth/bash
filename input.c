...
int ungetc_with_restart(c, stream)
int c;
FILE *stream;
{
...
static void allocate_buffers(n) int n;
{
...
/* Construct and return a BUFFERED_STREAM corresponding to file descriptor
   FD, using BUFFER. */
static BUFFERED_STREAM *make_buffered_stream(fd, buffer, bufsize)
int fd;
char *buffer;
size_t bufsize;
{
...
static BUFFERED_STREAM *copy_buffered_stream(bp)
BUFFERED_STREAM *bp;
{
	BUFFERED_STREAM *nbp;
...
int duplicate_buffered_stream(fd1, fd2)
int fd1, fd2;
{
	int is_bash_input, m;
...
BUFFERED_STREAM *fd_to_buffered_stream(fd)
int fd;
{
	char *buffer;
	size_t size;
...
BUFFERED_STREAM *open_buffered_stream(file)
char *file;
{
	int fd;
...
void free_buffered_stream(bp) BUFFERED_STREAM *bp;
{
	int n;
...
BUFFERED_STREAM *set_buffered_stream(fd, bp)
int fd;
BUFFERED_STREAM *bp;
{
	BUFFERED_STREAM *ret;
...
#define bufstream_getc(bp)                                            \
	(bp->b_inputp == bp->b_used || !bp->b_used) ? b_fill_buffer(bp) : \
	                                              bp->b_buffer[bp->b_inputp++] & 0xFF

/* Push C back onto buffered stream BP. */
static int bufstream_ungetc(c, bp)
int c;
BUFFERED_STREAM *bp;
{
...
void with_input_from_buffered_stream(bfd, name) int bfd;
char *name;
{
	...
	init_yy_io(bp == 0 ? return_EOF : buffered_getchar, buffered_ungetchar, st_bstream, name,
	           location);
}
