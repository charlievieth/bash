...
void assoc_dispose(hash) HASH_TABLE *hash;
{
	if (hash) {
		hash_flush(hash, 0);
		hash_dispose(hash);
...
void assoc_flush(hash) HASH_TABLE *hash;
{ hash_flush(hash, 0); }

int assoc_insert(hash, key, value)
HASH_TABLE *hash;
char *key;
char *value;
{
...
void assoc_remove(hash, string) HASH_TABLE *hash;
char *string;
{
	BUCKET_CONTENTS *b;
...
char *assoc_reference(hash, string)
HASH_TABLE *hash;
char *string;
{
	BUCKET_CONTENTS *b;
...
HASH_TABLE *assoc_quote_escapes(h)
HASH_TABLE *h;
{
	int i;
	BUCKET_CONTENTS *tlist;
...
HASH_TABLE *assoc_dequote_escapes(h)
HASH_TABLE *h;
{
	int i;
	BUCKET_CONTENTS *tlist;
...
HASH_TABLE *assoc_remove_quoted_nulls(h)
HASH_TABLE *h;
{
	int i;
	BUCKET_CONTENTS *tlist;
...
char *assoc_patsub(h, pat, rep, mflags)
HASH_TABLE *h;
char *pat, *rep;
int mflags;
{
...
char *assoc_modcase(h, pat, modop, mflags)
HASH_TABLE *h;
char *pat;
int modop;
int mflags;
...
char *assoc_to_kvpair(hash, quoted)
HASH_TABLE *hash;
int quoted;
{
	char *ret;
...
char *assoc_to_assign(hash, quoted)
HASH_TABLE *hash;
int quoted;
{
	char *ret;
...
WORD_LIST *assoc_to_word_list(h)
HASH_TABLE *h;
{ return (assoc_to_word_list_internal(h, 0)); }

WORD_LIST *assoc_keys_to_word_list(h)
HASH_TABLE *h;
{ return (assoc_to_word_list_internal(h, 1)); }

WORD_LIST *assoc_to_kvpair_list(h)
HASH_TABLE *h;
{
	WORD_LIST *list;
	int i;
...
char *assoc_to_string(h, sep, quoted)
HASH_TABLE *h;
char *sep;
int quoted;
{
