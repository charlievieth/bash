...
char *special_builtins[] = {":",     ".",     "source", "break",    "continue",  "eval",
                            "exec",  "exit",  "export", "readonly", "return",    "set",
                            "shift", "times", "trap",   "unset",    (char *)NULL};

/* The builtin commands that take assignment statements as arguments. */
char *assignment_builtins[] = {"alias",    "declare", "export",    "local",
                               "readonly", "typeset", (char *)NULL};

char *localvar_builtins[] = {"declare", "local", "typeset", (char *)NULL};

/* The builtin commands that are special to the POSIX search order. */
char *posix_builtins[] = {"alias", "bg",      "cd",    "command", "false",  "fc",
                          "fg",    "getopts", "jobs",  "kill",    "newgrp", "pwd",
                          "read",  "true",    "umask", "unalias", "wait",   (char *)NULL};
...
char *arrayvar_builtins[] = {"declare",   "let", "local",   "printf", "read",
                             "test",      "[",   "typeset", "unset",  "wait", /*]*/
                             (char *)NULL};
...
ARRAY *array;
{
	if (array->sindex + 2 > array->size)
		array->array =
		    (char **)xrealloc(array->array, (array->size += array->growth_rate) * array->width);
...
void array_free(array) ARRAY *array;
{
	if (array->array)
		free(array->array);
...
HANDLER_ENTRY handlers[] = {{"BUILTIN", builtin_handler},
...
void extract_info(filename, structfile, externfile) char *filename;
FILE *structfile, *externfile;
{
	register int i;
	DEF_FILE *defs;
				...
				if (output_cpp_line_info) {
					...
					if (defs->filename[0] == '/')
						...
						fprintf(defs->output, "#line %d \"%s%s\"\n", defs->line_number + 1,
						        error_directory ? error_directory : "./", defs->filename);
...
#define free_safely(x) \
	if (x)             \
	free(x)

static void free_builtin(builtin) BUILTIN_DESC *builtin;
{
	register int i;
...
void free_defs(defs) DEF_FILE *defs;
{
	register int i;
...
char *get_arg(for_whom, defs, string)
...
void must_be_building(directive, defs) char *directive;
DEF_FILE *defs;
{
	if (!building_builtin)
...
char *arg;
{
	...
	if (builtin->function)
		line_error(defs, "%s already has a function (%s)", builtin->name, builtin->function);
	else
...
int docname_handler(self, defs, arg)
char *self;
DEF_FILE *defs;
char *arg;
{
	...
	if (builtin->docname)
		line_error(defs, "%s already had a docname (%s)", builtin->name, builtin->docname);
	else
...
int short_doc_handler(self, defs, arg)
char *self;
DEF_FILE *defs;
char *arg;
{
	...
	if (builtin->shortdoc)
		line_error(defs, "%s already has short documentation (%s)", builtin->name,
		           builtin->shortdoc);
	else
...
int comment_handler(self, defs, arg)
char *self;
DEF_FILE *defs;
char *arg;
{ return (0); }
...
int depends_on_handler(self, defs, arg)
char *self;
DEF_FILE *defs;
char *arg;
{
...
int produces_handler(self, defs, arg)
char *self;
DEF_FILE *defs;
char *arg;
{
...
static char *xrealloc(pointer, bytes)
char *pointer;
int bytes;
{
	char *temp;
...
BUILTIN_DESC *builtin;
{
	...
	new->function = builtin->function ? savestring(builtin->function) : (char *)NULL;
	new->docname = builtin->docname ? savestring(builtin->docname) : (char *)NULL;

	return (new);
}

/* How to save away a builtin. */
void save_builtin(builtin) BUILTIN_DESC *builtin;
{
...
void write_file_headers(structfile, externfile) FILE *structfile, *externfile;
{
	register int i;
	...
	if (externfile)
		fprintf(externfile, "/* %s - The list of builtins found in libbuiltins.a. */\n",
		        include_filename ? include_filename : "builtext.h");
}

/* Write out any necessary closing information for
   STRUCTFILE and EXTERNFILE. */
void write_file_footers(structfile, externfile) FILE *structfile, *externfile;
{
	register int i;
...
/* Write out the information accumulated in DEFS to
   STRUCTFILE and EXTERNFILE. */
void write_builtins(defs, structfile, externfile) DEF_FILE *defs;
FILE *structfile, *externfile;
{
	register int i;
	...
	if (defs->builtins) {
		...
		for (i = 0; i < defs->builtins->sindex; i++) {
			...
			if (documentation_file) {
				...
				write_documentation(documentation_file, builtin->longdoc->array, 0, TEXINFO);
			}
		}
	}
}
...
/* Write an #ifdef string saying what needs to be defined (or not defined)
   ...
   reversed. */
void write_ifdefs(stream, defines) FILE *stream;
char **defines;
{
	register int i;
...
/* Write an #endif string saying what defines controlled the compilation
   ...
   STREAM is the stream to write the information to.
   DEFINES is a null terminated array of define names. */
void write_endifs(stream, defines) FILE *stream;
char **defines;
{
	register int i;
...
/* Write DOCUMENTATION to STREAM, perhaps surrounding it with double-quotes
   ...
   issues. */
void write_documentation(stream, documentation, indentation, flags) FILE *stream;
char **documentation;
...
static int _find_in_table(name, name_table)
...
static int is_special_builtin(name)
char *name;
{ return (_find_in_table(name, special_builtins)); }

static int is_assignment_builtin(name)
char *name;
...
static int is_arrayvar_builtin(name)
char *name;
