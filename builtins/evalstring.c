...
#if defined(HISTORY)
static void set_history_remembering() {
...
int subshell;
{
	return (running_trap == 0 && command->type == cm_simple && signal_is_trapped(EXIT_TRAP) == 0 &&
...
void optimize_connection_fork(command) COMMAND *command;
{
...
void optimize_shell_function(command) COMMAND *command;
{
	COMMAND *fc;
...
int can_optimize_cat_file(command)
COMMAND *command;
{
	return (command->type == cm_simple && !command->redirects &&
	        (command->flags & CMD_TIME_PIPELINE) == 0 && command->value.Simple->words == 0 &&
	        command->value.Simple->redirects && command->value.Simple->redirects->next == 0 &&
	        command->value.Simple->redirects->instruction == r_input_direction &&
	        command->value.Simple->redirects->redirector.dest == 0);
}

/* How to force parse_and_execute () to clean up after itself. */
void parse_and_execute_cleanup(old_running_trap) int old_running_trap;
{
	if (running_trap > 0) {
...
static void parse_prologue(string, flags, tag) char *string;
int flags;
char *tag;
{
		...
		if (parse_command() == 0) {
			...
			if ((flags & SEVAL_PARSEONLY) || (interactive_shell == 0 && read_but_dont_execute)) {
			   ...
			   command->value.Connection->second. */
				else if (command->type == cm_connection && (flags & SEVAL_NOOPTIMIZE) == 0 &&
			   ...
			   the global value of the flag (expaliases_flag) changes). */
				...
				if (startup_state == 2 && (subshell_environment & SUBSHELL_COMSUB) &&
				    *bash_input.location.string == '\0' && can_optimize_cat_file(command)) {
					int r;
...
int parse_string(string, from_file, flags, cmdp, endp)
char *string;
const char *from_file;
int flags;
COMMAND **cmdp;
...
int evalstring(string, from_file, flags)
char *string;
const char *from_file;
int flags;
{
