...
#else
    builtin_warning(format, va_alist) const char *format;
va_dcl
#endif
{
	va_list args;
...
void sh_noassign(s) const char *s;
...
void remember_args(list, destructive) WORD_LIST *list;
int destructive;
{
	register int i;
...
void shift_args(times) int times;
{
	WORD_LIST *temp;
	int count;
...
int get_numeric_arg(list, fatal, count)
WORD_LIST *list;
int fatal;
intmax_t *count;
{
...
#if defined(JOB_CONTROL)
int get_job_by_name(name, flags) const char *name;
int flags;
{
 ...
 */
int display_signal_list(list, forcecols)
WORD_LIST *list;
int forcecols;
{
...
sh_builtin_func_t *find_special_builtin(name)
char *name;
{
	current_builtin = builtin_address_internal(name, 0);
...
static int shell_builtin_compare(sbp1, sbp2)
...
SHELL_VAR *builtin_bind_variable(name, value, flags)
char *name;
char *value;
int flags;
{
...
SHELL_VAR *builtin_bind_var_to_int(name, val, flags)
char *name;
intmax_t val;
int flags;
{
...
#if defined(ARRAY_VARS)
SHELL_VAR *builtin_find_indexed_array(array_name, flags)
char *array_name;
int flags;
{
	SHELL_VAR *entry;
...
/* Like check_unbind_variable, but for use by builtins (only matters for
   error messages). */
int builtin_unbind_variable(vname) const char *vname;
{
	SHELL_VAR *v;
