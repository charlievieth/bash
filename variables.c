...
#if defined(ARRAY_VARS)
static SHELL_VAR *
    init_dynamic_array_var PARAMS((char *, sh_var_value_func_t *, sh_var_assign_func_t *, int));
static SHELL_VAR *
    init_dynamic_assoc_var PARAMS((char *, sh_var_value_func_t *, sh_var_assign_func_t *, int));
#endif
...
static SHELL_VAR *
    find_variable_nameref_context PARAMS((SHELL_VAR *, VAR_CONTEXT *, VAR_CONTEXT **));
static SHELL_VAR *
    find_variable_last_nameref_context PARAMS((SHELL_VAR *, VAR_CONTEXT *, VAR_CONTEXT **));
...
/* Initialize the shell variables from the current environment.
   ...
   parse $SHELLOPTS. */
void initialize_shell_variables(env, privmode) char **env;
int privmode;
{
...
void adjust_shell_level(change) int change;
{
...
void print_var_list(list) register SHELL_VAR **list;
{
	register int i;
...
/* Print LIST (a list of shell functions) to stdout in such a way that
   they can be read back in. */
void print_func_list(list) register SHELL_VAR **list;
{
	register int i;
	register SHELL_VAR *var;
...
void print_assignment(var) SHELL_VAR *var;
{
	if (var_isset(var) == 0)
		return;
...
void print_var_value(var, quote) SHELL_VAR *var;
int quote;
{
	char *t;
...
void print_var_function(var) SHELL_VAR *var;
{
	char *x;
...
static SHELL_VAR *null_assign(self, value, unused, key)
SHELL_VAR *self;
char *value;
arrayind_t unused;
char *key;
...
/* Degenerate `dynamic_value' function; just returns what's passed without
   manipulation. */
static SHELL_VAR *get_self(self)
SHELL_VAR *self;
{ return (self); }
...
static SHELL_VAR *init_dynamic_array_var(name, getfunc, setfunc, attrs)
char *name;
...
static SHELL_VAR *init_dynamic_assoc_var(name, getfunc, setfunc, attrs)
char *name;
sh_var_value_func_t *getfunc;
sh_var_assign_func_t *setfunc;
int attrs;
...
static inline SHELL_VAR *set_int_value(SHELL_VAR *var, intmax_t value, int flags) {
	char *p;
...
static inline SHELL_VAR *set_string_value(SHELL_VAR *var, const char *value, int flags) {
	char *p;
...
static SHELL_VAR *assign_seconds(self, value, unused, key)
SHELL_VAR *self;
char *value;
arrayind_t unused;
char *key;
...
static SHELL_VAR *assign_random(self, value, unused, key)
SHELL_VAR *self;
char *value;
arrayind_t unused;
char *key;
...
static SHELL_VAR *get_random(var)
SHELL_VAR *var;
{
	int rv;
...
static SHELL_VAR *get_urandom(var)
SHELL_VAR *var;
{
	u_bits32_t rv;
...
static SHELL_VAR *assign_lineno(var, value, unused, key)
SHELL_VAR *var;
char *value;
arrayind_t unused;
char *key;
...
static SHELL_VAR *get_lineno(var)
SHELL_VAR *var;
{
	int ln;
...
static SHELL_VAR *assign_subshell(var, value, unused, key)
SHELL_VAR *var;
char *value;
arrayind_t unused;
char *key;
...
static SHELL_VAR *get_subshell(var)
SHELL_VAR *var;
{ return (set_int_value(var, subshell_level, 0)); }

static SHELL_VAR *get_epochseconds(var)
SHELL_VAR *var;
{
	intmax_t now;
...
static SHELL_VAR *get_epochrealtime(var)
SHELL_VAR *var;
{
	char buf[32];
	struct timeval tv;
	...
	gettimeofday(&tv, NULL);
	snprintf(buf, sizeof(buf), "%u%c%06u", (unsigned)tv.tv_sec, locale_decpoint(),
	         (unsigned)tv.tv_usec);
...
static SHELL_VAR *get_bash_argv0(var)
SHELL_VAR *var;
{ return (set_string_value(var, dollar_vars[0], 0)); }

static char *static_shell_name = 0;

static SHELL_VAR *assign_bash_argv0(var, value, unused, key)
SHELL_VAR *var;
char *value;
arrayind_t unused;
char *key;
...
static SHELL_VAR *get_bash_command(var)
SHELL_VAR *var;
{
	char *p;
...
#if defined(HISTORY)
static SHELL_VAR *get_histcmd(var)
SHELL_VAR *var;
{
	int n;
...
static SHELL_VAR *get_comp_wordbreaks(var)
SHELL_VAR *var;
{
...
/* When this function returns, rl_completer_word_break_characters points to
   malloced memory. */
static SHELL_VAR *assign_comp_wordbreaks(self, value, unused, key)
SHELL_VAR *self;
char *value;
arrayind_t unused;
char *key;
...
static SHELL_VAR *get_groupset(self)
SHELL_VAR *self;
{
	register int i;
	int ng;
...
#if defined(DEBUGGER)
static SHELL_VAR *get_bashargcv(self)
SHELL_VAR *self;
{
...
static SHELL_VAR *get_hashcmd(self)
SHELL_VAR *self;
{
	build_hashcmd(self);
	return (self);
...
static SHELL_VAR *assign_hashcmd(self, value, ind, key)
SHELL_VAR *self;
char *value;
arrayind_t ind;
char *key;
...
#if defined(ALIAS)
static SHELL_VAR *build_aliasvar(self)
SHELL_VAR *self;
{
	HASH_TABLE *h;
	int i;
...
static SHELL_VAR *assign_aliasvar(self, value, ind, key)
SHELL_VAR *self;
char *value;
arrayind_t ind;
char *key;
...
void make_funcname_visible(on_or_off) int on_or_off;
{
	SHELL_VAR *v;
...
SHELL_VAR *var_lookup(name, vcontext) const char *name;
...
SHELL_VAR *find_variable_nameref(v)
SHELL_VAR *v;
{
	int level, flags;
	char *newname;
...
/* Resolve the chain of nameref variables for NAME.  XXX - could change later */
SHELL_VAR *find_global_variable_last_nameref(name, vflags) const char *name;
int vflags;
{
	SHELL_VAR *v, *nv;
	char *newname;
...
/* Do nameref resolution from the VC, which is the local context for some
   ...
   nameref resolution (so the bind_variable_internal can use the correct
   variable context and hash table). */
static SHELL_VAR *find_variable_last_nameref_context(v, vc, nvcp)
SHELL_VAR *v;
VAR_CONTEXT *vc;
VAR_CONTEXT **nvcp;
{
...
SHELL_VAR *find_variable_nameref_for_create(name, flags) const char *name;
int flags;
{
	SHELL_VAR *var;
...
char *nameref_transform_name(name, flags)
char *name;
int flags;
{
	SHELL_VAR *v;
...
SHELL_VAR *find_variable_tempenv(name) const char *name;
{
	SHELL_VAR *var;
...
SHELL_VAR *find_global_variable(name) const char *name;
{
	SHELL_VAR *var;
...
SHELL_VAR *find_global_variable_noref(name) const char *name;
{
	SHELL_VAR *var;
...
SHELL_VAR *find_variable(name) const char *name;
{
	SHELL_VAR *v;
	int flags;
...
/* Find the first instance of NAME in the variable context chain; return first
   ...
   found. */
SHELL_VAR *find_variable_no_invisible(name) const char *name;
{
	SHELL_VAR *v;
	int flags;
...
SHELL_VAR *find_variable_for_assignment(name) const char *name;
{
	SHELL_VAR *v;
	int flags;
...
SHELL_VAR *find_variable_noref(name) const char *name;
{
	SHELL_VAR *v;
	int flags;
...
SHELL_VAR *find_function(name) const char *name;
{ return (hash_lookup(name, shell_functions)); }

/* Find the function definition for the shell function named NAME.  Returns
   the entry or NULL. */
FUNCTION_DEF *find_function_def(name) const char *name;
{
#if defined(DEBUGGER)
...
char *get_string_value(var_name) const char *var_name;
{
	SHELL_VAR *var;
...
char *sh_get_env_value(v) const char *v;
...
SHELL_VAR *make_local_variable(name, flags) const char *name;
int flags;
{
	...
	if (old_var && (noassign_p(old_var) || (readonly_p(old_var) && old_var->context == 0))) {
...
static SHELL_VAR *new_shell_variable(name) const char *name;
{
	SHELL_VAR *entry;
...
/* Create a new shell variable with name NAME and add it to the hash table
   TABLE. */
static SHELL_VAR *make_new_variable(name, table) const char *name;
HASH_TABLE *table;
{
	SHELL_VAR *entry;
	BUCKET_CONTENTS *elt;
...
#if defined(ARRAY_VARS)
SHELL_VAR *make_new_array_variable(name)
char *name;
{
	SHELL_VAR *entry;
	ARRAY *array;
...
SHELL_VAR *make_local_array_variable(name, flags)
char *name;
int flags;
{
	SHELL_VAR *var;
...
SHELL_VAR *make_new_assoc_variable(name)
char *name;
{
	SHELL_VAR *entry;
	HASH_TABLE *hash;
...
SHELL_VAR *make_local_assoc_variable(name, flags)
char *name;
int flags;
{
	SHELL_VAR *var;
...
char *make_variable_value(var, value, flags)
SHELL_VAR *var;
char *value;
int flags;
{
	...
	if ((flags & ASS_NOEVAL) == 0 && integer_p(var)) {
	   ...
	   `rval'.  We can change it if we move the evaluation of lval to here. */
		...
		op = capcase_p(var) ? CASE_CAPITALIZE : (uppercase_p(var) ? CASE_UPPER : CASE_LOWER);
...
static int can_optimize_assignment(entry, value, aflags)
SHELL_VAR *entry;
char *value;
int aflags;
{
...
static SHELL_VAR *optimized_assignment(entry, value, aflags)
SHELL_VAR *entry;
char *value;
int aflags;
{
...
static SHELL_VAR *bind_variable_internal(name, value, table, hflags, aflags) const char *name;
char *value;
HASH_TABLE *table;
int hflags, aflags;
{
	...
	if (entry && invisible_p(entry) && nameref_p(entry)) {
		...
		if (valid_array_reference(newval, 0)) {
			...
			entry =
			    assign_array_element(newval, value, aflags | ASS_NAMEREF, (array_eltstate_t *)0);
			if (entry == 0)
				return entry;
		} else
#endif
...
SHELL_VAR *bind_variable(name, value, flags) const char *name;
char *value;
int flags;
{
	SHELL_VAR *v, *nv;
...
#endif
							return (bind_variable_internal(nameref_cell(nv), value, nvc->table, 0,
							                               flags));
...
static SHELL_VAR *bind_invalid_envvar(name, value, flags) const char *name;
char *value;
int flags;
{
	if (invalid_env == 0)
...
SHELL_VAR *bind_variable_value(var, value, aflags)
SHELL_VAR *var;
char *value;
int aflags;
{
...
SHELL_VAR *bind_int_variable(lhs, rhs, flags)
char *lhs, *rhs;
int flags;
{
	register SHELL_VAR *v;
...
SHELL_VAR *bind_function(name, value) const char *name;
COMMAND *value;
{
	SHELL_VAR *entry;
...
void bind_function_def(name, value, flags) const char *name;
FUNCTION_DEF *value;
int flags;
{
	FUNCTION_DEF *entry;
...
static void dispose_variable_value(var) SHELL_VAR *var;
{
	if (function_p(var))
...
int unbind_global_variable(name) const char *name;
{
	SHELL_VAR *v, *nv;
	int r;
...
int check_unbind_variable(name) const char *name;
{
	SHELL_VAR *v;
...
int unbind_func(name) const char *name;
{
	BUCKET_CONTENTS *elt;
	SHELL_VAR *func;
...
int delete_var(name, vc) const char *name;
VAR_CONTEXT *vc;
{
	BUCKET_CONTENTS *elt;
	SHELL_VAR *old_var;
...
int makunbound(name, vc) const char *name;
VAR_CONTEXT *vc;
{
...
void delete_all_variables(hashed_vars) HASH_TABLE *hashed_vars;
...
void set_var_read_only(name) char *name;
{
	SHELL_VAR *entry;
...
void set_func_read_only(name) const char *name;
{
	SHELL_VAR *entry;
...
/* Make the variable associated with NAME be auto-exported.
   If NAME does not exist yet, create it. */
void set_var_auto_export(name) char *name;
{
	SHELL_VAR *entry;
...
void set_func_auto_export(name) const char *name;
{
	SHELL_VAR *entry;
...
int n;
{
	...
	if (n > vlist->list_size) {
		vlist->list_size = n;
		vlist->list =
		    (SHELL_VAR **)xrealloc(vlist->list, (vlist->list_size + 1) * sizeof(SHELL_VAR *));
	}
	return vlist;
}

static void vlist_add(vlist, var, flags) VARLIST *vlist;
SHELL_VAR *var;
int flags;
{
	register int i;
...
SHELL_VAR **map_over_funcs(function)
sh_var_map_func_t *function;
{
	VARLIST *vlist;
	SHELL_VAR **ret;
...
static void flatten(var_hash_table, func, vlist, flags) HASH_TABLE *var_hash_table;
...
void sort_variables(array) SHELL_VAR **array;
{ qsort(array, strvec_len((char **)array), sizeof(SHELL_VAR *), (QSFUNC *)qsort_var_comp); }

static int qsort_var_comp(var1, var2)
SHELL_VAR **var1, **var2;
{
	int result;
...
static int visible_and_exported(var)
SHELL_VAR *var;
...
static int export_environment_candidate(var)
SHELL_VAR *var;
{ return (exported_p(var) && (invisible_p(var) == 0 || imported_p(var))); }

/* Return non-zero if VAR is a local variable in the current context and
   is exported. */
static int local_and_exported(var)
SHELL_VAR *var;
{
...
static int variable_in_context(var)
SHELL_VAR *var;
...
static int visible_array_vars(var)
SHELL_VAR *var;
...
static SHELL_VAR *bind_tempenv_variable(name, value) const char *name;
char *value;
{
	SHELL_VAR *var;
...
SHELL_VAR *find_tempenv_variable(name) const char *name;
...
/* Take a variable from an assignment statement preceding a posix special
   ...
   mode. */
static void push_posix_temp_var(data) PTR_T data;
{
	SHELL_VAR *var, *v;
...
static inline char *mk_env_string(name, value, attributes) const char *name, *value;
int attributes;
{
...
static char **make_env_array_from_var_list(vars)
SHELL_VAR **vars;
{
...
static char **make_var_export_array(vcxt)
VAR_CONTEXT *vcxt;
{
	char **list;
	SHELL_VAR **vars;
...
char **add_or_supercede_exported_var(assign, do_alloc)
char *assign;
int do_alloc;
{
	register int i;
...
static void add_temp_array_to_env(temp_array, do_alloc, do_supercede) char **temp_array;
...
void update_export_env_inplace(env_prefix, preflen, value) char *env_prefix;
int preflen;
char *value;
{
	char *evar;
...
void dispose_var_context(vc) VAR_CONTEXT *vc;
{
	FREE(vc->name);
...
/* Make a new variable context with NAME and FLAGS and a HASH_TABLE of
   ...
   for shell functions. */
VAR_CONTEXT *push_var_context(name, flags, tempvars)
char *name;
int flags;
HASH_TABLE *tempvars;
{
...
static inline void push_posix_tempvar_internal(var, isbltin) SHELL_VAR *var;
int isbltin;
{
	SHELL_VAR *v;
...
static void push_builtin_var(data) PTR_T data;
{
	SHELL_VAR *var;
...
VAR_CONTEXT *push_scope(flags, tmpvars)
int flags;
HASH_TABLE *tmpvars;
...
void pop_scope(is_special) int is_special;
{
...
static void restore_dollar_vars(args) char **args;
{
	int i;
...
static void free_saved_dollar_vars(args) char **args;
{
	int i;
...
void push_context(name, is_subshell, tempvars) char *name; /* function name */
int is_subshell;
HASH_TABLE *tempvars;
{
	if (is_subshell == 0)
...
void push_dollar_vars() {
	if (dollar_arg_stack_index + 2 > dollar_arg_stack_slots) {
		dollar_arg_stack = (struct saved_dollar_vars *)xrealloc(
		    dollar_arg_stack, (dollar_arg_stack_slots += 10) * sizeof(struct saved_dollar_vars));
	}
...
void push_args(list) WORD_LIST *list;
{
...
static struct name_and_function special_vars[] = {{"BASH_COMPAT", sv_shcompat},
...
static inline int find_special_var(name) const char *name;
{
	register int i, r;
...
void stupidly_hack_special_variables(name) char *name;
{
	...
	{
		qsort(special_vars, N_SPECIAL_VARS, sizeof(special_vars[0]), (QSFUNC *)sv_compare);
		sv_sorted = 1;
	}
...
void sv_funcnest(name) char *name;
{
	SHELL_VAR *v;
	intmax_t num;
...
void sv_execignore(name) char *name;
...
#if defined(READLINE)
void sv_comp_wordbreaks(name) char *name;
{
	SHELL_VAR *sv;
...
void sv_terminal(name) char *name;
{
	if (interactive_shell && no_line_editing == 0)
		rl_reset_terminal(get_string_value("TERM"));
}

void sv_hostfile(name) char *name;
{
	SHELL_VAR *v;
...
/* What to do after the HISTSIZE or HISTFILESIZE variables change.
   ...
   lines. */
void sv_histsize(name) char *name;
{
	char *temp;
	intmax_t num;
	int hmax;
...
void sv_histignore(name) char *name;
{ setup_history_ignore(name); }

/* What to do after the HISTCONTROL variable changes. */
void sv_history_control(name) char *name;
{
	char *temp;
	char *val;
	int tptr;
...
void sv_histchars(name) char *name;
{
	char *temp;
...
void sv_histtimefmt(name) char *name;
{
	SHELL_VAR *v;
...
void sv_strict_posix(name) char *name;
{
	SHELL_VAR *var;
...
#if defined(ARRAY_VARS)
void set_pipestatus_array(ps, nproc) int *ps;
int nproc;
{
	SHELL_VAR *v;
	ARRAY *a;
...
void sv_xtracefd(name) char *name;
{
	SHELL_VAR *v;
	char *t, *e;
	int fd;
...
void sv_shcompat(name) char *name;
{
	SHELL_VAR *v;
	char *val;
...
#if defined(JOB_CONTROL)
void sv_childmax(name) char *name;
{
	char *tt;
	int s;
