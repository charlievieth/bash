...
static char **mkseq(start, end, incr, type, width)
...
int type, width;
{
	...
	if (result == 0) {
		internal_error(_("brace expansion: failed to allocate memory for %u elements"),
		               (unsigned int)nelem);
		return ((char **)NULL);
	}
...
static char **expand_seqterm(text, tlen)
char *text;
size_t tlen;
{
	char *t, *lhs, *rhs;
	...
	/* Now figure out whether LHS and RHS are integers or letters.  Both
	   sides have to match. */
	lhs_t =
	    (legal_number(lhs, &tl)) ? ST_INT : ((ISALPHA(lhs[0]) && lhs[1] == 0) ? ST_CHAR : ST_BAD);
...
*/
static int brace_gobbler(text, tlen, indx, satisfy)
char *text;
size_t tlen;
int *indx;
int satisfy;
	   ...
	   escape the quote character in a double-quoted string. */
		...
		if (c == satisfy && level == 0 && quoted == 0 && commas > 0) {
			/* We ignore an open brace surrounded by whitespace, and also
			   ...
			   by whitespace.  */
			if (c == '{' && ((!i || brace_whitespace(text[i - 1])) &&
...
int internal_error(format, arg1, arg2)
...
main() {
	...
	for (;;) {
		...
		if ((!fgets(example, 256, stdin)) || (strncmp(example, "quit", 4) == 0))
			break;
