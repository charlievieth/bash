...
#undef PRIdPTR
#define PRIdPTR \
	(sizeof(void *) == sizeof(long) ? "ld" : sizeof(void *) == sizeof(int) ? "d" : "lld")
#endif
#if !defined PRIiPTR || PRI_MACROS_BROKEN
#undef PRIiPTR
#define PRIiPTR \
	(sizeof(void *) == sizeof(long) ? "li" : sizeof(void *) == sizeof(int) ? "i" : "lli")
#endif
#if !defined PRIoPTR || PRI_MACROS_BROKEN
#undef PRIoPTR
#define PRIoPTR \
	(sizeof(void *) == sizeof(long) ? "lo" : sizeof(void *) == sizeof(int) ? "o" : "llo")
#endif
#if !defined PRIuPTR || PRI_MACROS_BROKEN
#undef PRIuPTR
#define PRIuPTR \
	(sizeof(void *) == sizeof(long) ? "lu" : sizeof(void *) == sizeof(int) ? "u" : "llu")
#endif
#if !defined PRIxPTR || PRI_MACROS_BROKEN
#undef PRIxPTR
#define PRIxPTR \
	(sizeof(void *) == sizeof(long) ? "lx" : sizeof(void *) == sizeof(int) ? "x" : "llx")
#endif
#if !defined PRIXPTR || PRI_MACROS_BROKEN
#undef PRIXPTR
#define PRIXPTR \
...
static const char *get_sysdep_segment_value(name) const char *name;
{
	...
	if (name[0] == 'P' && name[1] == 'R' && name[2] == 'I') {
		if (name[3] == 'd' || name[3] == 'i' || name[3] == 'o' || name[3] == 'u' ||
			...
			if (name[4] == 'L' && name[5] == 'E' && name[6] == 'A' && name[7] == 'S' &&
			    name[8] == 'T') {
...
const char *internal_function _nl_init_domain_conv(domain_file, domain, domainbinding)
...
void internal_function _nl_free_domain_conv(domain)
struct loaded_domain *domain;
{
...
void internal_function _nl_load_domain(domain_file, domainbinding)
struct loaded_l10nfile *domain_file;
struct binding *domainbinding;
{
	int fd;
	...
	case 0:
		...
		domain->orig_tab = (const struct string_desc *)((char *)data + W(domain->must_swap,
		                                                                 data->orig_tab_offset));
		domain->trans_tab = (const struct string_desc *)((char *)data + W(domain->must_swap,
		                                                                  data->trans_tab_offset));
		domain->hash_size = W(domain->must_swap, data->hash_tab_size);
		domain->hash_tab =
		    (domain->hash_size > 2 ?
		         (const nls_uint32 *)((char *)data + W(domain->must_swap, data->hash_tab_offset)) :
		         NULL);
		...
		default: {
			...
			if (n_sysdep_strings > 0) {
				...
				sysdep_segments =
				    (const struct sysdep_segment *)((char *)data + W(domain->must_swap,
				                                                     data->sysdep_segments_offset));
				sysdep_segment_values = alloca(n_sysdep_segments * sizeof(const char *));
				for (i = 0; i < n_sysdep_segments; i++) {
					const char *name =
					    (char *)data + W(domain->must_swap, sysdep_segments[i].offset);
				...
				memneed = 2 * n_sysdep_strings * sizeof(struct sysdep_string_desc) +
				          domain->hash_size * sizeof(nls_uint32);
				for (i = 0; i < 2 * n_sysdep_strings; i++) {
					const struct sysdep_string *sysdep_string =
					    (const struct sysdep_string
					         *)((char *)data +
					            W(domain->must_swap, i < n_sysdep_strings ?
					                                     orig_sysdep_tab[i] :
