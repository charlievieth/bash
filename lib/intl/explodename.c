...
char *_nl_find_language(name) const char *name;
{
...
int _nl_explode_name(
    name, language, modifier, territory, codeset, normalized_codeset, special, sponsor, revision)
char *name;
const char **language;
const char **modifier;
	...
	*language = cp = name;
	...
	if (*language == cp)
		...
		*territory = ++cp;
		...
		while (cp[0] != '\0' && cp[0] != '.' && cp[0] != '@' && cp[0] != '+' && cp[0] != ',' &&
		       cp[0] != '_')
			++cp;
