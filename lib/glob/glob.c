...
int glob_pattern_p(pattern) const char *pattern;
{
#if HANDLE_MULTIBYTE
	size_t n;
	wchar_t *wpattern;
...
static int extglob_skipname(pat, dname, flags)
char *pat, *dname;
int flags;
{
	char *pp, *pe, *t, *se;
...
static int skipname(pat, dname, flags)
char *pat;
char *dname;
int flags;
{
...
int flags;
{
	...
	if (noglob_dot_filenames == 0 && pat[0] != L'.' && (pat[0] != L'\\' || pat[1] != L'.') &&
...
static int wextglob_skipname(pat, dname, flags)
wchar_t *pat, *dname;
int flags;
{
#if EXTENDED_GLOB
...
static int mbskipname(pat, dname, flags)
char *pat, *dname;
int flags;
{
	int ret, ext;
...
#if HANDLE_MULTIBYTE
/* Remove backslashes quoting characters in PATHNAME by modifying PATHNAME. */
void wcdequote_pathname(wpathname) wchar_t *wpathname;
{
	int i, j;
...
static void wdequote_pathname(pathname) char *pathname;
{
	mbstate_t ps;
	size_t len, n;
	wchar_t *wpathname;
...
static void dequote_pathname(pathname) char *pathname;
{
	if (MB_CUR_MAX > 1)
...
static int glob_testdir(dir, flags)
char *dir;
int flags;
{
	struct stat finfo;
...
char **glob_vector(pat, dir, flags)
char *pat;
char *dir;
int flags;
{
	...
	if (skip == 0) {
	   ...
	   need to do this every time through the loop. */
		...
		/* Scan the directory, finding all names that match	 For each name that matches, allocate a
	   struct globval on the stack and store the name in it. Chain those structs together; lastlink
	...
	if (directory_len > 0 && (hasglob = glob_pattern_p(directory_name)) == 1) {
	   ...
	   so we can compensate if filename is [star][star] */
		if ((flags & GX_GLOBSTAR) && all_starstar == 0) {
			...
			while (dl >= 4 && d[dl - 1] == '/' && d[dl - 2] == '*' && d[dl - 3] == '*' &&
			       d[dl - 4] == '/')
				prev = dl, dl -= 3;
			if (dl != directory_len)
				last_starstar = 1;
	   ...
	   so we don't have to scan everything twice. */
		if (last_starstar && directory_len > 4 && filename[0] == '*' && filename[1] == '*' &&
		    filename[2] == 0) {
			directory_len -= 3;
		}
	   ...
	   FILENAME.  Concatenate the results together.  */
		for (i = 0; directories[i] != NULL; ++i) {
			...
			if (dname[0] == '\0' && filename[0]) {
				dflags |= GX_NULLDIR;
				dname =
				    "."; /* treat null directory name and non-null filename as current directory */
			}
...
#undef NULL_PLACEHOLDER
				...
				if (array != temp_results)
					free((char *)array);
				else if ((dflags & GX_ALLDIRS) && filename[0] == '*' && filename[1] == '*' &&
				         filename[2] == '\0')
...
only_filename:
	...
	if (*filename == '\0') {
	  ...
	  /* If we want all directories (dflags & GX_ALLDIRS) and we're not
		...
		result =
		    glob_dir_to_array((dflags & GX_ALLDIRS) ? "" : directory_name, temp_results, flags);
