...
static int GMATCH(string, se, pattern, pe, ends, flags)
CHAR *string, *se;
CHAR *pattern, *pe;
struct STRUCT *ends;
int flags;
	...
	while (p < pe) {
	   ...
	   just return what EXTMATCH() returns. */
		...
		case L('*'): /* Match zero or more characters */
			...
			if (n == se &&
			    ((flags & FNM_EXTMATCH) && (c == L('!') || c == L('?')) && *p == L('('))) {
				--p;
				if (EXTMATCH(c, n, se, p, pe, flags) == 0)
...
static CHAR *PARSE_COLLSYM(p, vp)
CHAR *p;
INT *vp;
{
	register int pc;
...
/* Scan a pattern starting at STRING and ending at END, keeping track of
   ...
   empty or invalid. */
/*static*/ CHAR *PATSCAN(string, end, delim)
CHAR *string, *end;
INT delim;
{
...
static int STRCOMPARE(p, pe, s, se)
CHAR *p, *pe, *s, *se;
{
	int ret;
	CHAR c1, c2;
...
/* Match a ksh extended pattern specifier.  Return FNM_NOMATCH on failure or
   ...
   gmatch recursively. */
static int EXTMATCH(xc, s, se, p, pe, flags)
