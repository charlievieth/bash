...
static int _fnmatch_fallback(s, p)
...
static int charcmp(c1, c2, forcecoll)
int c1, c2;
int forcecoll;
{
	static char s1[2] = {' ', '\0'};
...
static char const *const cclass_name[] = {"",      "ascii", "alnum", "alpha", "blank",
...
static int cclass_test(c, char_class)
int c;
...
/* Construct a string w1 = "c1" and a pattern w2 = "[[=c2=]]" and pass them
   to fnmatch to see if wide characters c1 and c2 collate as members of the
   same equivalence class. We can't really do this portably any other way */
static int _fnmatch_fallback_wc(c1, c2)
...
static int charcmp_wc(c1, c2, forcecoll)
wint_t c1, c2;
int forcecoll;
{
...
static int rangecmp_wc(c1, c2, forcecoll)
wint_t c1, c2;
int forcecoll;
{
	int r;
...
/* Return 1 if there are no char class [:class:] expressions (degenerate case)
   ...
   locales. */
static int posix_cclass_only(pattern)
char *pattern;
{
	char *p, *p1;
