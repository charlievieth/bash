...
/* Copyright (C) 1985, 1986, 1993,1994, 1995, 1998, 2001,2003,2005,2006,2008,2009 Free Software
   Foundation, Inc.
...
__private_extern__ int tgetnum(cap)
char *cap;
{
...
__private_extern__ int tgetflag(cap)
char *cap;
{
	register char *ptr = find_capability(term_entry, cap);
	return ptr && ptr[-1] == ':';
...
__private_extern__ char *tgetstr(cap, area)
char *cap;
char **area;
{
	register char *ptr = find_capability(term_entry, cap);
...
__private_extern__ int tputs(str, nlines, outfun)
register char *str;
int nlines;
register int (*outfun)();
{
...
static int valid_filename_p(fn)
char *fn;
{
...
__private_extern__ int tgetent(bp, name)
char *bp, *name;
{
