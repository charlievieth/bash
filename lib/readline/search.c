...
/* Make the data from the history entry ENTRY be the contents of the
   ...
   must set it. */
static void make_history_line_current(HIST_ENTRY *entry) {
	UNDO_LIST *xlist;
...
static int noninc_search_from_pos(char *string, int pos, int dir, int flags, int *ncp) {
	int ret, old, sflags;
	char *s;
...
static int noninc_dosearch(char *string, int dir, int flags) {
	int oldpos, pos, ind;
	HIST_ENTRY *entry;
...
static _rl_search_cxt *_rl_nsearch_init(int dir, int pchar) {
	_rl_search_cxt *cxt;
	char *p;
...
int _rl_nsearch_cleanup(_rl_search_cxt *cxt, int r) {
...
static void _rl_nsearch_abort(_rl_search_cxt *cxt) {
	rl_maybe_unsave_line();
...
static int noninc_search(int dir, int pchar) {
	_rl_search_cxt *cxt;
	int c, r;
...
int _rl_nsearch_callback(_rl_search_cxt *cxt) {
	int c, r;
...
static int rl_history_search_internal(int count, int dir) {
	HIST_ENTRY *temp;
	...
	while (count) {
		RL_CHECK_SIGNALS();
		ret = noninc_search_from_pos(history_search_string, _rl_history_search_pos + dir, dir, 0,
		                             &newcol);
		if (ret == -1)
			break;
...
static void rl_history_search_reinit(int flags) {
	int sind;
...
/* Search backward through the history for the string of characters
   from the start of the line to rl_point.  This is a non-incremental
   search. */
int rl_history_search_backward(int count, int ignore) {
	if (count == 0)
		return (0);
...
/* Search backward through the history for the string of characters
   from the start of the line to rl_point.  This is a non-incremental
   search. */
int rl_history_substr_search_backward(int count, int ignore) {
	if (count == 0)
		return (0);
