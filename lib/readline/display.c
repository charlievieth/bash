...
static char *expand_prompt(char *pmt, int flags, int *lp, int *lip, int *niflp, int *vlp) {
	...
	ms = (((pmt == rl_prompt) ^ (flags & PMT_MULTILINE)) && _rl_show_mode_in_prompt) ?
	         prompt_modestr(&mlen) :
	         0;
	if (ms) {
		l = strlen(pmt);
	...
	local_prompt_newlines =
	    (int *)xrealloc(local_prompt_newlines, sizeof(int) * (newlines_guess + 1));
...
int rl_expand_prompt(char *prompt) {
	...
	if (p == 0) {
		...
		local_prompt = expand_prompt(prompt, 0, &prompt_visible_length, &prompt_last_invisible,
	   ...
	   final newline is now null-terminated. */
		local_prompt_prefix = expand_prompt(prompt, PMT_MULTILINE, &prompt_prefix_length,
		                                    (int *)NULL, (int *)NULL, (int *)NULL);
		*t = c;

		local_prompt =
		    expand_prompt(p, PMT_MULTILINE, &prompt_visible_length, &prompt_last_invisible,
		                  &prompt_invis_chars_first_line, &prompt_physical_chars);
		local_prompt_len = local_prompt ? strlen(local_prompt) : 0;
...
/* Allocate the various line structures, making sure they can hold MINSIZE
   ...
   anything. */
static void realloc_line(int minsize) {
	int minimum_size;
	int newsize, delta;
...
static void invis_adds(int *outp, const char *str, int n, char face) {
	int i;
...
static void set_active_region(int *beg, int *end) {
	...
	do {                                                                                         \
		...
		if (newlines >= (line_state_invisible->wbsize - 2)) {                                    \
			...
			line_state_invisible->wrapped_line = (int *)xrealloc(                                \
			    line_state_invisible->wrapped_line, line_state_invisible->wbsize * sizeof(int)); \
		}                                                                                        \
	} while (0)
#else
#define CHECK_INV_LBREAKS()                                                       \
	...
	do {                                                                              \
		...
		if (lpos >= _rl_screenwidth) {                                                \
			...
			if (newlines >= (line_state_invisible->wbsize - 2)) {                     \
				line_state_invisible->wbsize *= 2;                                    \
				line_state_invisible->wrapped_line =                                  \
				    (int *)xrealloc(line_state_invisible->wrapped_line,               \
...
#define INVIS_FIRST() \
	(prompt_physical_chars > _rl_screenwidth ? prompt_invis_chars_first_line : wrap_offset)
...
#define OLD_CPOS_IN_PROMPT()                                                           \
	(cpos_adjusted == 0 && _rl_last_c_pos != o_cpos && _rl_last_c_pos > wrap_offset && \
	 o_cpos < prompt_last_invisible)
			...
			for (linenum = 0; linenum <= inv_botlin; linenum++) {
			   ...
			   the locale from a non-multibyte to a multibyte one. */
				...
				cpos_adjusted = 0;
				update_line(VIS_LINE(linenum), VIS_LINE_FACE(linenum), INV_LINE(linenum),
				            INV_LINE_FACE(linenum), linenum, VIS_LLEN(linenum), INV_LLEN(linenum),
				            inv_botlin);
			   ...
			   wrap_offset. */
				...
				else if (cpos_adjusted == 0 && linenum == prompt_last_screen_line &&
				         prompt_physical_chars > _rl_screenwidth &&
				         (mb_cur_max > 1 && rl_byte_oriented == 0) && _rl_last_c_pos != o_cpos &&
				         _rl_last_c_pos >
				             (prompt_last_invisible - _rl_screenwidth -
				              prompt_invis_chars_first_line)) /* XXX - rethink this last one */
		  ...
		  if (nleft)
			...
			if (_rl_vis_botlin > inv_botlin) {
				...
				for (; linenum <= _rl_vis_botlin; linenum++) {
					...
					_rl_clear_to_eol((linenum == _rl_vis_botlin) ? strlen(tt) : _rl_screenwidth);
				}
			}
			...
			if (cursor_linenum == 0 && wrap_offset > 0 && _rl_last_c_pos > 0 &&
				...
				if (mb_cur_max > 1 && rl_byte_oriented == 0)
					_rl_last_c_pos =
					    _rl_col_width(local_prompt, 0, nleft, 1) - wrap_offset + modmark;
				else
...
#define M_OFFSET(margin, offset) ((margin) == 0 ? offset : 0)
	   ...
	   wrap offset into account. */
		...
		if (rl_display_fixed == 0 || forced_display || lmargin != last_lmargin) {
			...
			if ((mb_cur_max > 1 && rl_byte_oriented == 0) && displaying_prompt_first_line &&
			    OLD_CPOS_IN_PROMPT())
				_rl_last_c_pos -= prompt_invis_chars_first_line; /* XXX - was wrap_offset */
			...
			if ((M_OFFSET(lmargin, wrap_offset) > visible_wrap_offset) && (_rl_last_c_pos == out) &&
			    displaying_prompt_first_line && t < visible_first_line_len) {
...
static void putc_face(int c, int face, char *cur_face) {
	char cf;
	cf = *cur_face;
	if (cf != face) {
...
static void puts_face(const char *str, const char *face, int n) {
	int i;
	char cur_face;
...
static void norm_face(char *face, int n) {
...
static void update_line(char *old,
			...
			if (newwidth > 0) {
			   ...
			   can extend the old line enough to fit the new bytes. */
				...
				if (oldbytes != 0 && newbytes != 0) {
					...
					for (i = current_line + 1;
					     j != 0 && i <= inv_botlin + 1 && i <= _rl_vis_botlin + 1; i++)
						vis_lbreaks[i] += j;
				}
			} else {
				putc(' ', rl_outstream);
	...
	if (current_line == 0 && !_rl_horizontal_scroll_mode && _rl_term_cr &&
	   ...
	   the old and new lines are the same and short-circuit now if we are. */
		if ((od <= prompt_last_invisible || nd <= prompt_last_invisible) && omax == nmax &&
			...
			if (temp > 0) {
				...
				if (mb_cur_max > 1 && rl_byte_oriented == 0) {
					...
					if (wrap_offset > prompt_invis_chars_first_line &&
					    ...
					    prompt_physical_chars > _rl_screenwidth && _rl_horizontal_scroll_mode == 0)
					...
					/* If we just output a new line including the prompt, and
					   ...
					   a goto dumb_update; */
					else if (current_line == 0 && nfd == new &&prompt_invis_chars_first_line &&
					         local_prompt_len <= temp &&wrap_offset >=
					                 prompt_invis_chars_first_line &&_rl_horizontal_scroll_mode ==
					             0)
	...
	if (current_line == 0 && mb_cur_max > 1 && rl_byte_oriented == 0 &&
	    (_rl_last_c_pos > 0 || o_cpos > 0) && _rl_last_c_pos == prompt_physical_chars)
		cpos_adjusted = 1;
#endif
	...
	if (mb_cur_max > 1 && rl_byte_oriented == 0) {
	   ...
	   XXX - experimental */
		if (current_line == 0 && nfd == new &&newchars > prompt_last_invisible &&
		    newchars <= local_prompt_len &&local_prompt_len <= nmax &&current_invis_chars !=
		        visible_wrap_offset) {
	...
	if (col_lendiff > 0) /* XXX - was lendiff */
	{
	   ...
	   around on auto-wrapping terminals. */
		else if (_rl_terminal_can_insert && ((2 * col_temp) >= col_lendiff || _rl_term_IC) &&
			...
			if (*ols &&
			    ((_rl_horizontal_scroll_mode && _rl_last_c_pos == 0 &&
			      lendiff > prompt_visible_length && current_invis_chars > 0) == 0) &&
			    (((mb_cur_max > 1 && rl_byte_oriented == 0) && current_line == 0 && wrap_offset &&
			      ((nfd - new) <= prompt_last_invisible) &&
			...
			if ((mb_cur_max > 1 && rl_byte_oriented == 0) &&
			    ...
			    displaying_prompt_first_line && wrap_offset != prompt_invis_chars_first_line &&
			    ((nfd - new) < (prompt_last_invisible -
			...
			if ((mb_cur_max > 1 && rl_byte_oriented == 0) && current_line == 0 && wrap_offset &&
			    displaying_prompt_first_line && wrap_offset == prompt_invis_chars_first_line &&
			    visible_wrap_offset != current_invis_chars &&
	...
	} else /* Delete characters from line. */
	{
		...
		if (_rl_term_dc && (2 * col_temp) >= -col_lendiff) {
			...
			if (_rl_horizontal_scroll_mode && _rl_last_c_pos == 0 && displaying_prompt_first_line &&
			...
			if (_rl_horizontal_scroll_mode && displaying_prompt_first_line == 0 && col_lendiff &&
			...
			if (bytes_to_insert > 0) {
			   ...
			   set cpos_adjusted accordingly. */
				...
				if (mb_cur_max > 1 && rl_byte_oriented == 0) {
					...
					_rl_last_c_pos += _rl_col_width(nfd, 0, bytes_to_insert, 1);
					if (current_line == 0 && wrap_offset && displaying_prompt_first_line &&
		...
		/* Otherwise, print over the existing material. */
			...
			if (temp > 0) {
			   ...
			   set cpos_adjusted accordingly. */
				...
				if (mb_cur_max > 1 && rl_byte_oriented == 0) {
					if (current_line == 0 && wrap_offset && displaying_prompt_first_line &&
					    _rl_last_c_pos > wrap_offset && ((nfd - new) <= prompt_last_invisible))
						ADJUST_CPOS(wrap_offset); /* XXX - prompt_invis_chars_first_line? */
				}
			}
		clear_rest_of_line:
			...
			if (mb_cur_max > 1 && rl_byte_oriented == 0)
				col_lendiff =
				    _rl_col_width(old, 0, oe - old, 1) - _rl_col_width(new, 0, ne - new, 1);
			else
				col_lendiff = lendiff;
...
static void _rl_move_cursor_relative(int new, const char *data, const char *dataf) {
	register int i;
	...
	if (mb_cur_max > 1 && rl_byte_oriented == 0) {
	   ...
	   character function calls. */
		...
		else if (new > local_prompt_len &&
		         local_prompt &&memcmp(data, local_prompt, local_prompt_len) == 0) {
	   ...
	   buffer position relative to the start of the line */
		if (adjust &&
		    ((new > prompt_last_invisible) ||                /* XXX - don't use woff here */
		     (new + in_invisline > prompt_last_invisible) || /* invisible line */
		     (prompt_physical_chars >= _rl_screenwidth &&    /* visible line */
		      _rl_last_v_pos == prompt_last_screen_line && wrap_offset >= woff && dpos >= woff &&
...
#endif
{
	...
	local_prompt = expand_prompt(msg_buf, 0, &prompt_visible_length, &prompt_last_invisible,
	                             &prompt_invis_chars_first_line, &prompt_physical_chars);
...
void _rl_update_final(void) {
	...
	if (_rl_vis_botlin && _rl_last_c_pos == 0 && visible_line[vis_lbreaks[_rl_vis_botlin]] == 0) {
		_rl_vis_botlin--;
		full_lines = 1;
	}
	_rl_move_vert(_rl_vis_botlin);
	...
	if (full_lines && _rl_term_autowrap && botline_length == _rl_screenwidth) {
	   ...
	   subscripts of LAST_LINE. */
		...
		_rl_clear_to_eol(0);
		puts_face(&last_line[_rl_screenwidth - 1 + woff], &last_face[_rl_screenwidth - 1 + woff],
		          1);
	}
	_rl_vis_botlin = 0;
...
static void redraw_prompt(char *t) {
	...
	rl_display_prompt = t;
	local_prompt = expand_prompt(t, PMT_MULTILINE, &prompt_visible_length, &prompt_last_invisible,
	                             &prompt_invis_chars_first_line, &prompt_physical_chars);
	local_prompt_prefix = (char *)NULL;
	local_prompt_len = local_prompt ? strlen(local_prompt) : 0;
...
static int _rl_col_width(const char *str, int start, int end, int flags) {
	...
	else if (flags && start == 0 && local_prompt_len > 0 && end > local_prompt_len &&
	         local_prompt && memcmp(str, local_prompt, local_prompt_len) == 0) {
