...
STATIC_CALLBACK char *readline_internal_teardown(int eof) {
	char *temp;
	HIST_ENTRY *entry;
...
void _rl_internal_char_cleanup(void) {
	...
	if (rl_erase_empty_line && rl_done && rl_last_func == rl_newline && rl_point == 0 &&
	    rl_end == 0)
...
return (eof_found);
...
#if defined(READLINE_CALLBACKS)
	_rl_keyseq_cxt *_rl_keyseq_cxt_alloc(void) {
		_rl_keyseq_cxt *cxt;
	...
	void _rl_keyseq_cxt_dispose(_rl_keyseq_cxt * cxt) {
		xfree(cxt);
	}
	...
	static int _rl_subseq_getchar(int key) {
		int k;
	...
	int _rl_dispatch_subseq(register int key, Keymap map, int got_subseq) {
		int r, newkey;
		char *macro;
		...
		case ISFUNC:
			...
			if (func) {
				...
				if (rl_pending_input == 0 && map[key].function != rl_digit_argument &&
				    map[key].function != rl_vi_arg_digit)
#else
			if (rl_pending_input == 0 && map[key].function != rl_digit_argument)
#endif
...
#if defined(VI_MODE)
		if (rl_editing_mode == vi_mode && _rl_keymap == vi_movement_keymap && key != ANYOTHERKEY &&
		    _rl_dispatching_keymap == vi_movement_keymap && _rl_vi_textmod_command(key))
	...
	static int _rl_subseq_result(int r, Keymap map, int key, int got_subseq) {
		Keymap m;
		int type, nt;
...
_emx_build_environ (void)
{
  ...
  *tp = 0;
  ...
  running_in_emacs = sh_get_env_value ("EMACS") != (char *)0;
	...
	static void bind_arrow_keys_internal(Keymap map) {
		Keymap xkeymap;
	...
	int rl_restore_state(struct readline_state * sp) {
		if (sp == 0)
			return -1;
