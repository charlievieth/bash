...
void _rl_with_macro_input(char *string) {
...
int _rl_peek_macro_key(void) {
	if (rl_executing_macro == 0)
		return (0);
	if (rl_executing_macro[executing_macro_index] == 0 &&
	    (macro_list == 0 || macro_list->string == 0))
		return (0);
...
int rl_print_last_kbd_macro(int count, int ignore) {
	char *m;
...
void rl_push_macro_input(char *macro) {
