...
static int set_alarm(unsigned int *secs, unsigned int *usecs) {
	struct itimerval it;
...
int rl_set_timeout(unsigned int secs, unsigned int usecs) {
...
int rl_timeout_remaining(unsigned int *secs, unsigned int *usecs) {
...
#if defined(HAVE_PSELECT) || defined(HAVE_SELECT)
int _rl_timeout_select(int nfds,
                       fd_set *readfds,
                       fd_set *writefds,
                       fd_set *exceptfds,
...
static void _rl_timeout_handle() {
...
int _rl_timeout_handle_sigalrm() {
#if defined(RL_TIMEOUT_USE_SIGALRM)
	if (timerisunset(&timeout_point))
		return -1;
...
/* read multibyte char */
int _rl_read_mbchar(char *mbchar, int size) {
	int mb_len, c;
