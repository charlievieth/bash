...
static char *legal_lang_values[] = {"iso88591", "iso88592", "iso88593", "iso88594", "iso88595",
...
static int utf8locale(char *lspec) {
	char *cp;
	size_t len;
...
static int _rl_set_localevars(char *localestr, int force) {
#if defined(HAVE_SETLOCALE)
...
#if !defined(HAVE_SETLOCALE)
static char *normalize_codeset(char *codeset) {
	size_t namelen, i;
	int len, all_digits;
	char *wp, *retval;
...
static char *find_codeset(char *name, size_t *lenp) {
