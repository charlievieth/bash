...
char *get_history_event(const char *string, int *caller_index, int delimiting_quote) {
	register int i;
	register char c;
	HIST_ENTRY *entry;
...
static void hist_string_extract_single_quoted(char *string, int *sindex, int flags) {
	register int i;
...
static char *get_subst_pattern(char *str, int *iptr, int delimiter, int is_rhs, int *lenptr) {
...
static void postproc_subst_rhs(void) {
	char *new;
	int i, j, new_size;
...
static int history_expand_internal(
    char *string, int start, int qc, int *end_index_ptr, char **ret_string, char *current_line) {
...
static char *get_history_word_specifier(char *spec, char *from, int *caller_index) {
...
/* Parse STRING into tokens and return an array of strings.  If WIND is
   ...
   *INDP. */
static char **history_tokenize_internal(const char *string, int wind, int *indp) {
	char **result;
...
static char *history_find_word(char *line, int ind) {
	char **words, *s;
	int i, wind;
