...
static void rl_display_search(char *search_string, int flags, int where) {
	char *message;
...
static _rl_search_cxt *_rl_isearch_init(int direction) {
	...
	cxt->search_terminators =
	    _rl_isearch_terminators ? _rl_isearch_terminators : default_isearch_terminators;
...
static void _rl_isearch_fini(_rl_search_cxt *cxt) {
...
int _rl_isearch_dispatch(_rl_search_cxt *cxt, int c) {
	...
	if (c >= 0 && cxt->keymap[c].type == ISKMAP &&
	    strchr(cxt->search_terminators, cxt->lastc) == 0) {
	   ...
	   that key as if it was all we read. */
		if (_rl_keyseq_timeout > 0 && RL_ISSTATE(RL_STATE_CALLBACK) == 0 &&
		    RL_ISSTATE(RL_STATE_INPUTPENDING) == 0 && _rl_pushed_input_available() == 0 &&
...
add_character:
	...
	if (cxt->sflags & SF_CHGKMAP) {
	   ...
	   character and the current character into the search string. */
		else if (cxt->lastc > 0 && cxt->prevc > 0 && cxt->keymap[cxt->prevc].type == ISKMAP &&
		         (f == 0 || f == rl_insert)) {
...
opcode_dispatch:
	...
	case -3: /* C-H, DEL */
	   ...
	   do until we have a real isearch-undo. */
		if (cxt->search_string_index == 0)
			...
			wstart =
			    _rl_find_prev_mbchar(cxt->search_string, cxt->search_string_index, MB_FIND_NONZERO);
			if (wstart >= 0)
...
#endif
			cxt->search_string[cxt->search_string_index++] =
			    cxt->lastc; /* XXX - was c instead of lastc */
		cxt->search_string[cxt->search_string_index] = '\0';
		break;
	}
	...
	for (cxt->sflags &= ~(SF_FOUND | SF_FAILED);;) {
	   ...
	   searching for. */
		do {
			...
			if ((cxt->sflags & SF_REVERSE) ? (cxt->history_pos < 0) :
			                                 (cxt->history_pos == cxt->hlen)) {
				cxt->sflags |= SF_FAILED;
				break;
			}
		    ...
		    (cxt->prev_line_found && STREQ(cxt->prev_line_found, cxt->lines[cxt->history_pos])) ||
		...
		cxt->sline_index =
		    (cxt->sflags & SF_REVERSE) ? cxt->sline_len - cxt->search_string_index : 0;
	}
	...
	if (cxt->sflags & SF_FAILED) {
		...
		cxt->history_pos = cxt->last_found_line;
		rl_deactivate_mark();
		rl_display_search(cxt->search_string, cxt->sflags,
