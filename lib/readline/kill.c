...
static int _rl_copy_to_kill_ring(char *text, int append) {
	char *old, *new;
	int slot;
...
static int region_kill_internal(int delete) {
	char *text;
...
/* Copy COUNT words to the kill ring.  DIR says which direction we look
   to find the words. */
static int _rl_copy_word_as_kill(int count, int dir) {
	int om, op, r;
...
int _rl_read_bracketed_paste_prefix(int c) {
...
/* Get a character from wherever we read input, handling input in bracketed
   paste mode. If we don't have or use bracketed paste mode, this can be
   used in place of rl_read_key(). */
int _rl_bracketed_read_mbstring(char *mb, int mlen) {
	int c, r;
...
int rl_paste_from_clipboard(int count, int key) {
	char *data, *ptr;
	int len;
