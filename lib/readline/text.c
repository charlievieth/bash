...
int rl_insert(int count, int c) {
	...
	while (_rl_optimize_typeahead && rl_num_chars_to_read == 0 &&
	       ...
	       _rl_pushed_input_available() == 0 && _rl_input_queued(0) && (n = rl_read_key()) > 0 &&
		...
		do {
...
#endif
			{
				...
				if (dir < 0)
					...
					rl_point =
					    (dir == FTO) ? _rl_find_prev_mbchar(rl_line_buffer, pos, MB_FIND_ANY) : pos;
				break;
			}
#if defined(HANDLE_MULTIBYTE)
			prepos = pos;
...
#if defined(HANDLE_MULTIBYTE)
		while ((dir < 0) ?
		           (pos = _rl_find_prev_mbchar(rl_line_buffer, pos, MB_FIND_ANY)) != prepos :
...
#if defined(READLINE_CALLBACKS)
static int _rl_char_search_callback(data)
