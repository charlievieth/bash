...
static bool get_funky_string(char **dest, const char **src, bool equals_end, size_t *output_count) {
	...
	enum { ST_GND, ST_BACKSLASH, ST_OCTAL, ST_HEX, ST_CARET, ST_END, ST_ERROR } state;
	const char *p;
	char *q;
	...
	while (state > 0) {
		...
		case 1: /* First label character */
			...
			case '*':
				...
				state = (get_funky_string(&buf, &p, true, &ext->ext.len) ? 4 : -1);
				break;
			...
			if (*(p++) == '=') /* It *should* be...  */
			{
				for (ind_no = 0; indicator_name[ind_no] != NULL; ++ind_no) {
					if (STREQ(label, indicator_name[ind_no])) {
						...
						state =
						    (get_funky_string(&buf, &p, false, &_rl_color_indicator[ind_no].len) ?
						         1 :
						         -1);
						break;
					}
		...
		case 4: /* Equal sign after *.ext */
			if (*(p++) == '=') {
				ext->seq.string = buf;
				state = (get_funky_string(&buf, &p, false, &ext->seq.len) ? 1 : -1);
			} else
				state = -1;
