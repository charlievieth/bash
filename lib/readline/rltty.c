...
static void set_winsize(tty) int tty;
{
...
static void save_tty_chars(TIOTYPE *tiop) {
...
static int get_tty_settings(int tty, TIOTYPE *tiop) {
	set_winsize(tty);
...
static int set_tty_settings(int tty, TIOTYPE *tiop) {
	if (tiop->flags & SGTTY_SET) {
...
static void prepare_terminal_settings(int meta_flag, TIOTYPE oldtio, TIOTYPE *tiop) {
...
#endif
	if (((oldtio.sgttyb.sg_flags & ANYP) == ANYP) || ((oldtio.sgttyb.sg_flags & ANYP) == 0)) {
...
static void rltty_warning(char *msg) {
	_rl_errmsg("warning: %s", msg);
}
#endif

#if defined(_AIX)
void setopost(TIOTYPE *tp) {
...
static int _get_tty_settings(int tty, TIOTYPE *tiop) {
	int ioctl_ret;
...
#elif defined(NEW_TTY_DRIVER)
static void set_special_char(Keymap kmap, TIOTYPE *tiop, int sc, rl_command_func_t *func) {
...
static void _rl_bind_tty_special_chars(Keymap kmap, TIOTYPE ttybuff) {
...
void rltty_set_default_bindings(Keymap kmap) {
#if !defined(NO_TTY_DRIVER)
	TIOTYPE ttybuff;
	int tty;
...
int _rl_disable_tty_signals(void) {
	return 0;
}
