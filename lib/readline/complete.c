...
static void _rl_complete_sigcleanup(int sig, void *ptr) {
...
static int _rl_internal_pager(int lines) {
	int i;
...
static int path_isdir(const char *filename) {
	struct stat finfo;
...
static int stat_char(char *filename) {
	...
	if (S_ISDIR(finfo.st_mode))
		...
		ext = strrchr(fn, '.');
		if (ext && (_rl_stricmp(ext, ".exe") == 0 || _rl_stricmp(ext, ".cmd") == 0 ||
		            _rl_stricmp(ext, ".bat") == 0 || _rl_stricmp(ext, ".com") == 0))
			character = '*';
#else
...
#if defined(COLOR_SUPPORT)
static int colored_stat_start(const char *filename) {
...
static int fnprint(const char *to_print, int prefix_bytes, const char *real_pathname) {
	int printed_len, w;
	const char *s;
...
char _rl_find_completion_word(int *fp, int *dp) {
	...
	if (scan) {
		if (rl_char_is_quoted_p)
			isbrk = (found_quote == 0 || (*rl_char_is_quoted_p)(rl_line_buffer, rl_point) == 0) &&
		...
		if (isbrk) {
			...
			if (rl_basic_quote_characters && strchr(rl_basic_quote_characters, scan) &&
			    (end - rl_point) > 1)
				delimiter = scan;
...
static char **gen_completion_matches(char *text,
                                     int start,
                                     int end,
	   ...
	   the user typed in the face of multiple matches differing in case. */
		if (_rl_completion_case_fold) {
			...
			if (rl_filename_completion_desired && rl_filename_dequoting_function &&
				...
				dtext =
				    (*rl_filename_dequoting_function)((char *)text, rl_completion_quote_character);
				text = dtext;
			}
...
static int postprocess_matches(char ***matchesp, int matching_filenames) {
...
static void display_matches(char **matches) {
	int len, max, i;
	char *temp;
...
static char *make_quoted_replacement(char *match, int mtype, char *qc) {
	...
	should_quote = match && rl_completer_quote_characters && rl_filename_completion_desired &&
...
static void insert_match(char *match, int start, int mtype, char *qc) {
	char *replacement, *r;
	char oqc;
	int end, rlen;
	...
	if (replacement) {
		...
		if (qc && *qc && start && rl_line_buffer[start - 1] == *qc && replacement[0] == *qc)
			start--;
...
static int append_to_match(char *text, int delimiter, int quote_char, int nontrivial_match) {
	...
	if (rl_filename_completion_desired) {
		...
		s = (nontrivial_match && rl_completion_mark_symlink_dirs == 0) ? LSTAT(filename, &finfo) :
		                                                                 stat(filename, &finfo);
		if (s == 0 && S_ISDIR(finfo.st_mode)) {
			if (_rl_complete_mark_directories /* && rl_completion_suppress_append == 0 */) {
			   ...
			   slash. */
				if (rl_point && rl_line_buffer[rl_point] == '\0' &&
...
static void insert_all_matches(char **matches, int point, char *qc) {
	int i;
	char *rp;
...
/* Compare a possibly-quoted filename TEXT from the line buffer and a possible
   ...
   text. */
static int compare_match(char *text, const char *match) {
	char *temp;
	int r;
...
int rl_complete_internal(int what_to_do) {
	...
	our_func = rl_completion_entry_function ? rl_completion_entry_function :
 ...
 */
char **rl_completion_matches(const char *text, rl_compentry_func_t *entry_function) {
	register int i;
	...
	while (string = (*entry_function)(text, matches)) {
		...
		if (matches + 1 >= match_list_size)
			match_list =
			    (char **)xrealloc(match_list, ((match_list_size += 10) + 1) * sizeof(char *));
	...
	if (_rl_completion_case_fold && _rl_completion_case_map) {
		...
		if (MB_CUR_MAX > 1 && rl_byte_oriented == 0) {
		    ...
		    if ((_rl_to_lower(convfn[0]) == _rl_to_lower(filename[0])) &&
		        (convlen >= filename_len) && (_rl_strnicmp(filename, convfn, filename_len) == 0))
			return 1;
	} else {
	...
	if (state == 0) {
...
#if defined(__MSDOS__) || defined(_WIN32)
		/* special hack for //X/... */
		if (dirname[0] == '/' && dirname[1] == '/' && ISALPHA((unsigned char)dirname[2]) &&
		    dirname[3] == '/')
	   ...
	   it if necessary. */
		if (rl_completion_found_quote && rl_filename_dequoting_function)
			users_dirname =
			    (*rl_filename_dequoting_function)(dirname, rl_completion_quote_character);
		else
	   ...
	   not modify the directory name pointer passed as an argument. */
		if (rl_directory_rewrite_hook)
			...
			xfree(users_dirname);
			users_dirname = savestring(dirname);
		} else if (tilde_dirname == 0 && rl_completion_found_quote &&
	...
	while (directory && (entry = readdir(directory))) {
	   ...
	   All other entries except "." and ".." match. */
		if (filename_len == 0) {
			...
			if (convfn[0] != '.' || (convfn[1] && (convfn[1] != '.' || convfn[2])))
				break;
		} else {
...
int rl_old_menu_complete(int count, int invoking_key) {
	rl_compentry_func_t *our_func;
	...
	if (rl_last_func != rl_old_menu_complete) {
		...
		matches = gen_completion_matches(orig_text, orig_start, orig_end, our_func, found_quote,
		                                 quote_char);
...
int rl_menu_complete(int count, int ignore) {
	rl_compentry_func_t *our_func;
	int matching_filenames, found_quote;
	...
	/* The first time through, we generate the list of matches and set things
	   up to insert them. */
	if ((rl_last_func != rl_menu_complete && rl_last_func != rl_backward_menu_complete) ||
	    full_completion) {
		/* Clean up from previous call, if any. */
		FREE(orig_text);
		if (matches)
	   ...
	   code below should take care of it. */
		if (*matches[0]) {
			insert_match(matches[0], orig_start, matches[1] ? MULT_MATCH : SINGLE_MATCH,
			             &quote_char);
