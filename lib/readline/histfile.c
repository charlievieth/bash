...
static char *history_tempfile(const char *filename) {
	const char *fn;
	char *ret, linkbuf[PATH_MAX + 1];
	size_t len;
	ssize_t n;
...
static int history_rename(const char *old, const char *new) {
#if defined(_WIN32)
...
static int histfile_backup(const char *filename, const char *back) {
#if defined(HAVE_READLINK)
...
#define SHOULD_CHOWN(finfo, nfinfo) (finfo.st_uid != nfinfo.st_uid || finfo.st_gid != nfinfo.st_gid)
