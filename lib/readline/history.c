...
int _hs_at_end_of_history(void) {
	   ...
	   trailing NULL.  */
		...
		if (history_size == 0) {
			...
			if (history_length == (history_size - 1)) {
				...
				the_history =
				    (HIST_ENTRY **)xrealloc(the_history, history_size * sizeof(HIST_ENTRY *));
			}
...
histdata_t free_history_entry(HIST_ENTRY *hist) {
	histdata_t x;
...
HIST_ENTRY *copy_history_entry(HIST_ENTRY *hist) {
	HIST_ENTRY *ret;
	char *ts;
...
void _hs_append_history_line(int which, const char *line) {
	HIST_ENTRY *hent;
...
void _hs_replace_history_data(int which, histdata_t *old, histdata_t *new) {
	HIST_ENTRY *entry;
	register int i, last;
