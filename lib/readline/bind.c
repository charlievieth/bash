...
int rl_bind_keyseq_if_unbound(const char *keyseq, rl_command_func_t *default_func) {
	return (rl_bind_keyseq_if_unbound_in_map(keyseq, default_func, _rl_keymap));
}
...
int rl_generic_bind(int type, const char *keyseq, char *data, Keymap map) {
      ...
      if (META_CHAR (ic) && _rl_convert_meta_chars_to_ascii)
	{
	  ...
	  if (map[ESC].type == ISKMAP)
		...
		if ((i + 1) < keys_len) {
			...
			if (k.function &&
			    ((k.type == ISFUNC && k.function != rl_do_lowercase_version) || k.type == ISMACR)) {
				map[ANYOTHERKEY] = k;
				k.function = 0;
			}
		} else {
	...
	if (type == ISFUNC && data == 0 && prevmap[prevkey].type == ISKMAP &&
	    ...
	    rl_binding_keymap[ANYOTHERKEY].type == ISFUNC && rl_empty_keymap(rl_binding_keymap)) {
...
rl_command_func_t *rl_function_of_keyseq(const char *keyseq, Keymap map, int *type) {
...
/* Assuming there is a numeric argument at the beginning of KEYSEQ (the
   ...
   return -1. */
int rl_trim_arg_from_keyseq(const char *keyseq, size_t len, Keymap map) {
...
#if defined(VI_MODE)
			if (map[ic].function != rl_digit_argument &&
			    map[ic].function != rl_universal_argument && map[ic].function != rl_vi_arg_digit)
#else
...
#else
_rl_init_file_error(va_alist) va_dcl
#endif
{
	va_list args;
	...
	if (currently_reading_init_file)
		fprintf(stderr, "%s: line %d: ", current_readline_init_file, current_readline_init_lineno);
...
static int parse_comparison_op(s, indp) const char *s;
int *indp;
{
	int i, peekc, op;
...
const char *const _rl_possible_control_prefixes[] = {"Control-", "C-", "CTRL-", (const char *)NULL};

const char *const _rl_possible_meta_prefixes[] = {"Meta", "M-", (const char *)NULL};
	...
	if (rl_terminal_name && _rl_strnicmp(args, "term=", 5) == 0) {
	   ...
	   `$if term=sun-cmd' into their .inputrc. */
		_rl_parsing_conditionalized_out =
		    _rl_stricmp(args + 5, tname) && _rl_stricmp(args + 5, rl_terminal_name);
		xfree(tname);
	}
#if defined(VI_MODE)
	...
	/* Check to see if the first word in ARGS is the same as the
	   value stored in rl_readline_name. */
		...
		if (CMPSTART(args[i]) == 0) {
			_rl_init_file_error("equality comparison operator expected, found `%s'",
			                    args[i] ? args + i : "end-of-line");
			return 0;
		}
		previ = i;
...
} parser_directives[] = {{"if", parser_if},
                         {"endif", parser_endif},
                         {"else", parser_else},
...
static int _rl_skip_to_delim(char *string, int start, int delim) {
	int i, c, passc;
...
static const struct {
	...
	int *value;
	int flags;
} boolean_varlist[] = {{"bind-tty-special-chars", &_rl_bind_stty_chars, 0},
...
static const char *boolean_varname(int i) {
...
} string_varlist[] = {{"active-region-end-color", V_STRING, sv_region_end_color},
...
static const char *string_varname(int i) {
...
static int bool_to_int(const char *value) {
	return (value == 0 || *value == '\0' || (_rl_stricmp(value, "on") == 0) ||
	        (value[0] == '1' && value[1] == '\0'));
}
...
static int sv_editmode(const char *value) {
...
static int sv_combegin(const char *value) {
	if (value && *value) {
...
static int sv_dispprefix(const char *value) {
	int nval = 0;
...
static int sv_compquery(const char *value) {
	int nval = 100;
...
static int sv_compwidth(const char *value) {
	int nval = -1;
...
static int sv_histsize(const char *value) {
	int nval;
...
static int sv_keymap(const char *value) {
	Keymap kmap;
...
static int sv_seqtimeout(const char *value) {
	int nval;
...
static int sv_region_start_color(const char *value) {
	return (_rl_reset_region_color(0, value));
}

static int sv_region_end_color(const char *value) {
	return (_rl_reset_region_color(1, value));
}

static int sv_bell_style(const char *value) {
	if (value == 0 || *value == '\0')
		_rl_bell_preference = AUDIBLE_BELL;
...
static int sv_isrchterm(const char *value) {
	int beg, end, delim;
	char *v;
...
static int sv_emacs_modestr(const char *value) {
	if (value && *value) {
...
static int sv_viins_modestr(const char *value) {
	if (value && *value) {
...
static int sv_vicmd_modestr(const char *value) {
	if (value && *value) {
...
static const assoc_list name_key_alist[] = {{"DEL", 0x7f},    {"ESC", '\033'},   {"Escape", '\033'},
                                            {"LFD", '\n'},    {"Newline", '\n'}, {"RET", '\r'},
                                            {"Return", '\r'}, {"Rubout", 0x7f},  {"SPC", ' '},
                                            {"Space", ' '},   {"Tab", 0x09},     {(char *)0x0, 0}};
...
static struct name_and_keymap builtin_keymap_names[] = {
    {"emacs", emacs_standard_keymap},   {"emacs-standard", emacs_standard_keymap},
    {"emacs-meta", emacs_meta_keymap},  {"emacs-ctlx", emacs_ctlx_keymap},
#if defined(VI_MODE)
    {"vi", vi_movement_keymap},         {"vi-move", vi_movement_keymap},
    {"vi-command", vi_movement_keymap}, {"vi-insert", vi_insertion_keymap},
#endif /* VI_MODE */
    {(char *)0x0, (Keymap)0x0}};
...
static char *_rl_get_string_variable_value(const char *name) {
...
static int substring_member_of_array(const char *string, const char *const *array) {
	while (*array) {
