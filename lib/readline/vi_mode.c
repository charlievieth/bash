...
int rl_vi_bWord(int count, int ignore) {
	while (count-- && rl_point > 0) {
	   ...
	   we will go back to the start of the previous word. */
		if (!whitespace(rl_line_buffer[rl_point]) && whitespace(rl_line_buffer[rl_point - 1]))
			rl_point--;
...
void _rl_vi_done_inserting(void) {
	if (_rl_vi_doing_insert) {
	   ...
	   probably will not). */
		...
		if (rl_undo_list &&
		    (_rl_vi_last_key_before_insert == 'i' || _rl_vi_last_key_before_insert == 'a' ||
		     _rl_vi_last_key_before_insert == 'I' || _rl_vi_last_key_before_insert == 'A'))
...
static int _rl_vi_arg_dispatch(int c) {
	int key;
...
static void _rl_mvcxt_init(_rl_vimotion_cxt *m, int op, int key) {
	m->op = op;
	m->state = m->flags = 0;
	m->ncxt = 0;
	m->numeric_arg = -1;
...
static _rl_vimotion_cxt *_rl_mvcxt_alloc(int op, int key) {
	_rl_vimotion_cxt *m;
...
static void _rl_mvcxt_dispose(_rl_vimotion_cxt *m) {
	xfree(m);
}

static int rl_domove_motion_callback(_rl_vimotion_cxt *m) {
	int c;
...
int _rl_vi_domove_motion_cleanup(int c, _rl_vimotion_cxt *m) {
	int r;
...
static int rl_domove_read_callback(_rl_vimotion_cxt *m) {
	int c, save;
	...
	else if (_rl_digit_p(c) && RL_ISSTATE(RL_STATE_CALLBACK) && RL_ISSTATE(RL_STATE_VIMOTION) &&
...
static int rl_vi_domove_getchar(_rl_vimotion_cxt *m) {
...
int rl_vi_domove(int x, int *ignore) {
	int r;
	_rl_vimotion_cxt *m;
...
static int vi_delete_dispatch(_rl_vimotion_cxt *m) {
	/* These are the motion commands that do not require adjusting the
	   mark. */
	if (((strchr(" l|h^0bBFT`", m->motion) == 0) && (rl_point >= m->start)) && (rl_mark < rl_end))
...
static int vi_change_dispatch(_rl_vimotion_cxt *m) {
	/* These are the motion commands that do not require adjusting the
	   mark.  c[wW] are handled by special-case code in rl_vi_domove(),
	   and already leave the mark at the correct location. */
	if (((strchr(" l|hwW^0bBFT`", m->motion) == 0) && (rl_point >= m->start)) && (rl_mark < rl_end))
		INCREMENT_POS(rl_mark);
...
static int vi_yank_dispatch(_rl_vimotion_cxt *m) {
	/* These are the motion commands that do not require adjusting the
	   mark. */
	if (((strchr(" l|h^0%bBFT`", m->motion) == 0) && (rl_point >= m->start)) && (rl_mark < rl_end))
		INCREMENT_POS(rl_mark);
...
static int vidomove_dispatch(_rl_vimotion_cxt *m) {
	int r;
...
#if defined(READLINE_CALLBACKS)
static int _rl_vi_callback_char_search(_rl_callback_generic_arg *data) {
	int c;
#if defined(HANDLE_MULTIBYTE)
...
/* Match brackets */
int rl_vi_match(int ignore, int key) {
...
static int _rl_vi_change_char(int count, int c, char *mb) {
	int p;
...
static int rl_vi_overstrike_yank(int count, int key) {
	int r, end;
...
static int rl_vi_overstrike_bracketed_paste(int count, int key) {
	int r;
	char *pbuf;
	size_t pblen;
...
#if defined(READLINE_CALLBACKS)
static int _rl_vi_callback_set_mark(_rl_callback_generic_arg *data) {
	_rl_callback_func = 0;
	_rl_want_redisplay = 1;
...
#if defined(READLINE_CALLBACKS)
static int _rl_vi_callback_goto_mark(_rl_callback_generic_arg *data) {
	_rl_callback_func = 0;
	_rl_want_redisplay = 1;
