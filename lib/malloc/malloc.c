...
#define ISMEMALIGN                                      \
	((char)0xd6) /* Stored before the value returned by \
...
#define CHAIN(a) (*(union mhead **)(sizeof(char *) + (char *)(a)))
...
/* How many bytes are actually allocated for a request of size N --
   ...
   malloc overhead. */
#define ALLOCATED_BYTES(n) (((n) + MOVERHEAD + MSLOP + MALIGN_MASK) & ~MALIGN_MASK)
...
#define RIGHT_BUCKET(nb, nu) (((nb) > binsizes[(nu)-1]) && ((nb) <= binsizes[(nu)]))
...
static const unsigned long binsizes[NBUCKETS] = {
    32UL,       64UL,        128UL,       256UL,       512UL,        1024UL,       2048UL,
    4096UL,     8192UL,      16384UL,     32768UL,     65536UL,      131072UL,     262144UL,
    524288UL,   1048576UL,   2097152UL,   4194304UL,   8388608UL,    16777216UL,   33554432UL,
    67108864UL, 134217728UL, 268435456UL, 536870912UL, 1073741824UL, 2147483648UL, 4294967295UL};
...
#if !defined(botch)
static void botch(s, file, line) const char *s;
const char *file;
int line;
{
...
static void xbotch(mem, e, s, file, line) PTR_T mem;
int e;
const char *s;
const char *file;
int line;
...
static void bcoalesce(nu) register int nu;
{
...
static void bsplit(nu) register int nu;
{
...
static void xsplit(mp, nu) union mhead *mp;
int nu;
{
	union mhead *nh;
	int nbuck, nblks, split_max;
...
void _malloc_block_signals(setp, osetp) sigset_t *setp, *osetp;
{
...
void _malloc_unblock_signals(setp, osetp) sigset_t *setp, *osetp;
{
#ifdef HAVE_POSIX_SIGNALS
...
static void lesscore(nu) /* give system back some memory */
...
static void morecore(nu) register int nu; /* size index to get more of  */
{
	register union mhead *mp;
	register int nblks;
	register long siz;
	...
	if (nu >= COMBINE_MIN && nu < COMBINE_MAX && nu <= malloc_mmap_threshold && busy[nu - 1] == 0 &&
	    nextf[nu - 1]) {
		bcoalesce(nu);
		if (nextf[nu] != 0)
			goto morecore_done;
	...
	if (nu > malloc_mmap_threshold) {
		mp = (union mhead *)mmap(0, sbrk_amt, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS,
		                         -1, 0);
...
static void malloc_debug_dummy() {
...
static PTR_T internal_malloc(n, file, line, flags) /* get a block */
size_t n;
const char *file;
int line, flags;
{
...
static void internal_free(mem, file, line, flags) PTR_T mem;
const char *file;
int line, flags;
{
	register union mhead *p;
	...
	if (p->mh_alloc != ISALLOC) {
		if (p->mh_alloc == ISFREE)
			xbotch(mem, ERR_DUPFREE, _("free: called with already freed block argument"), file,
			       line);
		else
			xbotch(mem, ERR_UNALLOC, _("free: called with unallocated block argument"), file, line);
	}
	...
	if (IN_BUCKET(nbytes, nunits) == 0)
		xbotch(mem, ERR_UNDERFLOW, _("free: underflow detected; mh_nbytes out of range"), file,
		       line);
	{
		int i;
		for (i = 0, z = p->mh_magic8; i < MAGIC8_NUMBYTES; i++)
			if (*z++ != MAGIC1)
				xbotch(mem, ERR_UNDERFLOW, _("free: underflow detected; magic8 corrupted"), file,
				       line);
	}
...
/* Assume the caller (internal_realloc) has already performed the sanity and
   ...
   information back in. */
static PTR_T internal_remap(mem, n, nunits, flags)
PTR_T mem;
register size_t n;
int nunits;
int flags;
...
static PTR_T internal_realloc(mem, n, file, line, flags)
PTR_T mem;
register size_t n;
const char *file;
int line, flags;
{
	...
	if (p->mh_alloc != ISALLOC)
		xbotch(mem, ERR_UNALLOC, _("realloc: called with unallocated block argument"), file, line);
	...
	/* Since the sizeof(u_bits32_t) bytes before the memory handed to the user
	   ...
	   original number of bytes requested. */
	if (IN_BUCKET(nbytes, nunits) == 0)
		xbotch(mem, ERR_UNDERFLOW, _("realloc: underflow detected; mh_nbytes out of range"), file,
		       line);
	{
		int i;
		for (i = 0, z = p->mh_magic8; i < MAGIC8_NUMBYTES; i++)
			if (*z++ != MAGIC1)
				xbotch(mem, ERR_UNDERFLOW, _("realloc: underflow detected; magic8 corrupted"), file,
				       line);
	}
...
#endif /* USE_MREMAP */
	{
		if ((m = internal_malloc(n, file, line, MALLOC_INTERNAL | MALLOC_NOTRACE | MALLOC_NOREG)) ==
		    0)
			return 0;
...
static PTR_T internal_memalign(alignment, size, file, line, flags)
size_t alignment;
size_t size;
const char *file;
int line, flags;
...
int posix_memalign(memptr, alignment, size) void **memptr;
...
static PTR_T internal_valloc(size, file, line, flags)
size_t size;
const char *file;
int line, flags;
...
#ifndef NO_CALLOC
static PTR_T internal_calloc(n, s, file, line, flags)
size_t n, s;
const char *file;
int line, flags;
{
...
static void internal_cfree(p, file, line, flags) PTR_T p;
const char *file;
int line, flags;
...
PTR_T
sh_realloc(ptr, size, file, line) PTR_T ptr;
size_t size;
const char *file;
int line;
{ return internal_realloc(ptr, size, file, line, MALLOC_WRAPPER); }

void sh_free(mem, file, line) PTR_T mem;
const char *file;
int line;
...
void sh_cfree(mem, file, line) PTR_T mem;
const char *file;
int line;
...
PTR_T
realloc(mem, nbytes) PTR_T mem;
size_t nbytes;
{ return internal_realloc(mem, nbytes, (char *)NULL, 0, 0); }

void free(mem) PTR_T mem;
