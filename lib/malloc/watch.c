...
static void watch_warn(addr, file, line, type, data) PTR_T addr;
const char *file;
int line, type;
unsigned long data;
{
...
void _malloc_ckwatch(addr, file, line, type, data) PTR_T addr;
const char *file;
int line, type;
unsigned long data;
{
...
/* Remove a watchpoint set on ADDR.  If ADDR is NULL, remove all
   ...
   not being watched. */
PTR_T
malloc_unwatch(addr) PTR_T addr;
{
#ifdef MALLOC_WATCH
	register int i;
