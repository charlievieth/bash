...
static mr_table_t *find_entry(mem, flags)
PTR_T mem;
int flags;
{
	unsigned int bucket;
...
static void _register_dump_table(fp) FILE *fp;
{
	register int i;
	mr_table_t entry;
	...
	for (i = 0; i < REG_TABLE_SIZE; i++) {
		...
		if (entry.mem)
			fprintf(fp, "%s[%d] %p:%zu:%s:%s:%s:%d:%d:%d\n", (i == table_bucket_index) ? "*" : "",
			        i, entry.mem, entry.size, _entry_flags(entry.flags),
			        entry.func ? entry.func : "unknown", entry.file ? entry.file : "unknown",
			        entry.line, entry.nalloc, entry.nfree);
	}
}
...
static ma_table_t *find_location_entry(file, line) const char *file;
int line;
{
...
void mlocation_register_alloc(file, line) const char *file;
int line;
{
	ma_table_t *lentry;
	const char *nfile;
...
static void _location_dump_table(fp) FILE *fp;
{
	register ma_table_t *tp, *endp;

	endp = mlocation_table + location_table_count;
	for (tp = mlocation_table; tp < endp; tp++)
		fprintf(fp, "%s:%d\t%d\n", tp->file ? tp->file : "unknown", tp->line ? tp->line : 0,
		        tp->nalloc);
}

void mlocation_dump_table() {
	_location_dump_table(stderr);
}

#define LOCROOT "/var/tmp/maltrace/locations."

void mlocation_write_table() {
	FILE *fp;
