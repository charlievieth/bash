...
struct bucket_stats malloc_bucket_stats(size)
int size;
{
...
FILE *fp;
{
	...
	fprintf(fp,
	        "Memory allocation statistics: %s\n    size\tfree\tin use\ttotal\tmorecore lesscore "
	        "split\tcoalesce\n",
	        s ? s : "");
	   ...
	   allocate and munmap to free (munmap shows up as lesscore). */
		...
		if (v.nmal > 0)
			fprintf(fp, "%8lu\t%4d\t%6d\t%5d%8d\t%8d %5d %8d\n", (unsigned long)v.blocksize,
			        v.nfree, v.nused, v.nmal, v.nmorecore, v.nlesscore, v.nsplit, v.ncoalesce);
...
FILE *_imalloc_fopen(s, fn, def, defbuf, defsiz)
char *s;
char *fn;
char *def;
char *defbuf;
