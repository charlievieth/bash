...
char **strvec_mresize(array, nsize)
char **array;
int nsize;
...
void strvec_flush(array) char **array;
{
	register int i;
...
void strvec_dispose(array) char **array;
{
	if (array == 0)
		return;
...
void strvec_sort(array, posix) char **array;
int posix;
{
	if (posix)
