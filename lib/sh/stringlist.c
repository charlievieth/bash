...
STRINGLIST *strlist_resize(sl, n)
STRINGLIST *sl;
int n;
{
	register int i;
...
void strlist_flush(sl) STRINGLIST *sl;
{
	if (sl == 0 || sl->list == 0)
		return;
...
void strlist_dispose(sl) STRINGLIST *sl;
{
	if (sl == 0)
		return;
	if (sl->list)
...
STRINGLIST *strlist_merge(m1, m2)
STRINGLIST *m1, *m2;
{
	STRINGLIST *sl;
	int i, n, l1, l2;
...
STRINGLIST *strlist_append(m1, m2)
STRINGLIST *m1, *m2;
{
...
STRINGLIST *strlist_prefix_suffix(sl, prefix, suffix)
STRINGLIST *sl;
char *prefix, *suffix;
{
...
void strlist_print(sl, prefix) STRINGLIST *sl;
char *prefix;
{
	register int i;
...
void strlist_sort(sl) STRINGLIST *sl;
{
...
STRINGLIST *strlist_from_word_list(list, alloc, starting_index, ip)
WORD_LIST *list;
...
WORD_LIST *strlist_to_word_list(sl, alloc, starting_index)
STRINGLIST *sl;
