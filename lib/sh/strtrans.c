	...
	*r++ = '\'';
	...
	for (s = str; c = *s; s++) {
		...
		default:
			...
			if ((b == 0 && ((clen = mbrtowc(&wc, s, MB_CUR_MAX, 0)) < 0 || MB_INVALIDCH(clen) ||
			                iswprint(wc) == 0)) ||
			    (b == 1 && ISPRINT(c) == 0))
#else
			if (ISPRINT(c) == 0)
...
#if defined(HANDLE_MULTIBYTE)
int ansic_wshouldquote(string) const char *string;
{
	const wchar_t *wcs;
	wchar_t wcc;
	wchar_t *wcstr = NULL;
...
int ansic_shouldquote(string) const char *string;
{
	const char *s;
	unsigned char c;
