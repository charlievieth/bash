...
static int _path_readlink(path, buf, bufsiz)
char *path;
char *buf;
int bufsiz;
{
...
int flags;
{
  ...
  if (linklen >= PATH_MAX)
	...
	qbase =
	    (ISALPHA((unsigned char)workpath[0]) && workpath[1] == ':') ? workpath + 3 : workpath + 1;
#else
	qbase = workpath + 1;
#endif
...
#if defined(__CYGWIN__)
				qbase = (ISALPHA((unsigned char)workpath[0]) && workpath[1] == ':') ? workpath + 3 :
				                                                                      workpath + 1;
#else
				qbase = workpath + 1;
#endif
...
char *sh_realpath(pathname, resolved) const char *pathname;
char *resolved;
{
	char *tdir, *wd;
