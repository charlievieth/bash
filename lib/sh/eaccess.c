...
static int path_is_devfd(path) const char *path;
{
...
int sh_stat(path, finfo) const char *path;
struct stat *finfo;
{
	static char *pbuf = 0;
...
static int sh_stataccess(path, mode) const char *path;
int mode;
{
	struct stat st;
...
int sh_eaccess(path, mode) const char *path;
int mode;
{
	int ret;
