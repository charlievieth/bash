...
void shtimer_dispose(sh_timer *t) {
	free(t);
}

/* We keep the timer as an offset into the future from the time it's set. */
void shtimer_set(sh_timer *t, time_t sec, long usec) {
	struct timeval now;
...
void shtimer_cleanup(sh_timer *t) {
	shtimer_unset(t);
}
