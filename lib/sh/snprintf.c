...
#define GETSIGNED(p)             \
	(((p)->flags & PF_LONGINT) ? \
	     GETARG(long) :          \
	     (((p)->flags & PF_SHORTINT) ? (long)(short)GETARG(int) : (long)GETARG(int)))
...
static inline int isnan_ld(LONGDOUBLE x) {
	return x != x;
}
#define isnan(x)                                     \
...
static inline int isinf_ld(LONGDOUBLE x) {
	return !isnan(x) && isnan(x - x);
}
#define isinf(x)                                     \
...
#define ROUND(d, p) (d < 0.) ? d - pow_10(-(p)->precision) * 0.5 : d + pow_10(-(p)->precision) * 0.5
 ...
 */
static double integral(real, ip)
double real;
double *ip;
{
	int j;
 ...
 */
static char *numtoa(number, base, precision, fract)
double number;
int base, precision;
char **fract;
{
 ...
 */
static void number(p, d, base) struct DATA *p;
unsigned long d;
int base;
{
	char *tmp, *t;
	...
	flags = 0;
	flags = (*p->pf == 'x' || *p->pf == 'X' || *p->pf == 'o' || *p->pf == 'u' || *p->pf == 'U') ?
	            FL_UNSIGNED :
	            0;
	if (*p->pf == 'X')
		flags |= FL_HEXUPPER;
 ...
 */
static void lnumber(p, d, base) struct DATA *p;
...
static void pointer(p, d) struct DATA *p;
unsigned long d;
{
	char *tmp;
...
/* %s strings */
static void strings(p, tmp) struct DATA *p;
char *tmp;
{
	size_t len;
...
static void wstrings(p, tmp) struct DATA *p;
wchar_t *tmp;
{
	size_t len;
	mbstate_t mbs;
...
static void wchars(p, wc) struct DATA *p;
wint_t wc;
{
	char *lbuf, *l;
	mbstate_t mbs;
...
static void floating(p, d) struct DATA *p;
double d;
{
	char *tmp, *tmp2, *t;
	int i;
...
static void exponent(p, d) struct DATA *p;
double d;
{
	char *tmp, *tmp2;
	int j, i;
	...
	/* XXX - should this be d>0. && (p->flags & PF_PLUS) ? */
	     ...
	     ((d > 0. && p->justify == RIGHT) ? 1:0) -
	           ...
	           ((d > 0. && (p->flags & PF_PLUS)) ? 1 : 0) -
#endif
	           (p->precision != 0 || (p->flags & PF_ALTFORM)) - ((p->flags & PF_SPACE) ? 1 : 0) -
	           p->precision - 5;
...
static void init_conv_flag(p) struct DATA *p;
{
...
static void init_data(p, string, length, format, mode) struct DATA *p;
char *string;
size_t length;
const char *format;
int mode;
 ...
 */
static void ldfallback(data, fs, fe, ld) struct DATA *data;
const char *fs, *fe;
long double ld;
{
	register char *x;
...
static void dfallback(data, fs, fe, d) struct DATA *data;
const char *fs, *fe;
double d;
{
	register char *x;
...
static void *xrealloc(pointer, bytes) void *pointer;
size_t bytes;
{
	void *ret;
