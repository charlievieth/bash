...
static void without_interrupts(function, arg1, arg2) VFunction *function;
char *arg1, *arg2;
{ (*function)(arg1, arg2); }

/* Start the beginning of a region. */
void begin_unwind_frame(tag) char *tag;
...
void run_unwind_frame(tag) char *tag;
{
	if (unwind_protect_list)
...
void add_unwind_protect(cleanup, arg) Function *cleanup;
char *arg;
...
void remove_unwind_protect() {
	if (unwind_protect_list)
		without_interrupts(remove_unwind_protect_internal, (char *)NULL, (char *)NULL);
}

/* Run the list of cleanup functions in unwind_protect_list. */
void run_unwind_protects() {
	if (unwind_protect_list)
		without_interrupts(run_unwind_protects_internal, (char *)NULL, (char *)NULL);
}
...
void clear_unwind_protect_list(flags) int flags;
{
	...
	if (unwind_protect_list) {
		...
		without_interrupts(clear_unwind_protects_internal, flag, (char *)NULL);
	}
}

int have_unwind_protects() {
	return (unwind_protect_list != 0);
}

int unwind_protect_tag_on_stack(tag) const char *tag;
{
	UNWIND_ELT *elt;
...
static void add_unwind_protect_internal(cleanup, arg) Function *cleanup;
char *arg;
{
	UNWIND_ELT *elt;
...
static void remove_unwind_protect_internal(ignore1, ignore2) char *ignore1, *ignore2;
{
	UNWIND_ELT *elt;
...
static void run_unwind_protects_internal(ignore1, ignore2) char *ignore1, *ignore2;
{ unwind_frame_run_internal((char *)NULL, (char *)NULL); }

static void clear_unwind_protects_internal(flag, ignore) char *flag, *ignore;
{
	if (flag) {
...
static inline void restore_variable(sv) SAVED_VAR *sv;
...
static void unwind_protect_mem_internal(var, psize) char *var;
char *psize;
{
	int size, allocated;
	UNWIND_ELT *elt;
...
/* Save the value of a variable so it will be restored when unwind-protects
   ...
   bytes of VAR.  */
void unwind_protect_mem(var, size) char *var;
int size;
...
void print_unwind_protect_tags() {
	UNWIND_ELT *elt;
