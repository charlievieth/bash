...
static void
    print_formatted_time PARAMS((FILE *, char *, time_t, int, time_t, int, time_t, int, int));
...
static int execute_builtin_or_function
    PARAMS((WORD_LIST *, sh_builtin_func_t *, SHELL_VAR *, REDIRECT *, struct fd_bitmap *, int));
static void execute_subshell_builtin_or_function PARAMS((WORD_LIST *,
                                                         REDIRECT *,
                                                         sh_builtin_func_t *,
                                                         SHELL_VAR *,
                                                         int,
...
static int execute_disk_command
    PARAMS((WORD_LIST *, REDIRECT *, char *, int, int, int, struct fd_bitmap *, int));
...
/* Functions to allocate and deallocate the structures used to pass
   ...
   to close. */
struct fd_bitmap *new_fd_bitmap(size)
int size;
{
...
void dispose_fd_bitmap(fdbp) struct fd_bitmap *fdbp;
{
	FREE(fdbp->bitmap);
	free(fdbp);
}

void close_fd_bitmap(fdbp) struct fd_bitmap *fdbp;
{
	register int i;
...
int executing_line_number() {
	if (executing && showing_function_line == 0 &&
	    (variable_context == 0 || interactive_shell == 0) && currently_executing_command) {
#if defined(COND_COMMAND)
...
static int shell_control_structure(type) enum command_type type;
{
	switch (type) {
#if defined(ARITH_FOR_COMMAND)
	case cm_arith_for:
...
/* A function to use to unwind_protect the redirection undo list
   for loops. */
static void cleanup_redirects(list) REDIRECT *list;
{
...
static int restore_signal_mask(set)
sigset_t *set;
  ...
  if (running_trap == 0)
	...
	if (command->type == cm_subshell ||
	   ...
	   control and call execute_command () on the command again. */
		...
		if (user_subshell && signal_is_trapped(ERROR_TRAP) && signal_in_progress(DEBUG_TRAP) == 0 &&
		    running_trap == 0) {
		...
		if (paren_pid == 0) {
			...
			s = user_subshell == 0 && command->type == cm_group && pipe_in == NO_PIPE &&
			    pipe_out == NO_PIPE && asynchronous;
			...
			if (asynchronous == 0) {
				...
				if (user_subshell && was_error_trap && ignore_return == 0 && invert == 0 &&
				    exec_result != EXECUTION_SUCCESS) {
					save_line_number = line_number;
	...
	case cm_simple: {
	   ...
	   defer that until the command builtin itself returns failure. */
		...
		if (was_error_trap && ignore_return == 0 && invert == 0 && pipe_in == NO_PIPE &&
		    pipe_out == NO_PIPE && (command->value.Simple->flags & CMD_COMMAND_BUILTIN) == 0 &&
		    exec_result != EXECUTION_SUCCESS) {
			last_command_exit_value = exec_result;
			line_number = line_number_for_err_trap;
			run_error_trap();
		...
		if (ignore_return == 0 && invert == 0 &&
		    ((posixly_correct && interactive == 0 && special_builtin_failed) ||
		     (exit_immediately_on_error && pipe_in == NO_PIPE && pipe_out == NO_PIPE &&
		      exec_result != EXECUTION_SUCCESS))) {
			last_command_exit_value = exec_result;
			run_pending_traps();
...
/* Interpret the format string FORMAT, interpolating the following escape
   ...
   resectively. */
static void print_formatted_time(fp, format, rs, rsf, us, usf, ss, ssf, cpu) FILE *fp;
char *format;
time_t rs;
int rsf;
time_t us;
...
static int time_command(command, asynchronous, pipe_in, pipe_out, fds_to_close)
COMMAND *command;
...
struct fd_bitmap *fds_to_close;
{
	...
	nullcmd = (command == 0) || (command->type == cm_simple && command->value.Simple->words == 0 &&
...
static int execute_in_subshell(command, asynchronous, pipe_in, pipe_out, fds_to_close)
COMMAND *command;
int asynchronous;
int pipe_in, pipe_out;
struct fd_bitmap *fds_to_close;
{
	...
	if ((user_subshell || user_coproc) && (tcom->type == cm_simple || tcom->type == cm_subshell) &&
	    ((tcom->flags & CMD_TIME_PIPELINE) == 0) && ((tcom->flags & CMD_INVERT_RETURN) == 0)) {
	...
	if (result == EXITPROG || result == EXITBLTIN)
		...
		return_code = (last_command_exit_value == EXECUTION_SUCCESS) ? EXECUTION_FAILURE :
		                                                               last_command_exit_value;
	...
	if (invert)
		return_code = (return_code == EXECUTION_SUCCESS) ? EXECUTION_FAILURE : EXECUTION_SUCCESS;
...
static struct cpelement *cpe_alloc(cp)
Coproc *cp;
{
...
static struct cpelement *cpl_add(cp)
Coproc *cp;
{
	struct cpelement *cpe;
...
static struct cpelement *cpl_delete(pid)
pid_t pid;
{
...
static void cpl_fdchk(fd) int fd;
{
	struct cpelement *cpe;
...
static struct cpelement *cpl_search(pid)
pid_t pid;
{
	struct cpelement *cpe;
...
static struct cpelement *cpl_searchbyname(name) const char *name;
{
	struct cpelement *cp;
...
struct coproc *getcoprocbypid(pid)
pid_t pid;
{
#if MULTIPLE_COPROCS
	struct cpelement *p;
...
struct coproc *getcoprocbyname(name) const char *name;
{
#if MULTIPLE_COPROCS
	struct cpelement *p;
...
void coproc_init(cp) struct coproc *cp;
{
	cp->c_name = 0;
	cp->c_pid = NO_PID;
	cp->c_rfd = cp->c_wfd = -1;
...
struct coproc *coproc_alloc(name, pid)
char *name;
pid_t pid;
{
	struct coproc *cp;
...
static void coproc_free(cp) struct coproc *cp;
{ free(cp); }
...
void coproc_wclose(cp, fd) struct coproc *cp;
int fd;
{
...
void coproc_checkfd(cp, fd) struct coproc *cp;
int fd;
{
	int update;
...
void coproc_fdclose(cp, fd) struct coproc *cp;
int fd;
{
	coproc_rclose(cp, fd);
	coproc_wclose(cp, fd);
...
void coproc_fdsave(cp) struct coproc *cp;
{
	cp->c_rsave = cp->c_rfd;
	cp->c_wsave = cp->c_wfd;
}

void coproc_fdrestore(cp) struct coproc *cp;
{
	cp->c_rfd = cp->c_rsave;
	cp->c_wfd = cp->c_wsave;
}

static void coproc_setstatus(cp, status) struct coproc *cp;
int status;
{
	cp->c_lock = 4;
	cp->c_status = status;
...
void coproc_pidchk(pid, status) pid_t pid;
int status;
{
	struct coproc *cp;
...
void coproc_setvars(cp) struct coproc *cp;
{
	SHELL_VAR *v;
	char *namevar, *t;
	size_t l;
...
void coproc_unsetvars(cp) struct coproc *cp;
{
	int l;
	char *namevar;
...
static int execute_coproc(command, pipe_in, pipe_out, fds_to_close)
COMMAND *command;
int pipe_in, pipe_out;
struct fd_bitmap *fds_to_close;
{
	...
	if (sh_coproc.c_pid != NO_PID && (sh_coproc.c_rfd >= 0 || sh_coproc.c_wfd >= 0))
		internal_warning(_("execute_coproc: coproc [%d:%s] still exists"), sh_coproc.c_pid,
		                 sh_coproc.c_name);
...
static void restore_stdin(s) int s;
{
	if (s == -1)
		close(0);
	else {
...
static void lastpipe_cleanup(s) int s;
{ set_jobs_list_frozen(s); }

static int execute_pipeline(command, asynchronous, pipe_in, pipe_out, fds_to_close)
COMMAND *command;
int asynchronous, pipe_in, pipe_out;
struct fd_bitmap *fds_to_close;
{
	...
	while (cmd && cmd->type == cm_connection && cmd->value.Connection &&
	   ...
	   use fds_to_close->size. */
		new_bitmap_size = (fildes[0] < fds_to_close->size) ? fds_to_close->size : fildes[0] + 8;
	...
	if (lastpipe_flag) {
		...
		if (INVALID_JOB(lastpipe_jid) == 0) {
			append_process(savestring(the_printed_command_except_trap), dollar_dollar_pid,
...
static int execute_connection(command, asynchronous, pipe_in, pipe_out, fds_to_close)
COMMAND *command;
int asynchronous, pipe_in, pipe_out;
struct fd_bitmap *fds_to_close;
{
	...
	case '&':
	   ...
	   asynchronous command is forced to /dev/null. */
		...
		if (second) {
			...
			exec_result =
			    execute_command_internal(second, asynchronous, pipe_in, pipe_out, fds_to_close);
		}
...
#else
		execute_command_internal(command->value.Connection->first, asynchronous, pipe_in, pipe_out,
		                         fds_to_close);
#endif
...
#else
		exec_result = execute_command_internal(command->value.Connection->first, 0, NO_PIPE,
	...
	default:
		command_error("execute_connection", CMDERR_BADCONN, command->value.Connection->connector,
		              0);
...
#define NUMBER_LEN(s) \
	((s < 10) ? 1 : ((s < 100) ? 2 : ((s < 1000) ? 3 : ((s < 10000) ? 4 : ((s < 100000) ? 5 : 6)))))
...
static int print_index_and_element(len, ind, list)
int len, ind;
WORD_LIST *list;
{
...
static void indent(from, to) int from, to;
{
	while (from < to) {
...
static void print_select_list(list, list_len, max_elem_len, indices_len) WORD_LIST *list;
...
static char *select_query(list, list_len, prompt, print_menu)
WORD_LIST *list;
int list_len;
char *prompt;
int print_menu;
...
/* Execute a CASE command.  The syntax is: CASE word_desc IN pattern_list ESAC.
   ...
   execute. */
static int execute_case_command(case_command)
...
/* The WHILE command.  Syntax: WHILE test DO action; DONE.
   ...
   EXECUTION_SUCCESS. */
static int execute_while_command(while_command)
...
static int execute_while_or_until(while_command, type)
WHILE_COM *while_command;
int type;
{
...
static int execute_if_command(if_command)
IF_COM *if_command;
{
...
#if defined(DPAREN_ARITHMETIC)
static int execute_arith_command(arith_command)
...
static int execute_cond_node(cond)
COND_COM *cond;
{
  ...
  char *t1, *t2;
	...
	if (cond->type == COND_EXPR)
...
#if defined(ARRAY_VARS)
		varflag =
		    (varop && valid_array_reference(cond->left->op->word, VA_NOEXPAND)) ? TEST_ARRAYEXP : 0;
#else
		varflag = 0;
#endif
		...
		op = cond->op->word;
		mode = 0;
		patmatch = (((op[1] == '=') && (op[2] == '\0') && (op[0] == '!' || op[0] == '=')) ||
		            (op[0] == '=' && op[1] == '\0'));
#if defined(COND_REGEXP)
		rmatch = (op[0] == '=' && op[1] == '~' && op[2] == '\0');
#endif
		arith = STREQ(op, "-eq") || STREQ(op, "-ne") || STREQ(op, "-lt") || STREQ(op, "-le") ||
		        STREQ(op, "-gt") || STREQ(op, "-ge");
...
static void bind_lastarg(arg) char *arg;
{
	SHELL_VAR *var;
...
static int execute_null_command(redirects, pipe_in, pipe_out, async)
REDIRECT *redirects;
int pipe_in, pipe_out, async;
{
	...
	for (forcefork = 0, rd = redirects; rd; rd = rd->next) {
		...
		forcefork += (rd->redirector.dest == 0 || fd_is_bash_input(rd->redirector.dest)) &&
		             (INPUT_REDIRECT(rd->instruction) || TRANSLATE_REDIRECT(rd->instruction) ||
...
/* Set W_ARRAYREF on words that are valid array references to a builtin that
   ...
   time. */
static void fix_arrayref_words(words) WORD_LIST *words;
{
	WORD_LIST *w, *wcmd;
	struct builtin *b;
...
static WORD_LIST *check_command_builtin(words, typep)
WORD_LIST *words;
int *typep;
{
	int type;
...
static int is_dirname(pathname)
char *pathname;
{
	char *temp;
	int ret;
...
struct fd_bitmap *fds_to_close;
{
  ...
  if (signal_in_progress (DEBUG_TRAP) == 0 && (this_command_name == 0 || (STREQ (this_command_name, "trap") == 0)))
	...
	first_word_quoted = simple_command->words ? (simple_command->words->word->flags & W_QUOTED) : 0;
	...
	if (dofork && pipe_in == NO_PIPE && pipe_out == NO_PIPE && simple_command->words &&
	    simple_command->words->word && simple_command->words->word->word &&
	...
	if (words == 0) {
		this_command_name = 0;
		result = execute_null_command(simple_command->redirects, pipe_in, pipe_out,
	...
	/* One other possibililty.  The user may want to resume an existing job.
	   ...
	   job. */
	if (job_control && already_forked == 0 && async == 0 && !first_word_quoted && !words->next &&
	    words->word->word[0] && !simple_command->redirects && pipe_in == NO_PIPE &&
...
execute_from_filesystem:
	if (command_line == 0)
		command_line =
		    savestring(the_printed_command_except_trap ? the_printed_command_except_trap : "");
	...
	if (already_forked == 0 && (cmdflags & CMD_NO_FORK) && fifos_pending() > 0)
		cmdflags &= ~CMD_NO_FORK;
#endif
	result = execute_disk_command(words, simple_command->redirects, command_line, pipe_in, pipe_out,
...
int flags, subshell;
{
	...
	if (subshell == 0 && builtin == eval_builtin) {
		...
		if (sourcenest_max > 0 && sourcenest >= sourcenest_max) {
			internal_error(_("%s: maximum source nesting level exceeded (%d)"), this_command_name,
			               sourcenest);
			sourcenest = 0;
			jump_to_top_level(DISCARD);
		}
...
static void maybe_restore_getopt_state(gs) sh_getopt_state_t *gs;
{
...
#if defined(ARRAY_VARS)
void restore_funcarray_state(fa) struct func_array_state *fa;
{
	SHELL_VAR *nfv;
	ARRAY *funcname_a;
...
static int execute_function(var, words, flags, fds_to_close, async, subshell)
SHELL_VAR *var;
WORD_LIST *words;
int flags;
struct fd_bitmap *fds_to_close;
int async, subshell;
{
	...
	if (return_trap &&
	    (signal_in_progress(DEBUG_TRAP) || ((trace_p(var) == 0) && function_trace_mode == 0))) {
		if (subshell == 0) {
...
int execute_shell_function(var, words)
SHELL_VAR *var;
WORD_LIST *words;
{
	int ret;
...
static void execute_subshell_builtin_or_function(
    words, redirects, builtin, var, pipe_in, pipe_out, async, fds_to_close, flags) WORD_LIST *words;
REDIRECT *redirects;
sh_builtin_func_t *builtin;
SHELL_VAR *var;
int pipe_in, pipe_out, async;
	   ...
	   so we don't go back up to main(). */
		...
		if (result == EXITPROG || result == EXITBLTIN)
			...
			/* XXX - experimental */
				...
				command_line = savestring(
				    the_printed_command_except_trap ? the_printed_command_except_trap : "");
				r = execute_disk_command(words, (REDIRECT *)0, command_line, -1, -1, async,
				                         (struct fd_bitmap *)0, flags | CMD_NO_FORK);
			}
			subshell_exit(r);
		}
	} else {
...
static int execute_builtin_or_function(words, builtin, var, redirects, fds_to_close, flags)
WORD_LIST *words;
sh_builtin_func_t *builtin;
SHELL_VAR *var;
REDIRECT *redirects;
...
static int execute_disk_command(
    words, redirects, command_line, pipe_in, pipe_out, async, fds_to_close, cmdflags)
WORD_LIST *words;
REDIRECT *redirects;
char *command_line;
int pipe_in, pipe_out, async;
...
int cmdflags;
{
	...
	if (restricted && mbschr(pathname, '/')) {
		internal_error(_("%s: restricted: cannot specify `/' in command names"), pathname);
...
#if !defined(MSDOS)
#define STRINGCHAR(ind) (ind < sample_len && !whitespace(sample[ind]) && sample[ind] != '\n')
...
static char *getinterp(sample, sample_len, endp)
char *sample;
int sample_len, *endp;
{
	register int i;
...
static int execute_shell_script(sample, sample_len, command, args, env)
char *sample;
int sample_len;
char *command;
char **args, **env;
...
static void do_piping(pipe_in, pipe_out) int pipe_in, pipe_out;
{
	if (pipe_in != NO_PIPE) {
		if (dup2(pipe_in, 0) < 0)
			dup_error(pipe_in, 0);
