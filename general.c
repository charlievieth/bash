...
static struct {
	int *posix_mode_var;
} posix_vars[] = {&interactive_comments, &source_uses_path,  &expaliases_flag,
                  &inherit_errexit,      &print_shift_error, 0};
...
void set_posix_options(bitmap) const char *bitmap;
{
	register int i;
...
void print_rlimtype(n, addnl) RLIMTYPE n;
int addnl;
{
...
int all_digits(string) const char *string;
{
...
/* Return non-zero if the characters pointed to by STRING constitute a
   ...
   not null. */
int legal_number(string, result) const char *string;
intmax_t *result;
{
	intmax_t value;
	char *ep;
...
/* Return 1 if this token is a legal shell `identifier'; that is, it consists
   ...
   digit. */
int legal_identifier(name) const char *name;
{
	register const char *s;
	unsigned char c;
...
int valid_nameref_value(name, flags) const char *name;
int flags;
{
	if (name == 0 || *name == 0)
		return 0;
...
int check_selfref(name, value, flags) const char *name;
char *value;
int flags;
{
	char *t;
...
int check_identifier(word, check_word)
WORD_DESC *word;
int check_word;
{
...
/* Return 1 if STRING is a function name that the shell will import from
   ...
   used yet. */
int importable_function_name(string, len) const char *string;
size_t len;
{
...
int exportable_function_name(string) const char *string;
{
...
int legal_alias_name(string, flags) const char *string;
int flags;
{
	register const char *s;
...
/* Returns non-zero if STRING is an assignment statement.  The returned value
   ...
   statement. */
int assignment(string, flags) const char *string;
int flags;
{
...
int line_isblank(line) const char *line;
{
	register int i;
...
int same_file(path1, path2, stp1, stp2) const char *path1, *path2;
...
int move_to_high_fd(fd, check_new, maxfd)
...
int check_binary_file(sample, sample_len) const char *sample;
int sample_len;
{
	register int i;
	int nline;
	...
	if (sample_len >= 4 && sample[0] == 0x7f && sample[1] == 'E' && sample[2] == 'L' &&
	    sample[3] == 'F')
		return 1;
...
int file_exists(fn) const char *fn;
{
	struct stat sb;
...
int file_isdir(fn) const char *fn;
{
	struct stat sb;
...
int file_iswdir(fn) const char *fn;
{ return (file_isdir(fn) && sh_eaccess(fn, W_OK) == 0); }

/* Return 1 if STRING is "." or "..", optionally followed by a directory
   separator */
int path_dot_or_dotdot(string) const char *string;
{
...
/* Return 1 if STRING is an absolute program name; it is absolute if it
   ...
   up through $PATH. */
int absolute_program(string) const char *string;
...
char *polite_directory_format(name)
char *name;
{
	char *home;
	int l;
...
char *trim_pathname(name, maxlen)
char *name;
int maxlen;
{
	int nlen, ndirs;
...
char *printable_filename(fn, flags)
char *fn;
int flags;
{
	char *newf;
...
char *extract_colon_unit(string, p_index)
char *string;
int *p_index;
{
	int i, start, len;
...
/* If tilde_expand hasn't been able to expand the text, perhaps it
   ...
   directory stack. */
static char *bash_special_tilde_expansions(text)
char *text;
{
	char *result;
...
static int unquoted_tilde_word(s) const char *s;
{
	const char *r;
...
/* Tilde-expand S by running it through the tilde expansion library.
   ...
   so `=~' is not valid. */
char *bash_tilde_expand(s, assign_p) const char *s;
int assign_p;
{
	int r;
	char *ret;
	...
	tilde_additional_prefixes =
	    assign_p == 0 ? (char **)0 : (assign_p == 2 ? bash_tilde_prefixes2 : bash_tilde_prefixes);
	if (assign_p == 2)
