...
static const struct {
	...
	int *int_value;
	char **char_value;
} long_args[] = {{"debug", Int, &debugging, (char **)0x0},
#if defined(DEBUGGER)
...
#ifdef __CYGWIN__
static void _cygwin32_check_tmp() {
	struct stat sb;
	...
	if (locally_skip_execution == 0 && running_setuid == 0) {
	   ...
	   we get better error messages when trying to open script files. */
		if (shell_script_filename) {
			t = dollar_vars[0];
			dollar_vars[0] =
			    exec_argv0 ? savestring(exec_argv0) : savestring(shell_script_filename);
		}
		run_startup_files();
		if (shell_script_filename) {
			free(dollar_vars[0]);
...
static int parse_long_options(argv, arg_start, arg_end)
char **argv;
int arg_start, arg_end;
{
	...
	arg_index = arg_start;
	while ((arg_index != arg_end) && (arg_string = argv[arg_index]) && (*arg_string == '-')) {
		longarg = 0;
...
static int parse_shell_options(argv, arg_start, arg_end)
char **argv;
int arg_start, arg_end;
{
	int arg_index;
	...
	while (arg_index != arg_end && (arg_string = argv[arg_index]) &&
	   ...
	   getopt(3) meaning. */
		if (arg_string[0] == '-' &&
		    (arg_string[1] == '\0' || (arg_string[1] == '-' && arg_string[2] == '\0')))
			return (next_arg);
...
static void execute_env_file(env_file) char *env_file;
{
	char *fn;
...
static void run_startup_files() {
	...
	if (interactive_shell == 0 && no_rc == 0 && login_shell == 0 && act_like_sh == 0 &&
...
if (login_shell < 0 && posixly_correct == 0)
	...
	if (interactive_shell == 0 && !(su_shell && login_shell)) {
		if (posixly_correct == 0 && act_like_sh == 0 && privileged_mode == 0 && sourced_env++ == 0)
...
static int run_one_command(command)
char *command;
{
	int code;
...
static int bind_args(argv, arg_start, arg_end, start_index)
char **argv;
...
/* Close the current shell script input source and forget about it.  This is
   ...
   input (fd 0). */
void unset_bash_input(check_zero) int check_zero;
{
#if defined(BUFFERED_INPUT)
...
static void set_shell_name(argv0) char *argv0;
{
...
static void shell_initialize() {
	...
	/* Initialize the shell options.  Don't import the shell options
	   ...
	   setuid. */
#if defined(RESTRICTED_SHELL)
	initialize_shell_options(privileged_mode || restricted || should_be_restricted ||
	                         running_setuid);
...
static void show_shell_usage(fp, extra) FILE *fp;
int extra;
{
	int i;
	char *set_opts, *s, *t;
...
static void add_shopt_to_alist(opt, on_or_off) char *opt;
int on_or_off;
{
