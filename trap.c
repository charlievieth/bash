...
#define SPECIAL_TRAP(s) \
	((s) == EXIT_TRAP || (s) == DEBUG_TRAP || (s) == ERROR_TRAP || (s) == RETURN_TRAP)
...
void initialize_traps() {
	...
	trap_list[EXIT_TRAP] = trap_list[DEBUG_TRAP] = trap_list[ERROR_TRAP] = trap_list[RETURN_TRAP] =
	    (char *)NULL;
	sigmodes[EXIT_TRAP] = sigmodes[DEBUG_TRAP] = sigmodes[ERROR_TRAP] = sigmodes[RETURN_TRAP] =
	    SIG_INHERITED;
...
static char *trap_handler_string(sig)
int sig;
{
...
char *signal_name(sig)
int sig;
{
	char *ret;

	/* on cygwin32, signal_names[sig] could be null */
	ret = (sig >= BASH_NSIG || sig < 0 || signal_names[sig] == NULL) ? _("invalid signal number") :
	                                                                   signal_names[sig];
...
int decode_signal(string, flags)
char *string;
int flags;
{
	intmax_t sig;
	...
	if (STREQN(string, "SIGRTMIN+", 9) ||
		...
		if (legal_number(string + 9, &sig) && sig >= 0 && sig <= SIGRTMAX - SIGRTMIN)
			...
			return NO_SIG;
	           ...
	           ((flags & DSIG_NOCASE) && strncasecmp(string, "RTMIN+", 6) == 0)) {
	...
	if (running_trap > 0) {
	   ...
	   (running_trap == sig+1) */
		if (evalnest_max > 0 && evalnest > evalnest_max) {
			internal_error(_("trap handler: maximum trap handler level exceeded (%d)"),
			               evalnest_max);
			evalnest = 0;
	   ...
	   while (pending_traps[sig]--) instead of the if statement. */
		if (pending_traps[sig]) {
			...
			if (sig == SIGINT) {
			   ...
			   usually 0x0. */
				internal_warning(_("run_pending_traps: bad value in trap_list[%d]: %p"), sig,
				                 trap_list[sig]);
				if (trap_list[sig] == (char *)DEFAULT_SIG) {
					internal_warning(_("run_pending_traps: signal handler is SIG_DFL, resending %d "
					                   "(%s) to myself"),
					                 sig, signal_name(sig));
					kill(getpid(), sig);
				}
...
void set_trap_state(sig) int sig;
{
	catch_flag = 1;
	pending_traps[sig]++;
...
void set_sigchld_trap(command_string) char *command_string;
...
void maybe_set_sigchld_trap(command_string) char *command_string;
{
...
void queue_sigchld_trap(nchild) int nchild;
{
	if (nchild > 0) {
		catch_flag = 1;
...
static inline void trap_if_untrapped(sig, command) int sig;
char *command;
{
	if ((sigmodes[sig] & SIG_TRAPPED) == 0)
...
void set_debug_trap(command) char *command;
...
void maybe_set_debug_trap(command) char *command;
{ trap_if_untrapped(DEBUG_TRAP, command); }

void set_error_trap(command) char *command;
{ set_signal(ERROR_TRAP, command); }

void maybe_set_error_trap(command) char *command;
{ trap_if_untrapped(ERROR_TRAP, command); }

void set_return_trap(command) char *command;
{ set_signal(RETURN_TRAP, command); }

void maybe_set_return_trap(command) char *command;
{ trap_if_untrapped(RETURN_TRAP, command); }

#ifdef INCLUDE_UNUSED
void set_sigint_trap(command) char *command;
...
/* Return the correct handler for signal SIG according to the values in
   sigmodes[SIG]. */
SigHandler *trap_to_sighandler(sig)
int sig;
{
...
void set_signal(sig, string) int sig;
char *string;
{
	sigset_t set, oset;
...
static void free_trap_command(sig) int sig;
{
	if ((sigmodes[sig] & SIG_TRAPPED) && trap_list[sig] && (trap_list[sig] != (char *)IGNORE_SIG) &&
...
/* If SIG has a string assigned to it, get rid of it.  Then give it
   VALUE. */
static void change_signal(sig, value) int sig;
char *value;
{
...
void set_original_signal(sig, handler) int sig;
SigHandler *handler;
{
	if (sig > 0 && sig < NSIG && original_signals[sig] == (SigHandler *)IMPOSSIBLE_TRAP_HANDLER)
...
void ignore_signal(sig) int sig;
{
...
void run_trap_cleanup(sig) int sig;
{
...
static int _run_trap_internal(sig, tag)
int sig;
char *tag;
{
	char *trap_command, *old_trap;
...
int run_debug_trap() {
	...
	trap_exit_value = 0;
	if ((sigmodes[DEBUG_TRAP] & SIG_TRAPPED) && ((sigmodes[DEBUG_TRAP] & SIG_IGNORED) == 0) &&
...
void run_error_trap() {
	if ((sigmodes[ERROR_TRAP] & SIG_TRAPPED) && ((sigmodes[ERROR_TRAP] & SIG_IGNORED) == 0) &&
...
void run_return_trap() {
  ...
  if ((sigmodes[DEBUG_TRAP] & SIG_TRAPPED) && (sigmodes[DEBUG_TRAP] & SIG_INPROGRESS))
	...
	if ((sigmodes[RETURN_TRAP] & SIG_TRAPPED) && ((sigmodes[RETURN_TRAP] & SIG_IGNORED) == 0) &&
...
static void free_trap_string(sig) int sig;
{
	change_signal(sig, (char *)DEFAULT_SIG);
...
/* Reset the handler for SIG to the original value but leave the trap string
   in place. */
static void reset_signal(sig) int sig;
{
	set_signal_handler(sig, original_signals[sig]);
	sigmodes[sig] &= ~SIG_TRAPPED; /* XXX - SIG_INPROGRESS? */
}
...
static void restore_signal(sig) int sig;
{
	set_signal_handler(sig, original_signals[sig]);
	change_signal(sig, (char *)DEFAULT_SIG);
	sigmodes[sig] &= ~SIG_TRAPPED;
...
static void reset_or_restore_signal_handlers(reset) sh_resetsig_func_t *reset;
{
	register int i;
...
static void reinit_trap(sig) int sig;
{
	sigmodes[sig] |= SIG_TRAPPED;
...
int maybe_call_trap_handler(sig)
int sig;
{
...
int signal_is_trapped(sig)
int sig;
{ return (sigmodes[sig] & SIG_TRAPPED); }

int signal_is_pending(sig)
int sig;
{ return (pending_traps[sig]); }

int signal_is_special(sig)
int sig;
{ return (sigmodes[sig] & SIG_SPECIAL); }

int signal_is_ignored(sig)
int sig;
{ return (sigmodes[sig] & SIG_IGNORED); }

int signal_is_hard_ignored(sig)
int sig;
{ return (sigmodes[sig] & SIG_HARD_IGNORE); }

void set_signal_hard_ignored(sig) int sig;
{
	sigmodes[sig] |= SIG_HARD_IGNORE;
