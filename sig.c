...
sighandler termsig_sighandler(sig)
int sig;
{
...
#if defined(JOB_CONTROL)
	if (sig == SIGHUP &&
	    (interactive || (subshell_environment & (SUBSHELL_COMSUB | SUBSHELL_PROCSUB))))
		hangup_all_jobs();
...
static void kill_shell(sig) int sig;
{
	int i, core;
	sigset_t mask;
...
sighandler sigint_sighandler(sig)
int sig;
{
...
#if defined(SIGWINCH)
sighandler sigwinch_sighandler(sig)
int sig;
{
#if defined(MUST_REINSTALL_SIGHANDLERS)
...
sighandler sigterm_sighandler(sig)
int sig;
{
