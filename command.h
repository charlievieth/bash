...
#define WRITE_REDIRECT(ri)                                                      \
	(ri == r_output_direction || ri == r_input_output || ri == r_err_and_out || \
	 ri == r_appending_to || ri == r_append_err_and_out || ri == r_output_force)
...
#define W_NOSPLIT \
	(1 << 4) /* Do not perform word splitting on this word because ifs is empty string. */
#define W_NOGLOB (1 << 5) /* Do not perform globbing on this word. */
#define W_NOSPLIT2                                                                               \
	(1 << 6) /* Don't split word except for $@ expansion (using spaces) because context does not \
	            allow it. */
...
#define W_FORCELOCAL \
	(1 << 29) /* force assignments to be to local variables, non-fatal on assignment errors */
/* UNUSED		(1 << 30)	*/
