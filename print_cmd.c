...
void print_command(command) COMMAND *command;
{
...
static void make_command_string_internal(command) COMMAND *command;
{
	char s[3];
...
static void _print_word_list(list, separator, pfunc) WORD_LIST *list;
char *separator;
PFUNC *pfunc;
{
	WORD_LIST *w;
...
char *indirection_level_string() {
	...
	for (i = j = 0; ps4_firstc[0] && j < indirection_level && i < indirection_stringsiz - 1;
	     i += ps4_firstc_len, j++) {
		if (ps4_firstc_len == 1)
...
void xtrace_print_assignment(name, value, assign_list, xflags) char *name, *value;
...
void xtrace_print_word_list(list, xtflags) WORD_LIST *list;
int xtflags;
{
	WORD_LIST *w;
	char *t, *x;
...
void print_for_command_head(for_command) FOR_COM *for_command;
{
	cprintf("for %s in ", for_command->name->word);
	command_print_word_list(for_command->map_list, " ");
}

void xtrace_print_for_command_head(for_command) FOR_COM *for_command;
{
	CHECK_XTRACE_FP;
	fprintf(xtrace_fp, "%s", indirection_level_string());
...
#if defined(ARITH_FOR_COMMAND)
static void print_arith_for_command(arith_for_command) ARITH_FOR_COM *arith_for_command;
{
	cprintf("for ((");
...
#if defined(SELECT_COMMAND)
void print_select_command_head(select_command) SELECT_COM *select_command;
{
	cprintf("select %s in ", select_command->name->word);
	command_print_word_list(select_command->map_list, " ");
}

void xtrace_print_select_command_head(select_command) SELECT_COM *select_command;
{
	CHECK_XTRACE_FP;
	fprintf(xtrace_fp, "%s", indirection_level_string());
...
static void print_group_command(group_command) GROUP_COM *group_command;
{
...
void xtrace_print_case_command_head(case_command) CASE_COM *case_command;
{
	CHECK_XTRACE_FP;
	fprintf(xtrace_fp, "%s", indirection_level_string());
...
static void print_case_command(case_command) CASE_COM *case_command;
{
...
static void print_while_command(while_command) WHILE_COM *while_command;
{ print_until_or_while(while_command, "while"); }

static void print_until_command(while_command) WHILE_COM *while_command;
{ print_until_or_while(while_command, "until"); }

static void print_until_or_while(while_command, which) WHILE_COM *while_command;
char *which;
{
	cprintf("%s ", which);
	skip_this_indent++;
...
#if defined(COND_COMMAND)
static void print_cond_node(cond) COND_COM *cond;
{
...
void print_cond_command(cond) COND_COM *cond;
{
	cprintf("[[ ");
...
#ifdef DEBUG
void debug_print_word_list(s, list, sep) char *s;
WORD_LIST *list;
char *sep;
{
	WORD_LIST *w;
...
void debug_print_cond_command(cond) COND_COM *cond;
{
...
void xtrace_print_cond_term(type, invert, op, arg1, arg2) int type, invert;
WORD_DESC *op;
char *arg1, *arg2;
{
	CHECK_XTRACE_FP;
...
void print_simple_command(simple_command) SIMPLE_COM *simple_command;
{
...
static void print_heredocs(heredocs) REDIRECT *heredocs;
{
	REDIRECT *hdtail;
...
static void print_heredoc_bodies(heredocs) REDIRECT *heredocs;
{
	REDIRECT *hdtail;
...
static void print_redirection_list(redirects) REDIRECT *redirects;
{
...
static void print_heredoc_header(redirect) REDIRECT *redirect;
{
	int kill_leading;
	char *x;
...
static void print_heredoc_body(redirect) REDIRECT *redirect;
{
	/* Here doc body */
	cprintf("%s%s", redirect->redirectee.filename->word, redirect->here_doc_eof);
}

static void print_redirection(redirect) REDIRECT *redirect;
{
...
static void print_function_def(func) FUNCTION_DEF *func;
{
	...
	make_command_string_internal(cmdcopy->type == cm_group ? cmdcopy->value.Group->command :
	                                                         cmdcopy);
	PRINT_DEFERRED_HEREDOCS("");
...
static void newline(string) char *string;
{
	cprintf("\n");
	indent(indentation);
	if (string && *string)
...
#else
    cprintf(control, va_alist) const char *control;
va_dcl
#endif
{
...
/* Ensure that there is enough space to stuff LENGTH characters into
   THE_PRINTED_COMMAND. */
static void the_printed_command_resize(length) int length;
{
...
#else
    xprintf(format, va_alist) const char *format;
va_dcl
#endif
{
	va_list args;
...
static void xprintf(format, arg1, arg2, arg3, arg4, arg5) const char *format;
