...
static struct ignorevar histignore = {
    "HISTIGNORE", (struct ign *)0, 0, (char *)0, (sh_iv_item_func_t *)histignore_item_func,
};
...
int i;
{
	...
	else if (i > 1 && string[i - 1] == '{' && string[i - 2] == '$' && member('}', string + i + 1))
		return (1);
...
void bash_history_reinit(interact) int interact;
{
#if defined(BANG_HISTORY)
	history_expansion = (interact == 0) ? histexp_flag : HISTEXPAND_DEFAULT;
	history_expansion_inhibited =
	    (interact == 0) ? 1 - histexp_flag : 0; /* changed in bash_history_enable() */
	history_inhibit_expansion_function = bash_history_inhibit_expansion;
#endif
...
int bash_delete_history_range(first, last)
int first, last;
{
	register int i;
...
int maybe_append_history(filename)
char *filename;
{
...
static void re_edit(text) char *text;
{
...
static int history_expansion_p(line)
char *line;
{
	register char *s;
...
char *pre_process_line(line, print_changes, addit)
char *line;
...
static char *filter_comments(line)
char *line;
{
	char *p;
...
static void hc_erasedups(line) char *line;
{
	HIST_ENTRY *temp;
	int r;
...
/* Add LINE to the history list, handling possibly multi-line compound
   ...
   constructs. */
void maybe_add_history(line) char *line;
{
	int is_comment;
	...
	if (current_command_line_count > 1) {
		if (current_command_first_line_saved && ((parser_state & PST_HEREDOC) || literal_history ||
...
int check_add_history(line, force)
char *line;
int force;
{
...
void bash_syslog_history(line) const char *line;
{
...
void bash_add_history(line) char *line;
{
	...
	if (command_oriented_history && current_command_line_count > 1) {
	   ...
	   case if the other conditions are met. */
		if ((parser_state & PST_HEREDOC) && here_doc_first_line == 0 &&
		    line[strlen(line) - 1] == '\n')
			chars_to_add = "";
...
void setup_history_ignore(varname) char *varname;
...
static char *expand_histignore_pattern(pat)
char *pat;
{
	HIST_ENTRY *phe;
	char *ret;
...
static int history_should_ignore(line)
char *line;
{
