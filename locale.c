	...
	else if (var[3] == 'A') /* LC_ALL */
	{
		...
		if (x == 0) {
			if (errno == 0)
				...
				internal_warning(_("setlocale: LC_ALL: cannot change locale (%s): %s"), lc_all,
				                 strerror(errno));
		}
		locale_setblanks();
		locale_mb_cur_max = MB_CUR_MAX;
...
/* $"..." -- Translate the portion of STRING between START and END
   ...
   if non-null. */
char *locale_expand(string, start, end, lineno, lenp)
char *string;
...
static int locale_isutf8(lspec)
char *lspec;
{
	char *cp, *encoding;
