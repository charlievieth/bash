...
void posix_readline_initialize(on_or_off) int on_or_off;
{
...
void reset_completer_word_break_chars() {
	rl_completer_word_break_characters = perform_hostname_completion ?
...
int enable_hostname_completion(on_or_off)
int on_or_off;
{
	int old_value;
	char *nv, *nval;
...
static void initialize_hostname_list() {
	char *temp;
...
static void add_host_name(name) char *name;
{
...
static void snarf_hosts_from_file(filename) char *filename;
{
	FILE *file;
...
static char **hostnames_matching(text)
char *text;
{
...
char *edit_command;
{
	...
	r = parse_and_execute(command, (editing_mode == VI_EDITING_MODE) ? "v" : "C-xC-e",
	                      SEVAL_NOHIST);
...
#if defined(VI_MODE)
static int vi_edit_and_execute_command(count, c)
int count, c;
{
	if (posixly_correct)
...
static int emacs_edit_and_execute_command(count, c)
int count, c;
{ return (edit_and_execute_command(count, c, EMACS_EDITING_MODE, EMACS_EDIT_COMMAND)); }

#if defined(ALIAS)
static int posix_edit_macros(count, key)
int count, key;
{
	int c;
...
static int bash_forward_shellword(count, key)
int count, key;
{
	size_t slen;
	int c, p;
...
static int bash_kill_shellword(count, key)
int count, key;
{
	int p;
...
static int bash_backward_kill_shellword(count, key)
int count, key;
{
	int p;
...
static int bash_spell_correct_shellword(count, key)
int count, key;
{
 ...
 */
static int find_cmd_start(start)
int start;
{
...
static char *find_cmd_name(start, sp, ep)
int start;
int *sp, *ep;
{
	char *name;
...
static char *prog_complete_return(text, matchnum) const char *text;
int matchnum;
{
	static int ind;
...
/* Try and catch completion attempts that are syntax errors or otherwise
   invalid. */
static int invalid_completion(text, ind) const char *text;
int ind;
{
	int pind;
...
static char **attempt_shell_completion(text, start, end) const char *text;
int start, end;
{
	...
	if (in_command_position > 0 && ti >= 0 && rl_line_buffer[ti] == '`' && *text != '`' &&
	    unclosed_pair(rl_line_buffer, end, "`") == 0)
	...
	if (*text == '`' && rl_completion_quote_character != '\'' &&
	    (in_command_position > 0 ||
	     (unclosed_pair(rl_line_buffer, start, "`") && unclosed_pair(rl_line_buffer, end, "`"))))
	...
	if (matches == 0 &&
	   ...
	   if (e1 == end) we are at the end of the command name and completing it */
		...
		else if (e > s && was_assignment == 0 && e1 == end && rl_line_buffer[e] == 0 &&
		...
		else if (s >= e && n[0] == '\0' && text[0] == '\0' && start > 0 && was_assignment == 0 &&
...
char **bash_default_completion(text, start, end, qc, compflags) const char *text;
...
#define CMD_IS_DIR(x) \
	(absolute_pathname(x) == 0 && absolute_program(x) == 0 && *(x) != '~' && test_for_directory(x))
...
static int bash_command_name_stat_hook(name)
char **name;
{
	char *cname, *result;
...
static int executable_completion(filename, searching_path) const char *filename;
int searching_path;
{
	char *f, c;
	int r;
...
static char *command_subst_completion_function(text, state) const char *text;
int state;
{
...
static char *variable_completion_function(text, state) const char *text;
int state;
{
...
static char *hostname_completion_function(text, state) const char *text;
int state;
{
 ...
 */
char *bash_servicename_completion_function(text, state) const char *text;
int state;
{
...
static char *history_expand_line_internal(line)
char *line;
{
	char *new_line;
	int old_verify;
...
/* There was an error in expansion.  Let the preprocessor print
   the error here. */
static void cleanup_expansion_error() {
	char *to_free;
#if defined(BANG_HISTORY)
	int old_verify;
...
static void set_up_new_line(new_line) char *new_line;
{
...
static int alias_expand_line(count, ignore)
int count, ignore;
{
	char *new_line;
...
static int history_expand_line(count, ignore)
int count, ignore;
{
	char *new_line;
...
static int history_and_alias_expand_line(count, ignore)
int count, ignore;
{
	char *new_line, *t;
...
/* History and alias expand the line, then perform the shell word
   ...
   set of operations. */
static int shell_expand_line(count, ignore)
int count, ignore;
{
	char *new_line, *t;
...
static void _ignore_completion_names(names, name_func) char **names;
...
static int name_is_acceptable(name) const char *name;
{
	struct ign *p;
	int nlen;
...
static int test_for_directory(name) const char *name;
{
	char *fn;
	int r;
...
static int test_for_canon_directory(name) const char *name;
{
	char *fn;
	int r;
...
static int bash_ignore_filenames(names)
char **names;
{
...
static int bash_progcomp_ignore_filenames(names)
char **names;
{
...
static int return_zero(name) const char *name;
{ return 0; }

static int bash_ignore_everything(names)
char **names;
{
...
static char *maybe_restore_tilde(val, directory_part)
char *val, *directory_part;
{
	rl_icppfunc_t *save;
	char *ret;
...
static void bash_directory_expansion(dirname) char **dirname;
{
	char *d, *nd;
...
static rl_icppfunc_t *save_directory_hook() {
	rl_icppfunc_t *ret;
...
static void restore_directory_hook(hookf) rl_icppfunc_t *hookf;
{
	if (dircomplete_expand)
...
static int directory_exists(dirname, should_dequote) const char *dirname;
int should_dequote;
{
	char *new_dirname;
	int dirlen, r;
	...
	new_dirname = should_dequote ?
	                  bash_dequote_filename((char *)dirname, rl_completion_quote_character) :
	                  savestring(dirname);
...
static int bash_directory_completion_hook(dirname)
char **dirname;
{
	char *local_dirname, *new_dirname, *t;
	   ...
	   *local_dirname != '/' && *local_dirname != '.' == relative pathname
	   ...
	   is not changed by canonicalization as described above. */
		if (dircomplete_expand_relpath ||
		    ((local_dirname[0] != '/' && local_dirname[0] != '.') && STREQ(temp1, temp2) == 0))
...
static char *history_completion_generator(hint_text, state) const char *hint_text;
int state;
{
...
static int dynamic_complete_history(count, key)
int count, key;
{
	int r;
...
static int bash_dabbrev_expand(count, key)
int count, key;
{
...
static int bash_possible_username_completions(ignore, ignore2)
int ignore, ignore2;
{ return bash_complete_username_internal('?'); }

static int bash_complete_username_internal(what_to_do)
int what_to_do;
{ return bash_specific_completion(what_to_do, rl_username_completion_function); }

static int bash_complete_filename(ignore, ignore2)
int ignore, ignore2;
{ return bash_complete_filename_internal(rl_completion_mode(bash_complete_filename)); }

static int bash_possible_filename_completions(ignore, ignore2)
int ignore, ignore2;
...
static int bash_complete_hostname(ignore, ignore2)
int ignore, ignore2;
{ return bash_complete_hostname_internal(rl_completion_mode(bash_complete_hostname)); }

static int bash_possible_hostname_completions(ignore, ignore2)
int ignore, ignore2;
{ return bash_complete_hostname_internal('?'); }

static int bash_complete_variable(ignore, ignore2)
int ignore, ignore2;
{ return bash_complete_variable_internal(rl_completion_mode(bash_complete_variable)); }

static int bash_possible_variable_completions(ignore, ignore2)
int ignore, ignore2;
{ return bash_complete_variable_internal('?'); }

static int bash_complete_command(ignore, ignore2)
int ignore, ignore2;
{ return bash_complete_command_internal(rl_completion_mode(bash_complete_command)); }

static int bash_possible_command_completions(ignore, ignore2)
int ignore, ignore2;
{ return bash_complete_command_internal('?'); }

static int bash_complete_hostname_internal(what_to_do)
int what_to_do;
...
static int bash_complete_command_internal(what_to_do)
int what_to_do;
{ return bash_specific_completion(what_to_do, command_word_completion_function); }

static int completion_glob_pattern(string)
char *string;
...
static char *glob_complete_word(text, state) const char *text;
int state;
{
	static char **matches = (char **)NULL;
	static int ind;
...
static int bash_glob_completion_internal(what_to_do)
int what_to_do;
...
static char *bash_glob_quote_filename(s, rtype, qcp)
char *s;
int rtype;
char *qcp;
{
...
static int bash_glob_expand_word(count, key)
int count, key;
{ return bash_glob_completion_internal('*'); }

static int bash_glob_list_expansions(count, key)
int count, key;
{ return bash_glob_completion_internal('?'); }

static int bash_specific_completion(what_to_do, generator)
int what_to_do;
...
static char *bash_dequote_filename(text, quote_char)
char *text;
int quote_char;
{
	char *ret, *p, *r;
...
static char *quote_word_break_chars(text)
char *text;
{
	char *ret, *r, *s;
	int l;
...
static void set_filename_quote_chars(expchar, nextch, closer) int expchar, nextch, closer;
{
	int i, j, c;
...
/* Use characters in STRING to populate the table of characters that should
   ...
   this file. */
static void set_filename_bstab(string) const char *string;
{
	const char *s;
...
char *qcp;
{
	...
	if (*qcp == '\0' && cs == COMPLETE_BSQUOTE && dircomplete_expand == 0 &&
	    (expchar = bash_check_expchar(s, 0, &nextch, &closer)) && file_exists(s) == 0) {
...
static int readline_get_char_offset(ind)
int ind;
{
	int r, old_ch;
...
static void readline_set_char_offset(ind, varp) int ind;
int *varp;
{
	int i;
...
int bash_execute_unix_command(count, key)
int count; /* ignored */
int key;
{
	int type;
...
static void init_unix_command_map() {
...
static Keymap get_cmd_xmap_from_edit_mode() {
...
static int isolate_sequence(string, ind, need_dquote, startp)
char *string;
...
int bind_keyseq_to_unix_command(line)
char *line;
{
...
char **bash_directory_completion_matches(text) const char *text;
{
	char **m1;
	char *dfn;
	int qc;
...
char *bash_dequote_text(text) const char *text;
{
	char *dtxt;
	int qc;
