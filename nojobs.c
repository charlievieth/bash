...
static int find_index_by_pid(pid)
pid_t pid;
{
	register int i;
...
static int find_status_by_pid(pid)
pid_t pid;
{
	int i;
...
/* Return the status of PID as looked up in the PID_LIST array.  A
   return value of PROC_BAD indicates that PID wasn't found. */
static int find_termsig_by_pid(pid)
pid_t pid;
{
	int i;
...
static void set_pid_status(pid, status) pid_t pid;
WAIT status;
{
	int slot;
...
static void set_pid_flags(pid, flags) pid_t pid;
int flags;
{
	int slot;
...
static void unset_pid_flags(pid, flags) pid_t pid;
int flags;
{
	int slot;
...
static void add_pid(pid, async) pid_t pid;
int async;
{
	int slot;
...
/* Stop a pipeline. */
int stop_pipeline(async, ignore)
int async;
COMMAND *ignore;
{
...
int get_job_by_pid(pid, block, ignore)
pid_t pid;
int block;
PROCESS **ignore;
{
