...
void add_alias(name, value) char *name, *value;
{
	BUCKET_CONTENTS *elt;
	alias_t *temp;
	int n;
...
static void sort_aliases(array) alias_t **array;
...
static int skipquotes(string, start)
char *string;
int start;
{
	register int i;
...
/* Skip the white space and any quoted characters in STRING, starting at
   ...
   have been skipped. */
static int skipws(string, start)
char *string;
int start;
{
	register int i;
...
static int rd_token(string, start)
char *string;
int start;
{
	register int i;
...
char *string;
{
	...
	for (;;) {
	   ...
	   are sure we are expanding this token. */
		...
		if ((token[0]) && (expand_this_token || alias_expand_all) && (alias = find_alias(token))) {
			char *v;
			int vlen, llen;
			...
			if ((expand_this_token && vlen && whitespace(v[vlen - 1])) || alias_expand_all)
				expand_next = 1;
		} else {
			int llen, tlen;
