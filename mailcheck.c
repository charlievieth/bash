...
static void init_mail_file(i) int i;
{
	mailfiles[i]->access_time = mailfiles[i]->mod_time =
	    last_time_mail_checked ? last_time_mail_checked : shell_start_time;
	mailfiles[i]->file_size = 0;
	mailfiles[i]->flags = 0;
}

static void update_mail_file(i) int i;
{
	char *file;
	struct stat finfo;
...
static int add_mail_file(file, msg)
char *file, *msg;
{
	struct stat finfo;
	char *filename;
...
static void dispose_mail_file(mf) FILEINFO *mf;
{
	free(mf->name);
	FREE(mf->msg);
	free(mf);
...
static int file_mod_date_changed(i)
int i;
{
	time_t mtime;
	struct stat finfo;
...
static int file_access_date_changed(i)
int i;
{
	time_t atime;
	struct stat finfo;
...
static char *parse_mailpath_spec(str)
char *str;
{
	char *s;
	int pass_next;
