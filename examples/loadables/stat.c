...
static char *arraysubs[] = {"name",    "device", "inode", "type",  "nlink", "uid",
...
static char *stattime(t, timefmt)
time_t t;
const char *timefmt;
{
	char *tbuf, *ret;
...
static char *statval(which, fname, flags, fmt, sp)
int which;
char *fname;
int flags;
char *fmt;
...
static int loadstat(vname, var, fname, flags, fmt, sp)
char *vname;
SHELL_VAR *var;
char *fname;
int flags;
...
char *stat_doc[] = {"Load an associative array with file status information.",
                    "",
