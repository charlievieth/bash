...
static char *long_double_format(char const *fmt) {
	size_t i;
	...
	for (i++;; i += (fmt[i] == '%') + 1)
		if (fmt[i] == '%' && fmt[i + 1] != '%') {
			...
			strcpy(ldfmt + length_modifier_offset + 1, fmt + length_modifier_offset + has_L);
#else
...
static char *genformat(first, incr, last)
...
int print_intseq(ifirst, ilast, iincr)
...
WORD_LIST *list;
{
	...
	if (last_str && all_digits(last_str) && (first_str == 0 || all_digits(first_str)) &&
...
char *seq_doc[] = {"Print numbers from FIRST to LAST, in steps of INCREMENT.",
                   "",
