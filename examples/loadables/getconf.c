...
static const struct conf vars[] = {
    ...
    {"_POSIX_THREAD_ROBUST_PRIO_INHERIT", _SC_THREAD_ROBUST_PRIO_INHERIT, SYSCONF},
#endif
#ifdef _SC_THREAD_ROBUST_PRIO_PROTECT
    {"_POSIX_THREAD_ROBUST_PRIO_PROTECT", _SC_THREAD_ROBUST_PRIO_PROTECT, SYSCONF},
#endif
...
static int getconf_internal(const struct conf *c, int all) {
	long l, val;
	intmax_t v;
	uintmax_t uv;
	unsigned long ul;
...
static int getconf_one(WORD_LIST *list) {
	const struct conf *c;
	char *vname, *vpath;
	...
	for (c = vars; c->name != NULL; ++c) {
		if (strcmp(c->name, vname) == 0 ||
		    (strncmp(c->name, "_POSIX_", 7) == 0 && strcmp(c->name + 7, vname) == 0))
			break;
	}
	if (c->name == NULL) {
...
static int getconf_print(const struct conf *c, const char *vpath, int all) {
	long value;
	size_t clen;
	char *cvalue;
	int cn;
...
int getconf_builtin(WORD_LIST *list) {
	const struct conf *c;
...
static char *getconf_doc[] = {"Display values of system limits and options.", "",
                              "getconf writes the current value of a configurable system limit or",
                              "option variable to the standard output.", (char *)NULL};
